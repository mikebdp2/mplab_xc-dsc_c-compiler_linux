...
struct costs *debug_costs;
void set_debug_costs(char *arr, int num) {
  debug_costs = COSTS(arr,num);
}
...
void print_debug_costs(cost_classes_t cost_class_ptr) {
  int i;
  for (i = 0; i < cost_class_ptr->num; i++) {
    fprintf(stderr,"%d: %s = %d\n", 
                   i, reg_class_names[cost_class_ptr->classes[i]],
                   debug_costs->cost[i]);
  }
}
