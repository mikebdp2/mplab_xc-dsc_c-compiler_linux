...
#ifdef _BUILD_C30_
/* Need tm_p.h to call target functions pic30_extended_pointer_integer_type */
#include "tm_p.h"
#endif
...
#ifdef _BUILD_C30_
static void
move_variant_to_index (tree type, struct mem_address *parts, aff_tree *addr, tree v)
#else
static void
...
#ifdef _BUILD_C30_
  parts->index = fold_convert (this_sizetype, val);
  parts->step = wide_int_to_tree (this_sizetype, addr->elts[i].coef);
#else
			      ...
			      aff_tree *addr, bool speed)
{
  ...
  parts->step = wide_int_to_tree (this_sizetype, best_mult);
#else
	       ...
	       struct mem_address *parts, bool *var_in_base, bool speed)
{
  ...
  unsigned i;
#ifdef _BUILD_C30_
  /* Pointer mode may not fit sizetype, we need to modify the parts to use
     pointer mode type instead of sizetype (integer_type). This allows proper
     pointer arithmetic for memory located in different address spaces and
     take into account proper overflow or underflows */
  tree this_sizetype = sizetype;

#ifdef TARGET_POINTER_SIZETYPE
  if (base_hint) {
    this_sizetype = TARGET_POINTER_SIZETYPE(TREE_TYPE(base_hint));
  } else if ((addr->type) && (POINTER_TYPE_P(addr->type))) {
    this_sizetype = TARGET_POINTER_SIZETYPE(addr->type);
  }
#endif
  if (TYPE_ADDR_SPACE(type) != ADDR_SPACE_GENERIC) {
    this_sizetype = pic30_extended_pointer_integer_type(
  ...
  if (maybe_ne (addr->offset, 0))
    parts->offset = wide_int_to_tree (this_sizetype, addr->offset);
  else
  ...
  *var_in_base = (base_hint != NULL && parts->symbol == NULL);
  if (*var_in_base)
    *var_in_base = move_hint_to_base (type, parts, base_hint, addr);
    ...
    move_variant_to_index (type, parts, addr, iv_cand);
#else
  *var_in_base = (base_hint != NULL && parts->symbol == NULL);
  if (*var_in_base)
    *var_in_base = move_hint_to_base (type, parts, base_hint, addr);
  ...
  for (i = 0; i < addr->n; i++)
	...
	part = fold_build2 (MULT_EXPR, this_sizetype, part,
			    wide_int_to_tree (this_sizetype, addr->elts[i].coef));
		...
		tree alias_ptr_type, tree iv_cand, tree base_hint, bool speed)
{
  ...
  if (parts.symbol)
      ...
      if (parts.base)
	{
          tree this_sizetype = sizetype;
#ifdef TARGET_POINTER_SIZETYPE
          this_sizetype = pic30_extended_pointer_integer_type(TYPE_MODE(TREE_TYPE(parts.base)));
#endif
	  gcc_assert (useless_type_conversion_p (this_sizetype,
