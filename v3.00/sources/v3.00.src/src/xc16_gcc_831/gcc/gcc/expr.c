...
convert_mode_scalar (rtx to, rtx from, int unsignedp)
{
  ...
  if (GET_MODE_CLASS (from_mode) == MODE_PARTIAL_INT)
   ...
   scalar_int_mode int_to_mode,int_from_mode;
   if (is_int_mode (to_mode, &int_to_mode) &&
       is_int_mode (from_mode, &int_from_mode) &&
       (targetm.valid_pointer_mode(int_to_mode) ||
          targetm.valid_pointer_mode(int_from_mode))) {
...
emit_move_resolve_push (machine_mode mode, rtx x)
{
  ...
  /* Do not use anti_adjust_stack, since we don't want to update
     stack_pointer_delta.  */
  temp = expand_simple_binop (STACK_Pmode, PLUS, stack_pointer_rtx,
			      gen_int_mode (adjust, STACK_Pmode), stack_pointer_rtx,
			      0, OPTAB_LIB_WIDEN);
#else
  /* Do not use anti_adjust_stack, since we don't want to update
     stack_pointer_delta.  */
  ...
  switch (code)
    ...
    case POST_MODIFY:
#ifdef _BUILD_C30_
      temp = plus_constant (STACK_Pmode, stack_pointer_rtx, -adjust);
#else
...
push_block (rtx size, poly_int64 extra, int below)
{
  ...
  if (CONSTANT_P (size))
    anti_adjust_stack (plus_constant (STACK_Pmode, size, extra));
  ...
  else
    {
      ...
      if (maybe_ne (extra, 0))
	temp = expand_binop (STACK_Pmode, add_optab, temp,
			     gen_int_mode (extra, STACK_Pmode),
  ...
  if (STACK_GROWS_DOWNWARD)
	...
	temp = plus_constant (STACK_Pmode, temp, extra);
    }
  else
    {
      if (CONST_INT_P (size))
	temp = plus_constant (STACK_Pmode, virtual_outgoing_args_rtx,
			      -INTVAL (size) - (below ? 0 : extra));
      else if (maybe_ne (extra, 0) && !below)
	temp = gen_rtx_PLUS (STACK_Pmode, virtual_outgoing_args_rtx,
			     negate_rtx (STACK_Pmode, plus_constant (STACK_Pmode, size,
							       extra)));
      else
	temp = gen_rtx_PLUS (STACK_Pmode, virtual_outgoing_args_rtx,
...
emit_single_push_insn_1 (machine_mode mode, rtx x, tree type)
{
  ...
  /* If we are to pad downward, adjust the stack pointer first and
  ...
  else if (targetm.calls.function_arg_padding (mode, type) == PAD_DOWNWARD)
    {
      ...
      emit_move_insn (stack_pointer_rtx,
		      expand_binop (STACK_Pmode,
				    ...
				    : add_optab,
				    stack_pointer_rtx,
				    gen_int_mode (rounded_size, STACK_Pmode),
...
#ifdef _BUILD_C30_
      dest_addr = plus_constant (STACK_Pmode, stack_pointer_rtx, offset);
#else
	...
	dest_addr = plus_constant (STACK_Pmode, stack_pointer_rtx, -rounded_size);
      else
	/* ??? This seems wrong if STACK_PUSH_CODE == POST_INC.  */
	dest_addr = plus_constant (STACK_Pmode, stack_pointer_rtx, rounded_size);

      dest_addr = gen_rtx_PRE_MODIFY (STACK_Pmode, stack_pointer_rtx, dest_addr);
#else
      if (STACK_GROWS_DOWNWARD)
	/* ??? This seems wrong if STACK_PUSH_CODE == POST_DEC.  */
...
#ifdef _BUILD_C30_
  /* Surely the Address Space also determines whether or not an
   * unaligned access is supportable? */
#endif
...
#ifdef _BUILD_C30_
	    else if (targetm.slow_unaligned_access (mode, align, as))
#else
