		      ...
		      int align, int kind)
{
 ...
 found_space:
  ...
  if (virtuals_instantiated)
    addr = plus_constant (STACK_Pmode, frame_pointer_rtx,
			  trunc_int_for_mode
			  (slot_offset + bigend_correction
			   + targetm.starting_frame_offset (), STACK_Pmode));
  else
    addr = plus_constant (STACK_Pmode, virtual_stack_vars_rtx,
			  trunc_int_for_mode
			  (slot_offset + bigend_correction,
			   STACK_Pmode));
#else
  ...
  if (data->passed_pointer)
    {
      ...
      if ((TARGET_EDS) & (TARGET_EDS_MODE==P32UMMmode)) {
        // unless already set by set_mem_attributes
        if (MEM_ADDR_SPACE(rtl) == ADDR_SPACE_GENERIC) {
          set_mem_addr_space(rtl, pic30_space_stack);
        }
      }
#endif
    }
  ...
  if (data->passed_pointer && TYPE_MODE (TREE_TYPE (parm)) != BLKmode)
      ...
      else
	{
	  ...
	  set_mem_attributes (parmreg, parm, 1);
          ...
          if ((TARGET_EDS) & (TARGET_EDS_MODE==P32UMMmode)) {
            // unless already set by set_mem_attributes
            if (MEM_ADDR_SPACE(parmreg) == ADDR_SPACE_GENERIC) {
              set_mem_addr_space(parmreg,pic30_space_stack);
            }
          }
#endif
