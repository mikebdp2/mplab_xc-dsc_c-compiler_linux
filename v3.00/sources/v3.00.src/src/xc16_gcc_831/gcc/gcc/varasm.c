		...
		error ("section type conflict with %qD", sect->named.decl);
              ...
              { char *variable_flags;
                ...
                inform(DECL_SOURCE_LOCATION(decl), "%s section flags: %s",
                         name, section_flags);
                free(variable_flags); // Free the memory allocated by 
                                      //   pic30_report_section_flags_info
                free(section_flags);
              }
#else
...
#ifdef _BUILD_C30_
  desc->last_decl = last_assemble_variable_decl; 
  desc->section_name = NULL;
#endif
...
#ifdef _BUILD_C30_
      desc->section_name = pic30_get_constant_section();
#endif
...
output_constant_def (tree exp, int defer)
{
  ...
  if (desc == 0)
    {
#ifdef _BUILD_C30_
      // make a new copy of exp so that we can track whether or not it
      //   has been written
      exp = copy_node(exp);
#endif
      ...
      desc->hash = key.hash;
#ifdef _BUILD_C30_
      // it may have been written, but its not anymore...
...
#ifdef _BUILD_C30_
  /* CAW NB : might wrap this in decl_end(); ???? */
...
/* also, MIPS only */
#if (defined(_BUILD_MCHP_) && defined(TARGET_IS_PIC32MX)) || (defined(_BUILD_C30_))
          ...
          mchp_cache_conversion_state(x, insn, sym);
#endif
		  n_deferred_constants--;
  ...
  switch (TREE_CODE (exp))
	...
	output_constant_def (tem, 0);
        if ((addr_space_qualified == 0) && TARGET_CONST_IN_CODE &&
            TREE_READONLY(tem)) {
          // put it back
          pic30_set_constant_section(current_section_name,
                                     current_section_flags,
                                     current_decl);
        }
      }
	...
	output_constant_def (tem, 0);
      ...
      if (TREE_CODE (tem) == MEM_REF)
	output_addressed_constants_1 (TREE_OPERAND (tem, 0), addr_space_qualified);
      break;
    ...
    case VIEW_CONVERT_EXPR:
#if defined(_BUILD_C30_)
      output_addressed_constants_1 (TREE_OPERAND (exp, 0),addr_space_qualified);
#else
