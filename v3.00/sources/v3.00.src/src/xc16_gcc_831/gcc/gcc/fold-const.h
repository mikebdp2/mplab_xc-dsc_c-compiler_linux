...
#ifdef _BUILD_C30_
/* It will be usefull to have the ptr passed in as well, so that we can convert
   the offset to ptr type instead of 'just' sizetype. */
extern tree convert_to_ptrofftype_loc (location_t loc, tree off, tree ptr);
#define convert_to_ptrofftype(t) convert_to_ptrofftype_loc (UNKNOWN_LOCATION, t, 0)
#else
