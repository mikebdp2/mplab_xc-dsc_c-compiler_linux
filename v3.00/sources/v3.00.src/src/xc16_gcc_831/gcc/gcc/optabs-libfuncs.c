...
#ifdef _BUILD_C30_
  char *libfunc_name = XALLOCAVEC (char, len + sizeof(PIC30_QLIBFN_FLAG));
#else
			     ...
			     machine_mode fmode)
{
  ...
  if (ALL_FIXED_POINT_MODE_P(tmode) || ALL_FIXED_POINT_MODE_P(fmode)) {
    sprintf(nondec_name, "%s",PIC30_QLIBFN_FLAG);
    pre_prefix_len = strlen(PIC30_QLIBFN_FLAG);
  }
#else
  nondec_name = XALLOCAVEC (char, prefix_len + opname_len + mname_len + 1 + 1);
#endif
  /* this code used to be more flexible; not relying on fixed indecies... */
  nondec_name[pre_prefix_len+0] = '_';
  nondec_name[pre_prefix_len+1] = '_';
  if (targetm.libfunc_gnu_prefix)
    {
      nondec_name[pre_prefix_len+2] = 'g';
      nondec_name[pre_prefix_len+3] = 'n';
      nondec_name[pre_prefix_len+4] = 'u';
      nondec_name[pre_prefix_len+5] = '_';
    }

  memcpy (&nondec_name[pre_prefix_len+prefix_len], opname, opname_len);
  nondec_suffix = nondec_name + opname_len + prefix_len + pre_prefix_len;
...
#endif
  dec_name[pre_prefix_len+0] = '_';
  dec_name[pre_prefix_len+1] = '_';
  memcpy (&dec_name[pre_prefix_len+2], DECIMAL_PREFIX, dec_len);
  memcpy (&dec_name[pre_prefix_len+2+dec_len], opname, opname_len);
  dec_suffix = dec_name + dec_len + opname_len + pre_prefix_len + 2;
			     ...
			     machine_mode tmode, machine_mode fmode)
{
  ...
  memcpy (&nondec_name[pre_prefix_len+prefix_len], opname, opname_len);
  nondec_suffix = nondec_name + opname_len + pre_prefix_len+prefix_len;
  ...
  memcpy (&dec_name[pre_prefix_len+2], DECIMAL_PREFIX, dec_len);
  memcpy (&dec_name[pre_prefix_len+2 + dec_len], opname, opname_len);
  dec_suffix = pre_prefix_len+dec_name + dec_len + opname_len + 2;
