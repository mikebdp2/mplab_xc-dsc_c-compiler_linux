      ...
      if ((flags & ECF_BY_DESCRIPTOR) && !flag_trampolines)
	{
	 ...
	 function address into a register now.  */
      ...
      funexp = ((reg_parm_seen
		 ...
		 : memory_address_addr_space (FUNCTION_MODE, funexp,
                                              pic30_space_stack));
    /* by default the below code pushes the fn-pointer to the 'generic' address
...
#ifdef _BUILD_C30_
      if (GET_MODE (funexp) != FN_Pmode)
	funexp = convert_memory_address (FN_Pmode, funexp);
#else
      if (GET_MODE (funexp) != Pmode)
	funexp = convert_memory_address (Pmode, funexp);
#endif
...
#ifdef _BUILD_C30_
      if (tfom == 0) {
        /* Maybe it is an unsigned type mode */
        tfom = lang_hooks.types.type_for_mode (outmode, 1);
      }
#endif
