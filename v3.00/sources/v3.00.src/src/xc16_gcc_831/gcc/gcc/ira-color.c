...
#ifdef _BUILD_C30_
void
debug_spilled_pseudo_regs(char *file, int line, 
                          int *spilled_pseudo_regs, int num) {
  int i;

  fprintf(stderr,"%s:%d spilled_pseudo_regs: %d\n", file, line, num);
  for (i = 0; i < num; i++) {
    fprintf(stderr,"%d:%d%c", i, spilled_pseudo_regs[i], ((i&7)==7)?'\n':' ');
  }
  fprintf(stderr,"\n");
}
#endif
		      ...
		      poly_uint64 total_size)
{
  ...
  /* initiate_allocnos/ create_allocnos does not create an allocno for
     every pseudo up to max_reg_num(),  but reload calls alter_reg from
