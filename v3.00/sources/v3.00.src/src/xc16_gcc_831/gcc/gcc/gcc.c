...
#if defined(TARGET_IS_PIC32C) || defined(TARGET_IS_PIC32MX) || defined(_BUILD_C30_)
	     ...
	     bool do_multi)
{
  ...
  temp_name = (char *)xmalloc(sizeof("xc-dsc-")+strlen(name)+1);
  if (temp_name)  {
    sprintf(temp_name, "%s-%s", "xc-dsc", name);
    info.name = temp_name;
  } else info.name = name;
  ...
  ret = for_each_path (pprefix, do_multi,
		       info.name_len + info.suffix_len,
		       file_at_path, &info);

  if (!ret) {
    // ret is null, we couldn't find xc16- version, then try for the non xc16- version
    info.name = name;
    info.suffix = (mode & X_OK) != 0 ? HOST_EXECUTABLE_SUFFIX : "";
    info.name_len = strlen (info.name);
    info.suffix_len = strlen (info.suffix);
		 ...
		 struct cl_decoded_option *decoded_options)
{
  ...
  unsigned int i = 0;
  {
    ...
    {
      if (!strncmp(decoded_options[i].orig_option_with_args_text, 
           "-mdfp=", 
           sizeof("-mdfp=")-1))
      {
        /* Make the base path point to pic30_dfp/bin/. Add +6 i.e strlen(-mdfp)
         * since we will not use the string part '-mdfp='  */
        base_path = (char *)xmalloc(strlen(decoded_options[i].orig_option_with_args_text)+1);
        sprintf(base_path, "%s/bin/", decoded_options[i].orig_option_with_args_text+6);
      } 
    }
  if (base_path) {
    const char *library_paths[] = { LIBRARY_PATH_ENV, "LPATH", 0};

    int pathNo = 0;

    while (library_paths[pathNo])
    {
      temp = env.get(library_paths[pathNo]);
  ...
  }
  {
    ...
    base_path = make_relative_prefix(
                  decoded_options[0].orig_option_with_args_text,
                  standard_bindir_prefix,
                  standard_exec_prefix);
...
# endif
  ...
  char *default_pic30_resource_file;
  /* -mresource= is now available here? at least the symbol is */
  if (pic30_resource_file == NULL) 
    { char *c;
      int l = 0;

      default_pic30_resource_file = (char *)xmalloc(strlen(gcc_exec_prefix) +
                                    sizeof("/c30_device.info"));
      gcc_assert(default_pic30_resource_file);
      /* up three levels */
      strcpy(default_pic30_resource_file, gcc_exec_prefix);
      c = default_pic30_resource_file+strlen(gcc_exec_prefix)-1;
      while ((c > default_pic30_resource_file) && (l < 3)) {
        c--;
      ...
      *c = 0;
      strcat(default_pic30_resource_file, "/c30_device.info");
    } 
      else default_pic30_resource_file = (char *)pic30_resource_file;
  rib = read_rib(default_pic30_resource_file);
#endif
