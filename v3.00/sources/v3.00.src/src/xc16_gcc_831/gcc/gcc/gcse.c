...
#ifdef _BUILD_C30_
/* insert_insn_end_basic_block will move an instruction to the end (obviously)
   but it doesn't really care that the last instruction might be a conditional
   branch and the instruction being inserted might clobber the CC...

   the old code would attempt to not split this up... but 
  
   look for a better insesrtion point... */
#define emit_insn_before_noloc pic30_emit_insn_before_noloc
#endif
