...
#if 0
#ifdef _BUILD_C30_
      /* we cannot propagate casts away from named address space pointers.
       * this might lose some important conversion steps.  (CAW)
       */
      if (POINTER_TYPE_P(TREE_TYPE(rhs)) &&
          (TYPE_ADDR_SPACE(TREE_TYPE(TREE_TYPE(rhs))) != ADDR_SPACE_GENERIC))
        return false;
#endif
#endif
    }
