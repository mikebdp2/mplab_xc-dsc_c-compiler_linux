...
static void cci_attribute(void *pfile_v,const char *keyword, const char *target,
  ...
  if (n) {
    for (const_c = target; *const_c; const_c++) {
      if ((*const_c == 'P') && (const_c[1] >= '0') && (const_c[1] <= '9')) {
        params_specified=1;
        break;
      }
    }
  ...
  while (1)
    {
      ...
      tok = pragma_lex (&tok_value);
      if (tok == CPP_NAME)
        value_name = (char *)IDENTIFIER_POINTER (tok_value);
      else if (tok == CPP_NUMBER)
        {
          if (tree_fits_uhwi_p (tok_value))
          {
            ...
            value_name = (char*)xcalloc(MAX_VALUE_NAME_LENGTH,1);
            snprintf(value_name, MAX_VALUE_NAME_LENGTH, "%u", i);
