...
void pic30_cpu_cpp_builtins(void *pfile_v) {
  ...
  if (pic30_fp_round_p()) {
    sprintf(buffer,"__XC_DSC_ROUND__=%d", pic30_fp_round_p());
    cpp_define(pfile,buffer);
  ...
  if (pic30_isa32_target()) {
    sprintf(buffer,"__STACK_ALIGNMENT_TYPE=long");
    cpp_define(pfile,buffer);
    sprintf(buffer,"__HAS_ISA32__");
    cpp_define(pfile,buffer);
  } else {
    sprintf(buffer,"__STACK_ALIGNMENT_TYPE=int");
    sprintf(buffer,"__HAS_BUILTINS_16__");
    cpp_define(pfile,buffer);
  }

  if (TARGET_EDS) {
    sprintf(buffer,"__XC_DSC_UMM__=1");
    cpp_define(pfile,buffer);
  ...
  if (pic30_isa32_target()) {
    /* address space keywords are not required for 33A devices;
       silently ignore code that uses them by defining them to nothing */
    char *ignore[] = {
      "__psv__",
      "__prog__",
      "__eds__",
...
void pic30_register_target_pragmas(void) {
  ...
  if (pic30_isa32_target()) {
    ...
    /* Regsiter the address space keywords */
    c_register_addr_space("__psv__", pic30_space_psv);
    c_register_addr_space("__prog__", pic30_space_prog);
    c_register_addr_space("__eds__", pic30_space_eds);
    c_register_addr_space("__pack_upper_byte", pic30_space_packed);
  }
  if (1) {
    c_register_addr_space("__pmp__", pic30_space_pmp);
    c_register_addr_space("__external__", pic30_space_external);
    c_register_addr_space("__const__", pic30_space_const);
    c_register_addr_space("__auto__", pic30_space_stack);
  }
