...
#ifdef _BUILD_C30_
  /* NOTE: for the crt implementation (cc_bits elements are bytes) this is not needed
           but it may help to have some alignment when copying to/from flash... */
  ASM_OUTPUT_ALIGN (asm_out_file, 2);

  /* the label */
  ASM_OUTPUT_LABEL (asm_out_file, cc_bits_name);

  /* size in bytes (rounded up from the no. of bits) */
  assemble_zeros ((num_rtl_cc_bits * TARGET_XCCOV_POINTSIZE + 7) / 8);
#else
  /* size in bytes (rounded up from the no. of bits) */
