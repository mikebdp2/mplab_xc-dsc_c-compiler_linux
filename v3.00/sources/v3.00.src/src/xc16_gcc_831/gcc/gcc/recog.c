...
extract_constrain_insn (rtx_insn *insn)
{
  ...
  /* VRT:
     The insn might have changed. And extract_insn calls recog_memoized() which
     is not an uncached extraction of insn. So set the code to -1, so that
     recog_memoized will re-recognize the pattern.  */
  INSN_CODE(insn) = -1;
#endif
  extract_insn (insn);
...
const pass_data pass_data_split_for_xc16 =
{
  RTL_PASS, /* type */
  "split_for_xc16", /* name */
  OPTGROUP_NONE, /* optinfo_flags */
...
class pass_split_for_xc16 : public rtl_opt_pass
{
public:
  pass_split_for_xc16 (gcc::context *ctxt)
    : rtl_opt_pass (pass_data_split_for_xc16, ctxt)
  {}
...
rtl_opt_pass *
make_pass_split_for_xc16 (gcc::context *ctxt)
{
  return new pass_split_for_xc16 (ctxt);
}
#endif
