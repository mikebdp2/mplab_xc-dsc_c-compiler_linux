...
#ifdef _BUILD_C30_
  x = plus_constant (STACK_Pmode, base, offset);
#else
  ...
  for (si = 0; si < n; ++si)
      ...
      if (alignb * BITS_PER_UNIT <= MAX_SUPPORTED_STACK_ALIGNMENT)
	{
	  ...
	  if (asan_sanitize_stack_p ()
		...
		data->asan_base = gen_reg_rtx (STACK_Pmode);
#else
	      if (data->asan_base == NULL)
	  ...
	  if (maybe_ne (large_size, 0U) && ! large_allocation_done)
	    {
	      ...
	      rtx large_allocsize;
#ifdef _BUILD_C30_
	      large_allocsize = gen_int_mode (large_size, STACK_Pmode);
#else
...
#ifdef _BUILD_C30_
  if (align > MAX_SUPPORTED_STACK_ALIGNMENT)
    align = GET_MODE_ALIGNMENT (STACK_Pmode);
#else
  if (align > MAX_SUPPORTED_STACK_ALIGNMENT)
    align = GET_MODE_ALIGNMENT (Pmode);
#endif
