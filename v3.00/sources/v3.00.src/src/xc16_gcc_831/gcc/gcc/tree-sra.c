...
#ifdef _BUILD_C30_
/* tried using TREE_VISITED; but... this turned out to be fragile
 * use a simple list
 */
struct node_seen {
  tree node;
  struct node_seen *next;
} *sra_nodes_seen,*sra_nodes_seen_free;

static void sra_seen_add_node(tree node) {
  struct node_seen *seen;

  if (sra_nodes_seen_free) {
    seen = sra_nodes_seen_free;
    sra_nodes_seen_free = sra_nodes_seen_free->next;
  } else seen = (struct node_seen*)xmalloc(sizeof(struct node_seen));
  seen->next = sra_nodes_seen;
  seen->node = node;
  sra_nodes_seen = seen;
}

static bool sra_seen_node_seen(tree node) {
  struct node_seen *seen;

  for (seen = sra_nodes_seen; seen; seen = seen->next) {
    if (seen->node == node) return true;
  }
  return false;
}

static void sra_seen_clear_node_seen(void) {
  struct node_seen *seen;
  for (seen = sra_nodes_seen; seen->next; seen = seen->next);
  seen->next = sra_nodes_seen_free;
  sra_nodes_seen_free = sra_nodes_seen;
  sra_nodes_seen = NULL;
}
#endif
...
static bool
#ifdef _BUILD_C30_
type_internals_preclude_sra_r (tree type, const char **msg)
#else
  ...
  switch (TREE_CODE (type))
...
#ifdef _BUILD_C30_
            if (sra_seen_node_seen(ft)) continue;
#endif
...
#ifdef _BUILD_C30_
	    if (AGGREGATE_TYPE_P (ft) && type_internals_preclude_sra_r (ft, msg))
#else
	    if (AGGREGATE_TYPE_P (ft) && type_internals_preclude_sra_p (ft, msg))
#endif
	      return true;
#ifdef _BUILD_C30_
            // pointers to things in a different address space cannot be
            //   scalarized.  pointers to these things cannot be converted
            //   the type must be maintained.
            if (((TREE_CODE(ft) == POINTER_TYPE) || 
                 (TREE_CODE(ft) == REFERENCE_TYPE)) &&
                (!sra_seen_node_seen(TREE_TYPE(ft)))) { 
              if (type_internals_preclude_sra_r(TREE_TYPE(ft),msg)) return true;
            }
#endif
	  }
...
#ifdef _BUILD_C30_
      if (sra_seen_node_seen(et)) return false;
#endif
      if (TYPE_VOLATILE (et))
	{
...
#ifdef _BUILD_C30_
      if (AGGREGATE_TYPE_P (et) && type_internals_preclude_sra_r (et, msg))
#else
    ...
    default:
      ...
      if (TYPE_ADDR_SPACE(type)) {
        *msg = "structure field is an address_space";
        return true;
      }
#endif
      return false;
...
type_internals_preclude_sra_p (tree type, const char **msg)
{
  ...
  result = type_internals_preclude_sra_r (type, msg);

  sra_seen_clear_node_seen();
  return result;
}
#endif
