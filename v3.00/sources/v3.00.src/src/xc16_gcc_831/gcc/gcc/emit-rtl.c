...
gen_rtx_REG (machine_mode mode, unsigned int regno)
{
  /* In case the MD file explicitly references the frame pointer, have
    ...
    */
  if (mode == machine_Pmode && !reload_in_progress && !lra_in_progress)
#else
				 ...
				 poly_int64 bitpos)
{
   ...
   * not determined by type. Don't change it unless we have explicitly made it
   * something else */
  if (as != mem_as) {
...
init_emit_regs (void)
{
  ...
  virtual_preferred_stack_boundary_rtx =
    gen_raw_REG (STACK_Pmode, VIRTUAL_PREFERRED_STACK_BOUNDARY_REGNUM);
#else
  /* Assign register numbers to the globally defined register rtx.  */
...
init_emit_once (void)
{
  ...
  if (opt_fstack_limit_symbol_arg != NULL)
    stack_limit_rtx 
      = gen_rtx_SYMBOL_REF (STACK_Pmode, ggc_strdup (opt_fstack_limit_symbol_arg));
  if (opt_fstack_limit_register_no >= 0)
    stack_limit_rtx = gen_rtx_REG (STACK_Pmode, opt_fstack_limit_register_no);
#else
  if (opt_fstack_limit_symbol_arg != NULL)
    stack_limit_rtx 
  ...
  for (i = 0; i < 3; i++)
	...
	const_tiny_rtx[i][(int) mode] = GEN_INT (i);
#ifdef _BUILD_C30_
      /* We need the below to populate const_tiny_rtx because they are target
       * pointers. They can still be compared to 0,1,-1. But the iterator for
       * FOR_EACH_MODE_IN_CLASS iterates to the next wider mode in class
       * instead of every class in mode... not what the name stands for, maybe
       * we should not have modes of the same size.*/
      const_tiny_rtx[i][E_P16PMPmode] = GEN_INT(i);
      const_tiny_rtx[i][E_P32EXTmode] = GEN_INT(i);
      const_tiny_rtx[i][E_P24PSVmode] = GEN_INT(i);
      const_tiny_rtx[i][E_P24PROGmode] = GEN_INT(i);
      const_tiny_rtx[i][E_P32PEDSmode] = GEN_INT(i);
      const_tiny_rtx[i][E_P32EDSmode] = GEN_INT(i);
      const_tiny_rtx[i][E_P16APSVmode] = GEN_INT(i);
      const_tiny_rtx[i][E_P32DFmode] = GEN_INT(i);
      const_tiny_rtx[i][E_P32UMMmode] = GEN_INT(i);
#endif
    }
  ...
  FOR_EACH_MODE_IN_CLASS (mode, MODE_INT)
    const_tiny_rtx[3][(int) mode] = constm1_rtx;
       ...
       * instead of every class in mode... not what the name stands for, maybe
       * we should not have modes of the same size.*/
      const_tiny_rtx[3][E_P16PMPmode] = constm1_rtx;
      const_tiny_rtx[3][E_P32EXTmode] = constm1_rtx;
      const_tiny_rtx[3][E_P24PSVmode] = constm1_rtx;
      const_tiny_rtx[3][E_P24PROGmode] = constm1_rtx;
      const_tiny_rtx[3][E_P32PEDSmode] = constm1_rtx;
      const_tiny_rtx[3][E_P32EDSmode] = constm1_rtx;
      const_tiny_rtx[3][E_P16APSVmode] = constm1_rtx;
      const_tiny_rtx[3][E_P32DFmode] = constm1_rtx;
      const_tiny_rtx[3][E_P32UMMmode] = constm1_rtx;
#endif
