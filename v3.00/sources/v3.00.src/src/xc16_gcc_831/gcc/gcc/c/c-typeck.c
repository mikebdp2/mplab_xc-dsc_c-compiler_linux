...
#if 1
  /* we might prefer to do this the previous way... look for a failure
     first ... since this new code looks like its supposed to compensate
     for different sized types */
#ifdef _BUILD_C30_
  {
...
#ifdef _BUILD_C30_
/* We have to pass in the pointer as well to correctly codify offset type */
	    inc = convert_to_ptrofftype_loc (location, inc, arg);
#else
	  ...
	  && TREE_CODE (argtype) == FUNCTION_TYPE)
	{
	  ...
	  if (TREE_THIS_VOLATILE (arg))
	    quals |= TYPE_QUAL_VOLATILE;
          ...
          quals |= ENCODE_QUAL_ADDR_SPACE(TYPE_ADDR_SPACE(argtype));
#endif
  ...
  if (constructor_type == NULL_TREE)
  ...
  else if (TREE_CODE (constructor_type) == ARRAY_TYPE)
    {
      ...
      if (TREE_OVERFLOW(constructor_index)) {
        error_init(loc, "Array too large");
        return;
      }
#endif
