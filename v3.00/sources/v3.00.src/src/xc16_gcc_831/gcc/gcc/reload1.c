...
#ifdef _BUILD_C30_
	/* Now that targets are moving to have CC reg is a register that
	   gets explictly clobbered... a simple move can be less simple */
	if (NONJUMP_INSN_P(insn)
	    && GET_CODE(PATTERN(insn)) == PARALLEL
	    && XVECLEN(PATTERN(insn),0) == 2) {
	  /* check for a simple move, as above, and a clobber of CC_REG */
	  rtx x;
	  ...
	  if ((GET_CODE(x) == SET)
	       && REG_P(SET_SRC(x))
	       && REG_P(SET_DEST(x))
	       && (REGNO(SET_SRC(x)) == REGNO(SET_DEST(x)))) {
	    x = XVECEXP(PATTERN(insn),0,1);
		  ...
		  bool may_use_invariant, bool for_costs)
{
  ...
  if ((GET_CODE(x) == MEM) && (GET_CODE(XEXP(x,0)) == PLUS)) {
    ...
    if ((GET_CODE(lhs) == REG) && (GET_CODE(rhs) == REG) &&
        ((REGNO(lhs) == FP_REGNO) || (REGNO(rhs) == FP_REGNO))) {
      /*  if we eliminate w14 it goes to (w15 - x), so we can't then
       *  offset it by a reg... lets not even try 
       */
      return x;
    }
  } 
      ...
      if (regno < FIRST_PSEUDO_REGISTER)
	{
	  for (ep = reg_eliminate; ep < &reg_eliminate[NUM_ELIMINABLE_REGS];
	    ...
	    if (ep->from_rtx == x && ep->can_eliminate)
	      return plus_constant (machine_Pmode, ep->to_rtx, ep->previous_offset);
#else
	    if (ep->from_rtx == x && ep->can_eliminate)
		...
		if (mem_mode != 0
		  ...
		  return ep->to_rtx;
		    ...
		    return gen_rtx_PLUS (STACK_Pmode, ep->to_rtx,
		  		       plus_constant (STACK_Pmode, XEXP (x, 1),
						      ep->previous_offset));
                  } else {
		    return gen_rtx_PLUS (Pmode, ep->to_rtx,
		  		       plus_constant (machine_Pmode, XEXP (x, 1),
						      ep->previous_offset));
                  }
                }
#else
...
#ifdef _BUILD_C30_
                /* Swapping of 'add' can create the situation
                 * where the literal is not longer cannonical.
                 * Making this code not want to eliminate W14.
                 * I see no rules against having a commutative add
                 * (in fact its explicitly mentioned), so 'just' 
                 * address it here.
                 */
		else if (GET_CODE (src) == PLUS
		    && XEXP (src, 1) == SET_DEST (x)
		    && CONST_INT_P (XEXP (src, 0)))
		  ep->offset -= INTVAL (XEXP (src, 0));
#endif
		else
		  ep->can_eliminate = 0;
	      }
...
#if defined( _BUILD_C30_) && 0
  /* clobber (nil) will cause an error ie, (clobber "=X,&r") */
  if (!x) return;
#endif
  code = GET_CODE (x);
