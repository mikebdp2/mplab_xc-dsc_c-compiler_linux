...
#ifdef _BUILD_C30_
tree
convert_to_ptrofftype_loc (location_t loc, tree off, tree ptr)
{
  tree this_sizetype = sizetype;
#ifdef TARGET_POINTER_SIZETYPE
  if (ptr != 0)
    this_sizetype = TARGET_POINTER_SIZETYPE(TREE_TYPE(ptr));
#endif
  return fold_convert_loc (loc, this_sizetype, off);
}
#else
tree
...
fold_build_pointer_plus_loc (location_t loc, tree ptr, tree off)
{
  ...
  return fold_build2_loc (loc, POINTER_PLUS_EXPR, TREE_TYPE (ptr),
			  ptr, convert_to_ptrofftype_loc (loc, off, ptr));
#else
  return fold_build2_loc (loc, POINTER_PLUS_EXPR, TREE_TYPE (ptr),
