...
side_effects_p (const_rtx x)
{
  ...
  switch (code)
    ...
    case UNSPEC_VOLATILE:
#ifdef _BUILD_C30_
    /* not sure why UNSPEC does not induce side effects since we don't 
       really know if the operation is merge-able.  I suppse we could use
       unspec voltatile everywhere... 
 
       this may prevent deletion, anyway... in which case we should *just*
       change our unspecs to unspec_volatile */
    case UNSPEC:
#endif
      return 1;
