...
vn_reference_eq (const_vn_reference_t const vr1, const_vn_reference_t const vr2)
{
  unsigned i, j;
#ifdef _BUILD_C30_
  /* we cannot say values are equal if the address spaces are not */
  tree component_ref_of1, component_ref_of2;
  addr_space_t as1,as2;

  component_ref_of1 = NULL_TREE;
  component_ref_of2 = NULL_TREE;
#endif
  ...
  do
	...
	{
#ifdef _BUILD_C30_
          if (vro1->opcode == COMPONENT_REF) {
            component_ref_of1 = vro1->op0;
          } else
#endif
	...
	{
#ifdef _BUILD_C30_
          if (vro2->opcode == COMPONENT_REF) {
            component_ref_of2 = vro2->op0;
          } else
#endif
	...
	return false;
#ifdef _BUILD_C30_
      /* this makes all unions look like the same value; 
       *    which... uhm... is a gross simplification.
       * for structures, components of a different type will have
       * a different offset.   For unions, everything will have the same
       *    offset.
       * if there is a component ref, check we are looking at the same field
       */
      if (component_ref_of1 != component_ref_of2) return false;
#endif
