...
#ifdef _BUILD_C30_
#define THIS_SIZETYPE(for_type) (pic30_target_pointer_sizetype((for_type)))
#else
#define THIS_SIZETYPE(for_type) (sizetype)
#endif
...
add_iv_candidate_for_use (struct ivopts_data *data, struct iv_use *use)
{
  ...
  if (POINTER_TYPE_P (basetype)) {
    basetype = pic30_target_pointer_sizetype(basetype);
  }
#else
  if (POINTER_TYPE_P (basetype))
    basetype = sizetype;
...
#ifdef _BUILD_C30_
      step = fold_convert (pic30_target_pointer_sizetype(TREE_TYPE(base)), step);
#else
...
determine_common_wider_type (tree *a, tree *b)
{
  ...
  if (POINTER_TYPE_P(TREE_TYPE(*a))) {
    return atype;
  } else if (POINTER_TYPE_P(TREE_TYPE(*b))) {
    return TREE_TYPE(*b);
  }
#else
  *a = suba;
		       ...
		       struct aff_tree *aff_var, widest_int *prat = NULL)
{
  ...
  if (POINTER_TYPE_P(TREE_TYPE(var))) {
    tree_to_aff_combination (var, common_type, aff_var);
  } else {
...
#ifdef _BUILD_C30_
  if (!POINTER_TYPE_P(TREE_TYPE(ubase))) {
    /* Do not convert a pointer to uutype which will be a long unsigned int */
    if (common_type != uutype)
  ...
  if (!aff_combination_const_p (aff_inv))
      ...
      if (ok_with_ratio_p || ok_without_ratio_p)
	{
	  if (maybe_ne (aff_inv->offset, 0))
	    {
	      parts.offset = wide_int_to_tree (THIS_SIZETYPE(use->mem_type), aff_inv->offset);
			    ...
			    struct iv_use *use, struct iv_cand *cand)
{
  ...
  if (POINTER_TYPE_P (TREE_TYPE (comp_op1)))
    {
#ifdef _BUILD_C30_
      this_sizetype = TARGET_POINTER_SIZETYPE(TREE_TYPE(comp_op1));
#endif
      comp = fold_build_pointer_plus (comp_op1,
				      fold_convert (this_sizetype, comp_op2));
      comp = fold_build_pointer_plus (comp,
				      wide_int_to_tree (this_sizetype, offset));
    }
  else
    {
