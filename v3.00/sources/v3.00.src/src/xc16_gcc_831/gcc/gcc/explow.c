...
round_push (rtx size)
{
...
#ifdef _BUILD_C30_
      alignm1_rtx = force_operand (plus_constant (STACK_Pmode, align_rtx, -1),
				   NULL_RTX);
#else
  ...
  /* CEIL_DIV_EXPR needs to worry about the addition overflowing,
     ...
     TRUNC_DIV_EXPR.  */
  size = expand_binop (STACK_Pmode, add_optab, size, alignm1_rtx,
		       NULL_RTX, 1, OPTAB_LIB_WIDEN);
  size = expand_divmod (0, TRUNC_DIV_EXPR, STACK_Pmode, size, align_rtx,
			NULL_RTX, 1);
  size = expand_mult (STACK_Pmode, size, align_rtx, NULL_RTX, 1);
#else
  /* CEIL_DIV_EXPR needs to worry about the addition overflowing,
     but we know it can't.  So add ourselves and then do
     TRUNC_DIV_EXPR.  */
...
#ifdef _BUILD_C30_
      size = plus_constant (STACK_Pmode, size, extra);
#else
			      ...
			      bool cannot_accumulate)
{
  ...
  if (flag_split_stack)
	...
	ask = expand_binop (STACK_Pmode, add_optab, size,
			    gen_int_mode (required_align / BITS_PER_UNIT - 1,
					  STACK_Pmode),
			    NULL_RTX, 1, OPTAB_LIB_WIDEN);
#else
...
#ifdef _BUILD_C30_
      space = emit_library_call_value (func, target, LCT_NORMAL, STACK_Pmode,
				       ask, STACK_Pmode);
#else
	...
	return space;
#ifdef _BUILD_C30_
      final_target = gen_reg_rtx (STACK_Pmode);
#else
...
get_dynamic_stack_base (poly_int64 offset, unsigned required_align)
{
  ...
  target = expand_binop (STACK_Pmode, add_optab, target,
			 gen_int_mode (offset, STACK_Pmode),
			 NULL_RTX, 1, OPTAB_LIB_WIDEN);
#else
  target = gen_reg_rtx (Pmode);
  emit_move_insn (target, virtual_stack_vars_rtx);
...
#ifdef _BUILD_C30_
  stack_check_libfunc = gen_rtx_SYMBOL_REF (STACK_Pmode, libfunc_name);
#else
...
probe_stack_range (HOST_WIDE_INT first, rtx size)
{
      ...
      rtx addr = memory_address (STACK_Pmode,
				 gen_rtx_fmt_ee (STACK_GROW_OP, STACK_Pmode,
					         ...
					         plus_constant (STACK_Pmode,
								size, first)));
      bool success;
      create_input_operand (&ops[0], addr, STACK_Pmode);
#else
      rtx addr = memory_address (Pmode,
				 gen_rtx_fmt_ee (STACK_GROW_OP, Pmode,
					         stack_pointer_rtx,
  ...
  /* Otherwise we have to generate explicit probes.  If we have a constant
      ...
      for (i = PROBE_INTERVAL; i < isize; i += PROBE_INTERVAL)
	{
	  ...
	  addr = memory_address (STACK_Pmode,
				 plus_constant (STACK_Pmode, stack_pointer_rtx,
  ...
  /* In the variable case, do the same as above, but in a loop.  Note that we
  ...
  else
    {
      ...
      rounded_size
	= simplify_gen_binary (AND, STACK_Pmode, size,
			       gen_int_mode (-PROBE_INTERVAL, STACK_Pmode));
#else
      /* ROUNDED_SIZE = SIZE & -PROBE_INTERVAL  */
      rounded_size
	= simplify_gen_binary (AND, Pmode, size,
	 ...
	 until it is equal to ROUNDED_SIZE.  */
      ...
      temp = expand_binop (STACK_Pmode, STACK_GROW_OPTAB, test_addr,
			   gen_int_mode (PROBE_INTERVAL, STACK_Pmode), test_addr,
			   1, OPTAB_WIDEN);
#else
      /* Jump to END_LAB if TEST_ADDR == LAST_ADDR.  */
...
#ifdef _BUILD_C30_
	      addr = memory_address (STACK_Pmode,
				     plus_constant (STACK_Pmode, last_addr,
					  ...
					  rtx size)
{
...
#ifdef _BUILD_C30_
  *rounded_size = simplify_gen_binary (AND, STACK_Pmode, size,
  ...
  *residual = simplify_gen_binary (MINUS, STACK_Pmode, size, *rounded_size);
#else
  *last_addr = force_operand (gen_rtx_fmt_ee (STACK_GROW_OP, Pmode,
					      stack_pointer_rtx,
					      rounded_size_op),
  ...
  *residual = simplify_gen_binary (MINUS, Pmode, size, *rounded_size);
  ...
  if (dump_file)
    {
      if (*rounded_size == CONST0_RTX (STACK_Pmode))
	fprintf (dump_file,
	...
	fprintf (dump_file,
      ...
      if (*residual != CONST0_RTX (STACK_Pmode))
	fprintf (dump_file,
	  ...
	  && INTVAL (rounded_size) <= 4 * probe_interval)
	{
	  for (HOST_WIDE_INT i = 0;
	       ...
	       i += probe_interval)
	    {
		 ...
		 allocated.  */
#ifdef _BUILD_C30_
	      emit_stack_probe (plus_constant (STACK_Pmode, stack_pointer_rtx,
					       (probe_interval
  ...
  if (residual != CONST0_RTX (Pmode))
      ...
      if (!CONST_INT_P (residual))
	{
	  ...
	  emit_cmp_and_jump_insns (residual, CONST0_RTX (GET_MODE (residual)),
				   EQ, NULL_RTX, STACK_Pmode, 1, label);
#else
	  emit_cmp_and_jump_insns (residual, CONST0_RTX (GET_MODE (residual)),
...
#ifdef _BUILD_C30_
      rtx x = force_reg (STACK_Pmode, plus_constant (STACK_Pmode, residual,
					       -GET_MODE_SIZE (word_mode)));
#else
...
#ifdef _BUILD_C30_
      emit_stack_probe (gen_rtx_PLUS (STACK_Pmode, stack_pointer_rtx, x));
#else
...
#ifdef _BUILD_C30_
  if (size != CONST0_RTX (STACK_Pmode)
#else
...
anti_adjust_stack_and_probe (rtx size, bool adjust_back)
{
...
#ifdef _BUILD_C30_
      if (first_probe)
	anti_adjust_stack (plus_constant (STACK_Pmode, size, PROBE_INTERVAL + dope));
      else
	anti_adjust_stack (plus_constant (STACK_Pmode, size, PROBE_INTERVAL - i));
#else
      if (first_probe)
  ...
  if (adjust_back)
    adjust_stack (plus_constant (STACK_Pmode, size, PROBE_INTERVAL + dope));
  else
