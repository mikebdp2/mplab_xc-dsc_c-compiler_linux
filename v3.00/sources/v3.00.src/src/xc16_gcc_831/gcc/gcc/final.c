		   ...
		   int nopeepholes ATTRIBUTE_UNUSED, int *seen)
{
  ...
  switch (GET_CODE (insn))
	...
	if (HAVE_peephole && optimize_p && !flag_no_peephole && !nopeepholes)
	  {
#ifdef _BUILD_C30_
            bool was_call = CALL_P(insn);
#endif
	    rtx_insn *next = peephole (insn);
#ifdef _BUILD_C30_
            /* guide does not proscribe peepholing 'call' instructions
               however call_from_call_insn will fail on a peepholed
               pattern... turn it into a JUMP_INSN if it doesn't have the
               right form */
            if (was_call) {
               rtx p = PATTERN(insn);
               switch (GET_CODE(p)) {
                 case CALL: break;
                 case SET: 
                   if (GET_CODE(XEXP(p,1)) == CALL) break;
                   /* FALLS THROUGH */
                 default:
                   insn->code = JUMP_INSN; 
                   break;
