...
/*
 * Regarding the use of --offset option:
 *
 * - On classic devices, protect memory above 0x80_0000.
 *
 * - On dsPIC33A devices, protect memory below 0x80_0000.
 */
...
struct pic30_section *write_section_image(bfd *abfd, struct pic30_section *s, 
                                          PTR fp);
void write_section_pgm(bfd *abfd, asection *sect, PTR fp);
int write_hex_file(char *name, bfd *abfd);
int dump_symbols(bfd *abfd);
void print_symbol(asymbol *sym);
void write_pgm_image(bfd *abfd, asection *sect, PTR fp);
int write_pgm_file(char *name, bfd *abfd);
int write_image_file(char *name, bfd *abfd);
...
void write_section_pgm_list (bfd *, PTR, struct pic30_section *);
...
char *pic30_version = QUOTE(MCHP_VERSION);
...
static char *pgm_image_file = 0;
static long pgm_base_address = 0x7f0000;
static long pgm_address_range =  0xffff;
...
void help(char **argv);
void help(char **argv) {
  printf("\n  xc-dsc-bin2hex -- convert an object file to Intel hex format\n");
  printf  ("  Microchip version %s - GNU Binutils %s\n", pic30_version, VERSION);
  ...
  printf("    --pgm f           write a pgm file to f\n");
  printf("    --pgm-address x   determine where the base address for PGM\n"
         "                      should be; default: %p\n", pgm_base_address);
  printf("\n");
     ...
     char **argv;
{
        ...
        case '-':
          ...
          if (strcmp(argv[arg],"--offset") == 0) {
            ...
            if (arg+1 >= argc) {
              ...
              pgm_image_file = strdup(argv[arg+1]);
              arg++;
            }
          } else if (strcmp(argv[arg],"--pgm-address") == 0) {
            if (arg+1 >= argc) {
               fprintf(stderr,
                       "*** Error: Required argument for %s is missing\n", 
                       argv[arg]);
              ...
              pgm_base_address = strtoll(argv[arg+1],0,0);
              arg++;
            }
  ...
  if (pgm_image_file) {
    write_pgm_file(pgm_image_file, abfd);
  } else
...
int
write_pgm_file(char *name, bfd *abfd) {
  FILE *fp;
  int rc = 1;
  ...
  init_section_list(&outputs);
  bfd_map_over_sections(abfd, &build_section_list, outputs);
  write_section_pgm_list (abfd, fp, outputs);
  free_section_list(&outputs);
...
void
write_section_pgm_list (bfd *abfd, PTR fp, struct pic30_section *p)
{
  struct pic30_section *s;

  for (s = p; s != NULL; s = s->next) {
      if (s->sec)
        write_pgm_image(abfd, s->sec, fp);
    }
}
...
write_section(bfd *abfd, asection *sect, PTR fp) {
  ...
  unsigned address_shift = 1;
  int is_isav32 = pic30_is_isav32_machine(abfd->arch_info);
     ...
     )
  {
    ...
    if (abfd->arch_info->bits_per_address == 32)
      address_shift = 0;
    ...
    if (is_isav32) {
      /* on isav32 devices, protect memory below 0x800000 */
      if (PCstart >= 0x800000)
        start = (PCstart + offset_address_by) << address_shift;
      else
        start = PCstart << address_shift;
    }
    else {
      /* on classic devices, protect memory above 0x800000 */
      if (PCstart < 0x800000)
        start = (PCstart + offset_address_by) << address_shift;
      else
        start = PCstart << address_shift;
    ...
    if (is_isav32)
      ...
      actual = total * .75;  /* adjust for phantoms in program memory */

    /* report correct size of EEDATA sections */
    if (!is_isav32 && (PCstart >= EEDATA_BASE_ADDR))
      actual = total / 2;    /* adjust for phantoms in data memory */
    ...
    actual_prog_used += (actual);  /* actual program memory used */
...
#define DATA_LINE_WIDTH 4
  ...
  do {
    ...
    if (next) {
      ...
      last_end = last->sec->lma + (last->sec->size / 2);
      if (last_end & 2) {
      ...
      total = total + next->sec->size + pad_insn;
    } else break;
    last = next;
  } while (1);
  ...
  for (s = first; ; s = s->next) {
    ...
    if ((s->sec->flags & SEC_LOAD) && (s->sec->flags & SEC_HAS_CONTENTS)) {
      ...
      if (bfd_get_section_contents(abfd,s->sec,p,0,s->sec->size) == 0) {
        ...
        prev_end = prev->sec->lma + (prev->sec->size / 2);
        if (s != prev) {
...
void
write_pgm_image(bfd *abfd, asection *sect, PTR fp) {

  const bfd_size_type row_size=0x10;

  unsigned char *p;
  static file_ptr last_address = -1;
  file_ptr  PCstart, start, offset;
  file_ptr  upper_addr, base_addr;  /* for Extended Linear Address Records */
  bfd_size_type total;
  unsigned int   i,sum;
  ...
  if ((sect->flags & SEC_LOAD) && (sect->flags & SEC_HAS_CONTENTS))
  {
    ...
    p = malloc(total);
    bfd_get_section_contents(abfd, sect, p, 0, total);

    last_address = start;
    if ((start >= pgm_base_address) && 
        (start < (pgm_base_address + pgm_address_range))) {
      /* we are writing to 'test' memory */
      if (test_fp == NULL) test_fp = fopen("test_memory.pgm","w");
      if (test_fp != NULL) fp = test_fp;
      last_address -= pgm_base_address;
    }

    /* write the content */
    for (i = 0; i < total; i+=4) {
      fprintf(fp, "@%8.8X ",
              (last_address & 0x0FFFFF) / address_adjustment);

      last_address += 4;
      for (j = 3; j >= 0; j--) {
        fprintf(fp, "%2.2X", p[i+j]);
      }
      fprintf(fp,"\n");
    }
  }
