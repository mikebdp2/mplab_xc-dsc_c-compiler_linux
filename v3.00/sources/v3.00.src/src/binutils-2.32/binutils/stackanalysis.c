...
extern int pic30_print_insn             (bfd_vma, disassemble_info*);
...
static int block_add_label(struct sa_block *b, asymbol *label) {
  ...
  if (b->start_pc != -1) {
    bfd_vma section_end = (label->section->vma + label->section->size/opb);
...
int target_check_symbol(char **check_table, char *sym) {
  ...
  for (invalid = 0; check_table[invalid]; invalid++) {
    if (strncmp(check_table[invalid], sym, strlen(check_table[invalid])) == 0) 
      return 0;
  }
  return 1;
}
...
void sa_gather_flash_symbols(asymbol **syms, long int symcount) {
  ...
  for (i = 0; i < symcount; i++) {
    ...
    { asymbol *label;
      ...
      label = syms[i];
      section_end = (label->section->vma + label->section->size/opb);
