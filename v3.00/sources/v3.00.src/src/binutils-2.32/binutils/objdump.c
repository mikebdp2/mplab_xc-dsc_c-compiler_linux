...
/* We don't re-size sections during relocation */
#define bfd_get_section_size_before_reloc(section) \
     ((section)->size)

extern void pic30_set_extended_attributes
  (asection *, unsigned int, unsigned char );
...
static bfd_boolean no_file_offsets = FALSE; /* --no-file-offsets */
#endif
...
static enum psrd_psrd_mode save_back_to_back_psv = 0;
static int back_to_back_psrd_count = 0;
static int back_to_back_psrd_xfactor = 0;
static int instruction_stats = 0;
...
/* Previously objdump called bfd_octets_per_byte() to
 * resolve OCTETS_PER_BYTE, which must be determined
 * at run time in order to accomodate 33A devices.
 * But dwarf.c needs this info also, in functions
 * that don't have access to a bfd object.
 *
 * So we define pic30_octets_per_byte() here, based
 * on the value of global_PROCESSOR, which can be set
 * as bfd objects are examined by display_object_bfd().
 */
unsigned int pic30_octets_per_byte(void)
{
  /* if no processor defined, default to 30F type device */
  if (global_PROCESSOR && (global_PROCESSOR->bits_per_word == 32)) return 1;
  if (global_PROCESSOR && (global_PROCESSOR->bits_per_byte == 8)) return 1;
  return 2;
}
...
static void pic30_display_hex_byte
  (asection *, bfd_byte *, bfd_size_type, bfd_size_type, unsigned int);
static void pic30_display_char_byte
  (asection *, bfd_byte *, bfd_size_type, bfd_size_type, unsigned int);
static void pic30_annul_precondition
  (struct psrd_psrd_facts *, struct disassemble_info *, bfd_vma, struct pic30_private_data *, struct pic30_semantic_expr *);
static void pic30_detect_back_to_back_psv
  (bfd_vma, struct disassemble_info *, asymbol *);
#endif
...
static asymbol *find_symbol_for_address
  (bfd_vma, struct disassemble_info *, long *);
...
static void disassemble_bytes
  (struct disassemble_info *, disassembler_ftype, bfd_boolean,
	   bfd_byte *, bfd_vma, bfd_vma, bfd_vma, arelent ***, arelent **);
...
static void dump_stabs
  (bfd *);
static bfd_byte * read_section_stabs
 (bfd *abfd, const char *sect_name, bfd_size_type *size_ptr);
static void print_section_stabs
  (bfd *, const char *, unsigned *);
static void dump_section_stabs
  (bfd *, char *, char *);
static void process_stats(struct disassemble_info *info);
...
"));
  ...
  -F, --[no-]file-offsets        Include file offsets when displaying information\n\
...
enum option_values
  {
    ...
    OPTION_STOP_ADDRESS,
#if PIC30
    OPTION_PSRD_PSRD_CHECK,
    OPTION_MCHP_STACK_USAGE,
    OPTION_MCHP_STACK_SYM,
    OPTION_NO_FILE_OFFSETS,
    OPTION_PIC30_DFP,
    OPTION_INSN_STATS,
    OPTION_999,
#endif
...
static struct option long_options[]=
{
  ...
  {"insn-stats", no_argument, NULL, OPTION_INSN_STATS },
  {"no-file-offsets", no_argument, NULL, OPTION_NO_FILE_OFFSETS },
...
#if PIC30
  if (no_file_offsets)
    printf ("    2**%u", bfd_get_section_alignment (abfd, section));
  else
...
static void
pic30_annul_precondition(facts, inf, memaddr, private_data, e)
...
static void
pic30_detect_back_to_back_psv (memaddr, inf, sym)
  bfd_vma memaddr;
  struct disassemble_info *inf;
  asymbol *sym;
{
  ...
  if (!inf->private_data) return;
  private_data = inf->private_data;
		...
		bpc = 1;
...
#ifdef PIC30
              if (aux->abfd->arch_info->bits_per_byte == 16 ) {
	      ...
	      for (j = addr_offset * opb; j < addr_offset * opb + pb; j += bpc)
                ...
                if (!PIC30_DISPLAY_AS_DATA_MEMORY (section) ||
                    PIC30_CAN_PRINT_IN_DATA_MEMORY (j,opb))
#endif
		{
...
for (; j < addr_offset * opb + pb; j += bpc)
	...
	printf ("\n");
      ...
      if ((insns) && (back_to_back_psv != psrd_psrd_off)) {
          pic30_detect_back_to_back_psv(section->vma + addr_offset, inf, 0);
      }
#endif
...
disassemble_section (bfd *abfd, asection *section, void *inf)
{
  ...
  back_to_back_psv = save_back_to_back_psv;
	...
	return;
      ...
      if (!disassemble_all && (only_list == NULL) &&
...
#ifdef PIC30
              pic30_convert_reloc_addrs (abfd, section, rel_pp, rel_count);
#endif
  ...
  while (addr_offset < stop_offset)
    {
      ...
      addr = ((addr & ((sign_adjust << 1) - 1)) ^ sign_adjust) - sign_adjust;
          ...
          pinfo->symbols = sorted_syms;
          pinfo->num_symbols = sorted_symcount;
...
#ifdef PIC30
  /* Now declared at file scope, because disassemble_section()
   * has been broken out into a separate function. GM 9/16/21
   */
  save_back_to_back_psv = back_to_back_psv;
#endif
  ...
  if (PIC30_DISPLAY_AS_PROGRAM_MEMORY (section))
    {
      if (j < stop_offset * opb)
        {
          if (PIC30_CAN_PRINT_IN_PROGRAM_MEMORY (j,opb))
            printf ("%02x", (unsigned) (data[j]));
        }
      else
        {
  ...
  else if (PIC30_DISPLAY_AS_READONLY_MEMORY (section))
    {
      if (j < stop_offset * opb)
        {
        if (PIC30_CAN_PRINT_IN_READONLY_MEMORY (j,opb))
          printf ("%02x", (unsigned) (data[j]));
        }
      else
        {
  ...
  else
    {
      if (j < stop_offset * opb)
        {
        if (PIC30_CAN_PRINT_IN_DATA_MEMORY (j,opb))
          printf ("%02x", (unsigned) (data[j]));
        }
      else
        {
...
{
  if (PIC30_DISPLAY_AS_PROGRAM_MEMORY (section) &&
           !PIC30_CAN_PRINT_IN_PROGRAM_MEMORY(j,opb))
    {} /* skip phantom */
  else if (PIC30_DISPLAY_AS_READONLY_MEMORY (section) &&
           !PIC30_CAN_PRINT_IN_READONLY_MEMORY(j,opb))
    {} /* skip phantom */
...
#ifdef PIC30
          pic30_convert_reloc_addrs (abfd, section, relpp, relcount);
#endif
...
#if PIC30
  if (! dump_debugging_tags && ! suppress_bfd_header && disassemble)
#else
  if (! dump_debugging_tags && ! suppress_bfd_header)
#endif
    putchar ('\n');
...
display_object_bfd (bfd *abfd)
{
  ...
  if (bfd_check_format_matches (abfd, bfd_object, &matching))
    {
#ifdef PIC30
      /* save a copy, to allow runtime determination of OCTETS_PER_BYTE */
	...
	case 'w':
	  ...
	  seenflag = TRUE;
        ...
        case OPTION_MCHP_STACK_USAGE:
          ...
          break;
        case OPTION_NO_FILE_OFFSETS:
          no_file_offsets = TRUE;
          break;
#endif
