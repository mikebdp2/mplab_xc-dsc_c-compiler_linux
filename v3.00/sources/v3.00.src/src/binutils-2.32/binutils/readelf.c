...
char *pic30_resource_version = QUOTE(MCHP_VERSION);

Elf_Internal_Ehdr *GLOBAL_file_header = NULL; /* set by process_object() */

unsigned int pic30_octets_per_byte(void);

unsigned int pic30_octets_per_byte(void)
{
  unsigned int opb = 2; /* default for classic dsPIC */
  if (GLOBAL_file_header &&
      (GLOBAL_file_header->e_flags & EF_PIC30_NO_PHANTOM_BYTE))
    opb = 1;

  return opb;
}

#define OCTETS_PER_BYTE pic30_octets_per_byte()
#endif
...
#ifdef PIC30
  /* dsPIC classic devices have phantoms in DWARF sections;
   * we can test OCTETS_PER_BYTE to identify this. If so,
   * ignore phantoms here while loading object file data. */
  if (reason && !strncmp(reason, ".debug_", 7) &&
    OCTETS_PER_BYTE == 2)
  {
    size_t j, len = nmemb * size;
    char *p = (char *) mvar;
...
#ifdef PIC30
    /* adjust relocations for phantom bytes */
    relas[i].r_offset = relas[i].r_offset / OCTETS_PER_BYTE;
#endif
	}
...
#ifdef PIC30
    /* adjust relocations for phantom bytes */
    rels[i].r_offset = rels[i].r_offset / OCTETS_PER_BYTE;
#endif
	}
		  ...
		  bfd_boolean         is_dynsym)
{
  ...
  for (i = 0; i < rel_size; i++)
	...
	case EM_DSPIC30F:
	  rtype = elf_pic30_reloc_type (type);
	  break;
...
is_32bit_abs_reloc (Filedata * filedata, unsigned int reloc_type)
{
  ...
  switch (filedata->file_header.e_machine)
    ...
    case EM_DSPIC30F:
      return reloc_type == 3; /* R_PIC30_32.  */
    case EM_CYGNUS_FR30:
    case EM_FR30:
...
#ifdef PIC30
      section->sh_size = num_bytes / OCTETS_PER_BYTE;
#endif
...
#ifdef PIC30
  /* adjust section length for phantom byte removal */
  length = length / OCTETS_PER_BYTE;
#endif
...
#if 0
  /* debug */
  /* note: 'end' and 'length' have been adjusted for phantoms */
  /*       'external' and 'pnotes' point to adjusted section contents */
  printf("  pnotes = 0x%lx\n", pnotes);
  printf("  length = 0x%lx\n", length);
  printf("  end = 0x%lx\n", end);
  printf("  external = 0x%lx\n", external);
#endif
...
#ifdef PIC30
    /* adjust field sizes to reflect phantom byte removal */
    inote.namesz = inote.namesz / OCTETS_PER_BYTE;
    inote.descsz = inote.descsz / OCTETS_PER_BYTE;
#endif
...
#if 0
      /* debug */
      printf("  inote.descdata = 0x%lx\n", inote.descdata);
      printf("  inote.namedata = 0x%lx\n", inote.namedata);
      printf("  inote.namesz = 0x%lx\n", inote.namesz);
      printf("  data_remaining = 0x%lx\n", data_remaining);
      printf("  inote.descsz = 0x%lx\n", inote.descsz);
      printf("  next = 0x%lx\n", next);
#endif
...
#ifdef PIC30
  /* On PIC30, SHT_NOTE is used to denote debugging info.
   * Therefore we have nothing to process here.
   */
  return res;
#endif
...
process_object (Filedata * filedata)
{
  ...
  GLOBAL_file_header = &filedata->file_header;
#endif
