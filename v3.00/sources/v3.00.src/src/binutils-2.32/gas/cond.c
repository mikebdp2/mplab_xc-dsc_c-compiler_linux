...
#ifdef PIC30
/* Performs the .ifdecl (test_declared == 1) and
   the .ifndecl (test_declared == 0) pseudo op.  */

void
s_ifdecl (test_declared)
     int test_declared;
{
  ...
  *input_line_pointer = c;
  ...
  if (cframe.dead_tree)
  ...
  else
    {
      ...
      /* A declared symbol has been referenced and may
         also have been defined. */
      is_declared =
	symbolP != NULL;

      cframe.ignoring = ! (test_declared ^ is_declared);
    }
