...
extern const char *pic30_target_format (void);
 ...
 */
extern unsigned long pic30_target_mach (void);
 ...
 */
extern int pic30_parse_name (const char *, struct expressionS *);
#define md_parse_name(name, exp, mode, c) pic30_parse_name (name, exp)

/*
 * If this macro is defined, GAS will call it when the align directive is seen
 * in the input file.
 */
extern int pic30_do_align (int, const char *, int, int);
 ...
 */
extern void pic30_handle_align (fragS *);
 ...
 *
 * Note: The prototype for this function changed in binutils-v2.32
 */
extern bfd_reloc_code_real_type pic30_expr (struct expressionS *, unsigned char);
#define TC_PARSE_CONS_EXPRESSION(EXP, NBYTES) pic30_expr(EXP, TRUE)
 ...
 * pseudo-op.
 */
extern void pic30_cons_fix_new (fragS *, int, int,
                                        struct expressionS *);
...
/*
 * This macro satisfies a test in write_relocs()
 * when the (external) memory section attribute is used.
 */
#define TC_FX_SIZE_SLACK(FIX) 2

/* dsPIC instructions are 2 'address units' long.  */
 ...
 *
 * Note: This block moved to bfd.h *and* bfd-in2.h,
 * by way of bfd/section.c, which propagates as a result
 * of the next configure, followed by 'make headers'
 * in the binary bfd directory.
 *
 * You can't make this stuff up <sigh> - GM 8/8/21
 */
...
extern unsigned char global_pic30_dot_expression;
 ...
 * by way of bfd/section.c, which propagates as a result
 * of the next configure, followed by 'make headers'
 * in the binary bfd directory. - GM 8/8/21
 */
 ...
 * relax table.
 */
extern void pic30_prepare_relax_scan (fragS *, unsigned long,
                                              long *, relax_substateT,
                                              PTR);
#define md_prepare_relax_scan(fragP, address, aim, this_state, this_type) \
   pic30_prepare_relax_scan (fragP, address, &aim, this_state, (relax_typeS *) this_type)
 ...
 */
extern int pic30_fix_adjustable (struct fix *);
 ...
 * a label.
 */
extern void pic30_frob_label_before_define (const char *);
 ...
 */
extern int pic30_frob_symbol (symbolS * );
 ...
 * to the object file
 */
extern void pic30_frob_file_after_relocs (void);
 ...
 * simply "GAS LISTING"
 */
#define LISTING_HEADER "MPLAB XC-DSC ASSEMBLY Listing: "
 ...
 */
#define MD_DEBUG_FORMAT_SELECTOR pic30_debug_format_selector
extern enum debug_info_type pic30_debug_format_selector (int * use_gnu_extensions);

/******************************************************************************
 * New macros used by gas/expr.h
 ******************************************************************************/

#ifndef TC_EXPRESSION_TYPE
/*
...
extern unsigned char pic30_do_org (register segT, PTR,
                                           long, unsigned char);
 ...
 * output_string.
 */
extern unsigned char pic30_cons_emit (char *, unsigned int);
#define TC_CONS_EMIT(out_string, length) pic30_cons_emit(out_string, length)

extern unsigned char pic30_cons_emit_expr (struct expressionS *,
                                                   unsigned int);
 ...
 * in the listing file.
 */
extern void pic30_list_symbol_info (FILE *);
 ...
 * in the listing file.
 */
extern void pic30_list_section_info (FILE *);
...
#define PIC30_PROGRAM_MEMORY_UPPER_BYTE_LOCATION \
          (pic30_has_phantom_byte(0) ? 2 : -1)
#define PIC30_PHANTOM_BYTE_LOCATION (pic30_has_phantom_byte(0) ? 3 : -1)
...
#define PIC30_INSERT_CHAR_INTO_FIX(fix, value, output, location)        \
   while (TRUE)                                                          \
   {                                                                     \
...
#define PIC30_INSERT_VALUE_INTO_DATA_MEMORY(value)                       \
   {                                                                     \
     int opb = OCTETS_PER_BYTE;                                          \
     char * frag_output = frag_more (opb);                               \
     *frag_output++ = (value);                                           \
     if (opb != 1) *frag_output++ = (PIC30_PHANTOM_BYTE);                \
   }
...
#define PIC30_INSERT_DATA_INTO_DATA_MEMORY(ptr,len)                      \
   {                                                                     \
     ...
     {                                                                   \
       *frag_output++ = *p++;                                            \
        if (OCTETS_PER_BYTE != 1) *frag_output++ = (PIC30_PHANTOM_BYTE); \
     }                                                                   \
   }
...
#define PIC30_CAN_WRITE_DATA(byte)                                       \
   ((((byte) % PIC30_SIZE_OF_DATA_WORD) == 0) || (OCTETS_PER_BYTE == 1))
...
#define tc_frob_label(sym) pic30_define_label (sym)
extern void pic30_define_label (symbolS *);
