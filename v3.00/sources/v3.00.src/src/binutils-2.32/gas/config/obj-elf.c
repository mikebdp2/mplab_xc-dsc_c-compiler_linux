...
extern struct elf_obj_sy * symbol_get_obj (symbolS *s);
...
#ifdef TC_PIC30
  /* Ensure common symbols are correctly marked */
  symbol_get_bfdsym (symbolP)->flags |= BSF_ELF_COMMON;
  symbol_get_bfdsym (symbolP)->section = bfd_com_section_ptr;
#endif
			...
			int push)
{
   ...
   * that can be set with single character
   * flags. The other attributes are
   * processed in gas/config/tc-pic30.c.
   *
   */
  if (attr & SHF_NOLOAD)
...
#if defined(TC_PIC30)
	/*
	** Compatibility with COFF assembler.
	*/
	case 'b':
	  /*
	  ** bss
	...
	case 'r':
	  /*
	  ** read-only
	  ** This is used to signify PSV access
	  */
	  attr |= SHF_ALLOC;
	  attr |= SHF_PSV;
	  break;
...
#if defined(TC_PIC30)
	  attr |= obj_elf_parse_section_letters (beg, strlen (beg), &type, &is_clone);
#else
...
#if defined(TC_PIC30)
  if ((type == SHT_NOBITS) && (attr & SHF_EXECINSTR))
  {
...
#if defined(TC_PIC30)
      flags = SEC_HAS_CONTENTS | SEC_IN_MEMORY | SEC_GROUP;
#else
