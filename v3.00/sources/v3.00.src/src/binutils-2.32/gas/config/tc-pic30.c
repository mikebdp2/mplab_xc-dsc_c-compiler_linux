 ...
 ******************************************************************************/
#if defined(OBJ_ELF)
static void pic30_emit_leb128_expr (expressionS *exp, int sign);
static int pic30_output_sleb128 (char *p, offsetT value);
static int pic30_output_uleb128 (char *p, valueT value);
extern int pic30_output_leb128 (char *p, valueT value, int sign);
#endif

extern void md_number_to_chars (char *, valueT, int);
extern void number_to_chars_with_phantom_bytes (char *, bfd_signed_vma, int);
extern unsigned int pic30_attribute_map (asection *);
extern unsigned int pic30_extended_attribute_map (asection *);
extern int pic30_set_implied_attributes (asection *, unsigned char );
...
extern int pic30_is_valid_attributes (unsigned int, unsigned char);
...
extern void silent_ignore_rest_of_line(void);

static int pic30_align_power (offsetT);
static int pic30_address_align_power (bfd_vma);
static  bfd_vma pic30_address (bfd_vma);
static  bfd_vma pic30_element_size (bfd_vma);
int subseg_readonly (segT);
int subseg_psv (segT);
int subseg_auxpsv (segT);
int subseg_eedata (segT);
int subseg_dma (segT);
int subseg_code (segT);
int subseg_auxflash (segT);
int subseg_memory (segT);
int subseg_heap (segT);
int subseg_stack (segT);
static unsigned int pic30_next_char_of_string (void);
char * demand_copy_string (int *);
unsigned char pic30_do_org (register segT, PTR,
                                           long, unsigned char);
void pic30_update_section_info (void);
void pic30_set_global_info (void);
static int pic30_update_processor (const char *);
void pic30_display_section_info (FILE *, asection *, unsigned char);
void pic30_display_code_section_info (bfd *, asection *, PTR);
void pic30_display_data_section_info (bfd *, asection *, PTR);
void print_expression (const expressionS *);
long pic30_apply_operator (bfd_reloc_code_real_type,
                                   long, unsigned char);
...
void pic30_create_call_insn (struct pic30_opcode *, expressionS []);
void pic30_create_goto_insn (struct pic30_opcode *, expressionS []);
void pic30_create_do_insn (struct pic30_opcode *, expressionS []);
void pic30_create_2word_insn (struct pic30_opcode *, expressionS []);
void pic30_create_insn (struct pic30_opcode *, expressionS [], short);
void pic30_check_isa (struct pic30_opcode *, expressionS []);
bfd_boolean insn_is_REPEAT_hazard(struct pic30_opcode *, expressionS [], short );
int md_chars_to_number (unsigned char *, int);
static void pic30_record_do (fixS *);
void md_apply_fix(fixS *, valueT *, segT);
void pic30_relax_call (fragS *);
void pic30_prepare_relax_scan (fragS *, unsigned long,
                                       long *, relax_substateT,
                                       PTR);
void pic30_create_bf_insn (struct pic30_opcode *, expressionS []);
...
unsigned char global_pic30_dot_expression = FALSE;
symbolS *global_last_sym = NULL;
unsigned long global_last_sym_loc = 0;
...
#define PIC30_DO_CODE_ALIGNMENT()                          \
   do {                                                    \
      ...
      if (pic30_is_isav32_machine(global_PROCESSOR))       \
        align = PIC30_ISAV32_CODE_ALIGNMENT;               \
      else                                                 \
        align = PIC30_CODE_ALIGNMENT;                      \
      if (pic30_do_align (align, 0, 0, 0))                 \
         record_alignment (now_seg, align);                \
                                                           \
      global_must_align_location_counter = FALSE;          \
   } while (0);

#define PIC30_DO_DATA_ALIGNMENT()                          \
   do {                                                    \
      int align = PIC30_DATA_ALIGNMENT;                    \
      if (pic30_do_align (align, 0, 0, 0))                 \
         record_alignment (now_seg, align);                \
                                                           \
      global_must_align_location_counter = FALSE;          \
...
#define PIC30_ISAV32_CODE_ALIGNMENT 2
...
const char *attributes_ignored_for_isa32_devices[] =
{
  "auxpsv",
  "boot",
  "secure",
...
/*
 * binutils-2.32 calls this function when --gen-debug is specified
 */
enum debug_info_type pic30_debug_format_selector (int * use_gnu_extensions) {
  return DEBUG_DWARF2;
}

unsigned int pic30_octets_per_byte_power(void) {
  /* if no processor defined, default to 30F type device */
  if (global_PROCESSOR && (global_PROCESSOR->bits_per_word == 32)) return 0;
  if (global_PROCESSOR && (global_PROCESSOR->bits_per_byte == 8)) return 0;
  return 1;
}
...
{ char *f,safe;
   ...
   sprintf(base,"%s/support/",pic30_dfp);
    ...
    if (base == 0) {
      ...
      do {
        ...
        sprintf(buf, "%s%cxc-dsc-as%s", p, DIR_SEPARATOR_2, EXE_SUFFIX);
  ...
  if (base) {
    ...
    /* can't test for P33A, 'cause we ran out of bits in c30_flag_definitions.h */
    if (pic30_is_isav32_machine(global_PROCESSOR)) {
      include_path = strdup(MPLABC30_PIC33A_INC_PATH);
    }
    if (include_path) {
      char *c;
...
/* pad or not 2 pad */
void pic30_pad_insn(opcode) 
  struct pic30_opcode *opcode;
{
  char *output_frag;

  if ((opcode->bytes_per_IW == 4) && ((global_current_location & 3) != 0)) {
    if (flag_debug) {
      printf ("    pic30_create_insn::pad for 4 byte insn\n");
    }
    /* need to pad out with neop */
    output_frag = frag_more (2);
    /* check to see if we have just made a symbol */
    if ((global_last_sym) && (S_GET_SEGMENT(global_last_sym) == now_seg) &&
        (global_last_sym_loc == global_current_location))  {
       /* attempt to move it */ 
       symbol_set_frag(global_last_sym, frag_now);
       S_SET_VALUE(global_last_sym, (valueT)frag_now_fix());
    }
    global_current_location += 2;
    md_number_to_chars (output_frag, 0x0, 2);
    listing_prev_line();
    if (flag_debug) {
      printf ("    pic30_create_insn::global_current_location = %ld\n",
...
/******************************************************************************
 *
 *   Provide an externally visible number-to-chars function
 *
 *   Note: This was previously a macro defined in tc-pic30.h
 *
 ******************************************************************************/
void
...
static void report_target_attribute_err (const char *attr)
{
  as_bad (_("processor %s does not support %s memory"),
          global_PROCESSOR->printable_name, attr );
}

static void report_target_attribute_ignored (const char *attr)
{
  as_warn ("attribute \'%s\' ignored for processor %s\n",
          attr, global_PROCESSOR->printable_name );
}

static int ignore_attribute_for_isa32(char *attr)
{
  int i = 0;
  char *p;

  while (p = attributes_ignored_for_isa32_devices[i++]) {
    if (strstr(attr,p))
      return TRUE;
  }
  return FALSE;
...
record_access_entry(const char *name, int seg, int slot)
{
  ...
  if (!symbol_find (s3)) {
    /*
     * XC16-1564 The .section directive may appear
     * more than once, but we only need one label.
     */
...
static const char * valid_argument_names[] = {
  "unused",
  "isr",
  0, };
...
#define NEAR_DATA_LIMIT   (pic30_is_isav32_machine(global_PROCESSOR) ? \
                           0xFFFF : 0x1FFF)
  ...
  while (!is_it_end_of_statement ())
    {
      ...
      name = strncpy(s, name, sizeof(s)-1);
      len = strlen(name);
      for(i = 0; i < len; i++)
        s[i] = tolower(s[i]);
      ...
      *input_line_pointer = c;
      ...
      if (pic30_is_isav32_machine(global_PROCESSOR) &&
          ignore_attribute_for_isa32(name))
      {
        report_target_attribute_ignored(name);
        SKIP_COMMA ();
        continue;
      }
...
#include "pic30-attributes.h"
  ...
  */
  ...
  if (PIC30_IS_AUXFLASH_ATTR(sec) && !pic30_is_ecore_machine(global_PROCESSOR))
    report_target_attribute_err ("auxflash");

  if (PIC30_IS_AUXPSV_ATTR(sec) && !pic30_is_ecore_machine(global_PROCESSOR))
    report_target_attribute_err ("auxpsv");
     ...
     int sign;
{
  ...
  nbytes = -1;
  if (check_eh_frame (exp, (unsigned int *) &nbytes))
    abort ();
  ...
  */
  ...
  if (op == O_constant)
  {
      ...
      */
      ...
      if (pic30_has_phantom_byte(0)) size = size * 2;
      p = frag_more (size);
      pic30_output_leb128 (p, value, sign);
  }
  else
   ...
   int append_zero;
{
   ...
   stringer (8 + append_zero); /* arg = number_of_bits + boolean */
   ...
   do
   {
      ...
      *p = '\0';
      ...
      if ((d >= -1.0) && (d < 1.0))
      {
         ...
         {
#endif
            pp = frag_more ((int) PIC30_FIXED_SIZE);
            memcpy(pp, buf, PIC30_FIXED_SIZE);
#ifdef TC_CONS_EMIT
         } /* if (!TC_CONS_EMIT(buf, PIC30_FIXED_SIZE)) */
#endif
...

{
      ...
      if (where->X_op != O_constant)
        ...
        if (now_seg == absolute_section)
        ...
        else
          {
            int i, j;
            char *p;
            unsigned char fill_pattern[PIC30_MAX_SIZE_OF_PROGRAM_WORD];
            ...
            if (!pic30_has_phantom_byte(global_PROCESSOR)) {
              /* no such thing as fill upper and phantom byte */
              fill_pattern[2] = fill;
              fill_pattern[3] = fill;
            } else {
            ...
            if (flag_debug)
              {
                ...
                for (n = 0; n < PIC30_SIZE_OF_PROGRAM_WORD; n++)
                  printf (" 0x%02x", fill_pattern[n]);
   ...
   if (flag_debug) {
      printf ("    pic30_do_org::return %s\n", rc ?
        "TRUE (.org was handled)" : "FALSE (.org was not handled)");
...

{
   ...
   int cons_data_number_of_bytes =  cons_data.number_of_bytes;   /* not const */

   if ((global_PROCESSOR) && (global_PROCESSOR->bits_per_byte != 16)) {
      if (directive == PIC30_CONS_PWORD) {
         cons_data_number_of_bytes = 4;
      }
   }
   if (flag_debug)
   {
   ...
   if (cons_data.p_directive)
   {
      ...
      {
        PIC30_DO_CODE_ALIGNMENT ();  /* always do this for .pword directives */
        listing_prev_line();     /* attach padding (if any) to previous line */
      }
   ...
   /* DEBUG */
#if 1
     cons (cons_data_number_of_bytes);
#else
...

{
   ...
   if (flag_debug)
   {
      ...
      else
      {
         ...
         printf ("    pic30_update_section_info::old.current_location = %lu\n"
                 ...
                 "    pic30_update_section_info::old.last_sym = 0x%x\n",
                 ...
                 info->tc_segment_info_data.last_sym);
      } /* else */
   } /* if (flag_debug) */
   ...
   else
   {
      ...
      info->tc_segment_info_data.last_sym = global_last_sym;
...

{
   ...
   if (info)
   {
      ...
      global_last_sym = info->tc_segment_info_data.last_sym;
   } /* if (section_info) */
   else
   {
   ...
   int unused ATTRIBUTE_UNUSED;
{
  ...
  while (!is_it_end_of_statement ()) {
    ...
    attr = input_line_pointer;
    c = get_symbol_end ();

    /* make a local copy, lower case */
    attr = strncpy(s, attr, sizeof(s)-1);
    len = strlen(attr);
    for(i = 0; i < len; i++)
      s[i] = tolower(s[i]);
    ...
    *input_line_pointer = c;
    ...
    if (strcmp(attr, "size") == 0)
      ...
      as_bad (_("Unrecognized memory attribute \'%s\'"), attr);
      demand_empty_rest_of_line ();
      break;
    }
  }
...
#if !defined(OBJ_ELF)
   {
    ...
    if (PIC30_IS_INFO_ATTR(sec))
     ...
     if (sec->priority) {
       ...
       sym_name = xmalloc (strlen(sec->name) + strlen(PRIORITY_ATTR_PREFIX) + 1);
       (void) sprintf(sym_name, "%s%s", PRIORITY_ATTR_PREFIX, sec->name);
       if (!symbol_find(sym_name)) {
         symbolp = symbol_new (sym_name, absolute_section,
                              (valueT) sec->priority,
                              &zero_address_frag);
...

{
   ...
   if (flag_debug)
   {
      ...
      for (i = 0; i < length; i++)
         printf ("0x%02x%c", (unsigned char) out_string[i], (i == (length - 1)) ? '\n' : ' ');
   } /* if (flag_debug) */
...

{
   ...
   if ((subseg_text_p (now_seg)) || (now_seg == text_section))
   {
      ...
      TC_CONS_FIX_NEW (frag_now, p - frag_now->fr_literal,
                       (int) number_of_bytes, exp, BFD_RELOC_NONE);
   ...
   switch (option)
   {
	 ...
	 */
      ...
      case 'p':
         ...
         if (has_proc_option)
         {
            fprintf (stderr, "%s", multi_proc_message);
            rc = 0;
         }
         else 
#endif
      ...
      case PIC30_ISAV4:
         if (strcmp(argument, "v4") == 0)
           pic30_isa_v4 = TRUE;
         /* FIXME: Other possible values of option 'isa' are not handled here */
         break;
...
md_begin (void)
{
   ...
   int is_isaBB = pic30_is_isaBB_machine(global_PROCESSOR);
   int is_isav32 = pic30_is_isav32_machine(global_PROCESSOR);
   int is_fcore;
   int f_any_core;

   is_fcore = (!is_ecore) && (!is_isav4) && (!is_isav32);

   /* any core has none of these flags set */
   f_any_core = (F_ECORE | F_FCORE | F_ISAV4 | F_DUALPARTITION | F_CONTEXTS |
                 F_ISA32V016 | F_ISA32V0);
   if (flag_debug)
   ...
   if (is_isav32 == 0) {
     pic30_num_registers = 16; /* integer 'Wn' registers only */
   }
   ...
   { int add_me = 0;
      ...
      if ((is_dualpartition) && (opcode->flags & F_DUALPARTITION)) {
        /* must test these cases separately */
        add_me  = ((opcode->flags & F_ISAV4) && (is_isav4));
        add_me |= ((opcode->flags & F_ISA32V0) && (is_isav32));
      } else {
      add_me = ((opcode->flags & F_ECORE) && (is_ecore));
      add_me |= ((opcode->flags & F_FCORE) && (is_fcore));
      /* isav32 cannot use any of the instructions not marked for isa32 */
      add_me |= (((opcode->flags & f_any_core) == 0) && (!is_isav32));
      add_me |= ((opcode->flags & F_ISA32V0) && (is_isav32));
      add_me |= ((opcode->flags & F_ISA32V016) && (is_isav32) && (isa32_16_ok));
      add_me |= ((opcode->flags & F_ISAV4) && (is_isav4));
      add_me |= ((opcode->flags & F_CONTEXTS) && (is_contexts));
      }

      
      if ((opcode->flags & F_NOTISAB) && (is_isaBB)) {
        add_me = 0;
      }

      if (add_me == 0) {
         if (flag_debug) {
            printf ("      NOT adding %s (add_me == 0)\n",opcode->name);
         }
      } else if (strcmp (opcode->name, previous_name) != 0) {
         if (flag_debug) 
           printf ("      adding %s\n",opcode->name);
         previous_name = (char *)opcode->name;
         hash_insert (pic30_opcodes_hash, opcode->name, (void *) opcode);
      } else if (flag_debug) {
         printf ("      NOT adding %s (matches previous)\n",opcode->name);
      }
   } /* for opcode */
        ...
        symbol_count++)
   {
       ...
       *
       * insert loweracase 'w' and uppercase 'W' registers
       *
       */
      char symbol[8];

      snprintf(symbol, sizeof(symbol), "%s", pic30_registers[symbol_count]);
      symbol_table_insert (symbol_create (symbol, reg_section, 
                                          symbol_count, &zero_address_frag));
      if (strcmp(symbol,"sr") == 0) {
        if (pic30_usr_register == 0) pic30_usr_register = symbol_count;
      } else if (strcmp(symbol,"fsr") == 0) {
        if (pic30_fsr_register == -1) pic30_fsr_register = symbol_count;
      } else if (strcmp(symbol,"fcr") == 0) {
        if (pic30_fcr_register == -1) pic30_fcr_register = symbol_count;
      } else if (strcmp(symbol,"fsrh") == 0) {
        if (pic30_fsrh_register == -1) pic30_fsrh_register = symbol_count;
      } else if (strcmp(symbol,"fear") == 0) {
        if (pic30_fear_register == -1) pic30_fear_register = symbol_count;
      } else if (symbol[0] == 'w') {
        symbol[0] = 'W';
      } else if ((symbol[0] == 'f') && (symbol[1] == '0')) {
        if (pic30_first_float_register == -1) {
          pic30_first_float_register = symbol_count;
        }
...
#define MAX_PROC_FAMILY_LEN 16
      ...
      if (strcmp(global_PROCESSOR->printable_name, "GENERIC-16BIT")==0)
      {
        processor=(char *)"__GENERIC_16BIT";
          ...
          strcmp(global_PROCESSOR->printable_name, "GENERIC-32DSP-AK")==0) {
          processor=(char *)"__GENERIC_16DSP_AA";
      } else if (
          strcmp(global_PROCESSOR->printable_name, "GENERIC-16DSP-AA")==0) {
          processor=(char *)"__GENERIC_16DSP_AA";
      }
      if (processor) {
        symbolp = symbol_new(processor, absolute_section,
      ...
      } else if (pic30_is_isav32_machine(global_PROCESSOR)) {
        create_absolute_symbol("__TARGET_DIVIDE_CYCLES16", 5);
        create_absolute_symbol("__TARGET_DIVIDE_CYCLES32", 9);
        create_absolute_symbol("__HAS_AA",1);
        create_absolute_symbol("__HAS_ISA32",1);
      } else {
        create_absolute_symbol("__TARGET_DIVIDE_CYCLES", 17);
      }

     if (pic30_is_isaBB_machine(global_PROCESSOR)) {
       create_absolute_symbol("__HAS_BB",1);
     }
   ...
   else
   {
     if (((sec->flags & SEC_LOAD) && (sec->flags & SEC_DATA)) /* DATA */ || 
         ((sec->flags & (SEC_ALLOC | SEC_LOAD)) == SEC_ALLOC) /* BSS */ || 
          (sec == data_section))
      {
         bfd_size_type total = sec->size / opb;
 ...
 *
 *   Note: BFD_VMA_FMT is defined as "l" or "ll" in BFD header files
 *
 ******************************************************************************/

{
   ...
   switch (expressionP->X_md.modifier)
   {
      ...
      case PIC30_PACKED_HI_FOUND:
         fprintf (stdout, "packed_hi (");
         break;
   ...
   {
      case O_register:
         fprintf (stdout, "O_register::%" BFD_VMA_FMT "d", expressionP->X_add_number);
         break;
      ...
      case O_constant:
         fprintf (stdout, "O_constant::%" BFD_VMA_FMT "d", expressionP->X_add_number);
         break;
      ...
      case O_symbol:
         ...
         fprintf (stdout, "+%" BFD_VMA_FMT "d", expressionP->X_add_number);
         break;
      ...
      case O_REGISTER_INDIRECT:
         fprintf (stdout, "O_REGISTER_INDIRECT::%" BFD_VMA_FMT "d",
                  expressionP->X_add_number);
         break;

      case O_REGISTER_PRE_INCREMENT:
         fprintf (stdout, "O_REGISTER_PRE_INCREMENT::%" BFD_VMA_FMT "d",
                  expressionP->X_add_number);
         break;

      case O_REGISTER_PRE_DECREMENT:
         fprintf (stdout, "O_REGISTER_PRE_DECREMENT::%" BFD_VMA_FMT "d",
                  expressionP->X_add_number);
         break;
      ...
      case O_REGISTER_WITH_DISPLACEMENT:
      {
         ...
         fprintf (stdout, "O_REGISTER_WITH_DISPLACEMENT::w%" BFD_VMA_FMT "d + w%" BFD_VMA_FMT "d",
      ...
      case O_REGISTER_PLUS_LITERAL:
      {
         ...
         fprintf (stdout, "O_REGISTER_PLUS_LITERAL::w%" BFD_VMA_FMT "d + %" BFD_VMA_FMT "d",
                  left->X_add_number, right->X_add_number);
         break;
      }
      ...
      case O_REGISTER_MINUS_LITERAL:
      {
         ...
         fprintf (stdout, "O_REGISTER_MINUS_LITERAL::w%" BFD_VMA_FMT "d - %" BFD_VMA_FMT "d",
                  left->X_add_number, right->X_add_number);
         break;
      }
      ...
      {
         fprintf (stdout, "O_REGISTER_POST_INCREMENT::%" BFD_VMA_FMT "d",
                  expressionP->X_add_number);
         break;
      }
      ...
      {
         fprintf (stdout, "O_REGISTER_POST_DECREMENT::%" BFD_VMA_FMT "d",
                  expressionP->X_add_number);
         break;
      }
      ...
      case O_REGISTER_POST_INCREMENT_BY_N:
      {
         ...
         fprintf (stdout, "O_REGISTER_POST_INCREMENT_BY_N::[w%" BFD_VMA_FMT "d]+=%" BFD_VMA_FMT "d",
                  left->X_add_number, right->X_add_number);
      ...
      case O_REGISTER_POST_DECREMENT_BY_N:
      {
         ...
         fprintf (stdout, "O_REGISTER_POST_DECREMENT_BY_N::[w%" BFD_VMA_FMT "d]+=%" BFD_VMA_FMT "d",
                  left->X_add_number, right->X_add_number);
      ...
      case O_DSP_SQUARE:
      {
         fprintf (stdout, "O_DSP_SQUARE::w%" BFD_VMA_FMT "d*w%" BFD_VMA_FMT "d",
                  expressionP->X_add_number, expressionP->X_add_number);
         break;
      }

      case O_REGISTER_PRE_INCREMENT_BY_N:
      {
         expressionS * left =
            symbol_get_value_expression (expressionP->X_add_symbol);
         expressionS * right =
         ...
         fprintf (stdout, "O_REGISTER_PRE_INCREMENT_BY_N::[w%" BFD_VMA_FMT "d+=%" BFD_VMA_FMT "d]",
                  left->X_add_number, right->X_add_number);
      ...
      case O_REGISTER_PRE_DECREMENT_BY_N:
      {
         expressionS * left =
            symbol_get_value_expression (expressionP->X_add_symbol);
         expressionS * right =
         ...
         fprintf (stdout, "O_REGISTER_PRE_DECREMENT_BY_N::[w%" BFD_VMA_FMT "d+=%" BFD_VMA_FMT "d]",
                  left->X_add_number, right->X_add_number);
...

{
...
#if 0
   /* WREG is listed as a reserved word, it does not need to be contextual */
   ...
   /* check floating piont first */
   if ((!rc) && ((global_current_opcode_flags & F_HAS_F_BRANCH_FLAG) ==
         F_HAS_F_BRANCH_FLAG) &&
        (global_current_operand_position == PIC30_BRANCH_FLAG_OPERAND_POSITION))
   {
      ...
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_EQ;
         expr->X_op = O_BRANCH_ON_CONDITION;
      }
      else if (strcasecmp (name, "GE") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_GE;
         expr->X_op = O_BRANCH_ON_CONDITION;
      }
      else if (strcasecmp (name, "NE") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_NE;
         expr->X_op = O_BRANCH_ON_CONDITION;
      }
      else if (strcasecmp (name, "GT") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_GT;
         expr->X_op = O_BRANCH_ON_CONDITION;
      }
      else if (strcasecmp (name, "LE") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_LE;
         expr->X_op = O_BRANCH_ON_CONDITION;
      }
      else if (strcasecmp (name, "LT") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_LT;
         expr->X_op = O_BRANCH_ON_CONDITION;
      }
      else if (strcasecmp (name, "UN") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_UN;
         expr->X_op = O_BRANCH_ON_CONDITION;
      } 
      else if (strcasecmp (name, "UNE") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_UNE;
         expr->X_op = O_BRANCH_ON_CONDITION;
      } 
      else if (strcasecmp (name, "UEQ") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_UEQ;
         expr->X_op = O_BRANCH_ON_CONDITION;
      } 
      else if (strcasecmp (name, "ULE") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_ULE;
         expr->X_op = O_BRANCH_ON_CONDITION;
      } 
      else if (strcasecmp (name, "ULT") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_ULT;
         expr->X_op = O_BRANCH_ON_CONDITION;
      } 
      else if (strcasecmp (name, "UGE") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_UGE;
         expr->X_op = O_BRANCH_ON_CONDITION;
      } 
      else if (strcasecmp (name, "UGT") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_UGT;
         expr->X_op = O_BRANCH_ON_CONDITION;
      } 
      else if (strcasecmp (name, "OR") == 0)
      {
         expr->X_add_number = PIC30_F_BRANCH_ON_OR;
         expr->X_op = O_BRANCH_ON_CONDITION;
      } 
      else
         rc = 0;
   ...
   if (!rc)
   /* Recognize 'a' and 'b' as DSP registers; these are reserved
       names and can not be symbols in an expression. */
   {
      rc = 1;
...
{
extern relax_addressT relax_align (relax_addressT addr, int align);
   int rc = 0;
   int PCx = /* adjustment factor for PC calculations */
             (pic30_is_isav32_machine(global_PROCESSOR) ? 0 : 1);
   ...
   if ((subseg_text_p (now_seg)) || (now_seg == text_section))
   {
      ...
      unsigned long overflow_of_alignment =
         (global_current_location + (1 << (n+PCx))) % PIC30_SIZE_OF_PROGRAM_WORD;
      unsigned long number_of_bytes_to_skip =
         relax_align (global_current_location, n+PCx);
      ...
      if (!rc)
      {
         ...
         else
         {
            ...
            if (flag_debug)
            {
              printf ("    pic30_do_align::number_of_bytes_to_skip = %ld\n",
                      number_of_bytes_to_skip);
              printf ("    pic30_do_align::overflow_of_alignment = %ld\n",
                      overflow_of_alignment);
            }
            /*
             * The current fill pattern is based on what will be placed into
             * each byte of the program word.  The pattern should be:
            ...
            if (!pic30_has_phantom_byte(global_PROCESSOR)) {
              /* no such thing as fill upper and phantom byte */
              fill_pattern[2] = fill_value;
              fill_pattern[3] = fill_value;
            } else {
            fill_pattern[2] = fill_upper;
            fill_pattern[3] = PIC30_PHANTOM_BYTE;
            }
            ...
            {
               unsigned char * start = fill_pattern + overflow_of_alignment;
               int size_of_pattern =
                ...
                */
               frag_align_pattern (PIC30_CODE_ALIGNMENT+1, (const char *) start,
                                   size_of_pattern, max);
            ...
            if (number_of_bytes_to_skip)
            {
               ...
               */
             frag_align_pattern (n+PCx, (const char *) fill_pattern, sizeof (fill_pattern), max);
            } /* if number_of_bytes_to_skip */
         } /* else */
   ...
   else if (strncmp(now_seg->name, ".debug", 6) == 0)
   {
   ...
   else
     {
       ...
       */
       /*
        * commentary about 'octets' and 'bytes' asside (an octet is a fixed
        *   size and a byte is the smallest addressable unit), the 
        *   dsPIC family can access a byte in RAM, but this must be set
        *   this way to match the samllest addressable unit in Flash?
        *
        *   The A core has byte addressable FLASH, so this does not apply -
        *   in other words bfd_octets_per_byte = 1, not 2.
        *
        * (CAW)
        */
       int scale = 2;
       if (!pic30_has_phantom_byte(global_PROCESSOR)) scale = 1;
       if (n != 0 && !need_pass_2)
         {
           if (fill == NULL)
             frag_align (n+scale-1, 0, max*scale);
           else if (len <= 1)
             frag_align (n+scale-1, *fill, max*scale);
           else
             frag_align_pattern (n+scale-1, fill, len, max*scale);
           /*
            * Return that this function did or ignored the alignment.
            */
           rc = 1;
   ...
   if (flag_debug) {
      printf ("    pic30_do_align::return %s\n", rc ?
        "TRUE (.align was handled)" : "FALSE (.align was not handled)");
...

{
   if (flag_debug)
   {
      ...
      printf ("    pic30_handle_align::fragP == %p\n", fragP);
...

{
   ...
   int is_isa32 = pic30_is_isav32_machine(global_PROCESSOR);
             ...
             (input_line_pointer == search_location))
         {
            ...
            if (offset.X_op == O_constant)
            {
               if (is_isa32) {
                 ...
                 operand->X_op_symbol = make_expr_symbol (&offset);
                 operand->X_add_number = 0;
                 operand->X_op = O_REGISTER_PRE_INCREMENT_BY_N;
                 if (input_line_pointer[0] == ']') input_line_pointer++;
                 else {
                   operand->X_op = O_illegal;
                   as_bad (_("Invalid pre increment value, expected ']'."));
                 }
               } 
               else 
               {
                 operand->X_op = O_illegal;
                 as_bad (_("Invalid pre increment value."));
               }
            } /* if (offset.X_op == O_constant) */
            else
            {
             ...
             (input_line_pointer == search_location))
         {
            ...
            if (offset.X_op == O_constant)
            {
               if (is_isa32) {
                 ...
                 operand->X_op_symbol = make_expr_symbol (&offset);
                 operand->X_add_number = 0; 
                 operand->X_op = O_REGISTER_PRE_DECREMENT_BY_N;
                 if (input_line_pointer[0] == ']') input_line_pointer++;
                 else {
                   operand->X_op = O_illegal;
                   as_bad (_("Invalid pre increment value, expected ']'."));
                 ...
                 operand->X_op = O_illegal;
                 as_bad (_("Invalid pre decrement value."));
               }
            } /* if (offset.X_op == O_constant) */
            else
            {
...
#if 0
      if (!is_isa32)  {
                  /* uhmm, shouldn't this be in is_match() ? */
...

{
   ...
   switch (type)
   {
      ...
      case BFD_RELOC_PIC30_ADDR_LO:
      case BFD_RELOC_PIC30_WORD_ADDR_LO:
         value = PIC30_ADDR_LO(value);
         break;

      case BFD_RELOC_PIC30_ADDR_HI:
      case BFD_RELOC_PIC30_WORD_ADDR_HI:
         value = PIC30_ADDR_HI(value);
         break;
 ...
 *   data_allocation   TRUE if the expression appears in a data directive
 *
 *   Note: binutils-2.32 expects a return value, which should be BFD_RELOC_NONE.
 *
 ******************************************************************************/
...
#define PARSE_ERROR(...)                                              \
   {                                                                  \
     result = BFD_RELOC_PIC30_ERROR;                                  \
     as_bad(__VA_ARGS__);                                             \
     silent_ignore_rest_of_line();                                    \
   }                                                                  \

{
   ...
   int is_isa32 = pic30_is_isav32_machine(global_PROCESSOR);
   bfd_reloc_code_real_type result = BFD_RELOC_NONE;
   ...
   if ((*input_line_pointer == '+') || (*input_line_pointer == '-'))
     {
       ...
       if ((c == '+') || (c == '-') || (c == '[') || (c == ']') )
         PARSE_ERROR (_("Invalid operand syntax ('%s')."), input_line_pointer);
     }
   ...
   if (expr_to_populate->X_md.modifier == PIC30_PSVPTR_FOUND)
     PARSE_ERROR(_("psvptr() is obsolete. Please re-build this file."));
   ...
   if (expr_to_populate->X_md.modifier != PIC30_NO_MODIFIER_FOUND)
   {
      ...
      else if (!data_allocation && !BOOT_OR_SECURE)
      {
        ...
        ** without the # sign
        */
         PARSE_ERROR (_("%s modifier must be preceded by a #."),
                 (expr_to_populate->X_md.modifier ==
      ...
      if ((expr_to_populate->X_md.modifier == PIC30_PADDR_FOUND) &&
          (!data_allocation) && !pic30_is_isav32_machine(global_PROCESSOR))
         PARSE_ERROR (_("paddr() operator not allowed in instruction."));

      /* Reject operators not supported on isav32 devices */
      if (pic30_is_isav32_machine(global_PROCESSOR)) {
        const char *s1 = "() operator is not supported on ";

        if (expr_to_populate->X_md.modifier == PIC30_BOOT_FOUND)
          PARSE_ERROR (_("boot%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_SECURE_FOUND)
          PARSE_ERROR (_("secure%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_DMAPAGE_FOUND)
          PARSE_ERROR (_("dmapage%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_DMAOFFSET_FOUND)
          PARSE_ERROR (_("dmaoffset%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_EDSPAGE_FOUND)
          PARSE_ERROR (_("edspage%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_EDSOFFSET_FOUND)
          PARSE_ERROR (_("edsoffset%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_HANDLE_FOUND)
          PARSE_ERROR (_("handle%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_PSVPAGE_FOUND)
          PARSE_ERROR (_("psvpage%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_PSVOFFSET_FOUND)
          PARSE_ERROR (_("psvoffset%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_TBLPAGE_FOUND)
          PARSE_ERROR (_("tblpage%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_TBLOFFSET_FOUND)
          PARSE_ERROR (_("tbloffset%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_UNIFIED_LO_FOUND)
          PARSE_ERROR (_("unified_lo%s%s"), s1, global_PROCESSOR->printable_name);

        if (expr_to_populate->X_md.modifier == PIC30_UNIFIED_HI_FOUND)
          PARSE_ERROR (_("unified_hi%s%s"), s1, global_PROCESSOR->printable_name);
      }

   } /* if (expr_to_populate->X_md.modifier != PIC30_NO_MODIFIER_FOUND) */

   if (is_isa32) {
      /* ignore some modifiers */
   ...
   if (data_allocation && pound_sign_location)
      PARSE_ERROR(_("# sign not valid in data allocation directive."));
   ...
   if (BOOT_OR_SECURE && close_paren_location) {
     ...
     if ((input_line_pointer[0] == '(') && 
       ...
       PARSE_ERROR(_("%s() requires an argument"),
     ...
     /* call the generic expression parser in expr.c */
     expression (expr_to_populate);
   }
      ...
      if ((left->X_op == O_register) && (right->X_op == O_register))
      {
         if (left->X_add_number == right->X_add_number)
         {
            if ((left->X_add_number < 4) || (left->X_add_number > 7))
              PARSE_ERROR (_("DSP square operand register must be in range W4..W7"
   ...
   if ((expr_to_populate->X_add_symbol != NULL) && BOOT_OR_SECURE)
     PARSE_ERROR(_("Argument to %s() cannot be a symbol"),
            BOOT_ACCESS_P(expr_to_populate) ? "boot" : "secure" );
   ...
   */
   if ((expr_to_populate->X_op == O_constant) && BOOT_OR_SECURE) {
     ...
     if (value < 0 || value == 16 || value > 31) {
       PARSE_ERROR (_("Argument to %s() must be in range 0..15 "
                 "or 17..31"),
               BOOT_ACCESS_P(expr_to_populate) ? "boot" : "secure" );
       expr_to_populate->X_op = O_illegal;
     }
            ...
            (expr_to_populate->X_md.modifier != PIC30_NO_MODIFIER_FOUND))
   {
      ...
      switch (type)
        ...
        case BFD_RELOC_PIC30_DMAOFFSET:
          PARSE_ERROR (_("Cannot use a constant as the argument of dmaoffset."));
          expr_to_populate->X_add_number = 0;
...
pic30_count_operands (char *str)
{
  ...
  if (num)
    while (!is_end_of_line[(unsigned char) *c])
      {
        if (*c == ',')
          num++;
        c++;
...

{
   ...
   while (TRUE)
   {
      ...
      if (pic30_expr (&(operands[operand_count]), FALSE)
          == BFD_RELOC_PIC30_ERROR)
      {
        operand_count = -1;
        silent_ignore_rest_of_line();
        break;
 ...
 *   operand_type      Optional value to help qualify the expression.
 *
 *   RETURNS:
...

{
   ...
   switch (expression->X_op)
   {
      ...
      case O_symbol:
        /* Only one kind of symbol is currently accepted, the name
         * of a page register such as DSRPAG, DSWPAG, or TBLPAG */
 ...
 *
 *   *opcode is the first opcode record with a matching name.
 *
...
{
   struct pic30_opcode * discovered_opcode = NULL;
   ...
   int is_isav4 = pic30_is_isav4_machine(global_PROCESSOR);
   int is_isav32 = pic30_is_isav32_machine(global_PROCESSOR);
   char * line_pointer_start = input_line_pointer;
   char *format_gap = "                                       ";
   char * line_save_buf=NULL;


   if (flag_debug)
      printf ("--> pic30_find_opcode_based_on_operands::begin\n");
   ...
   *err_string = (char *) 0;
   ...
   /* Preserve the input line, which may become corrupted
    * while processing operands. */
   line_save_buf = strdup(input_line_pointer);

   for (; ((current_opcode < last_opcode) &&
	   (strcmp (opcode->name, current_opcode->name) == 0));
	   current_opcode++)
   {
      *operand_count = pic30_count_operands (input_line_pointer);
      ...
      if (((current_opcode->flags & F_ISAV4) && is_isav4) ||
          ((current_opcode->flags & F_ISA32V0) && is_isav32) ||
          ((current_opcode->flags & F_ECORE) && is_ecore) ||
          ((current_opcode->flags & F_FCORE) && 
             !is_ecore && !is_isav4 && !is_isav32) ||
          ((current_opcode->flags & (F_ISAV4|F_ISA32V0|F_ECORE|F_FCORE)) == 0))
        (void)0;
      else
        continue;
      ...
      if (current_opcode->number_of_operands == *operand_count)
      {
         ...
         int cannot_populate_all_operands = 0;
         short valid_operands;

         if (flag_debug)
            printf ("%s::testing '%s' opcode %lx\n",
                    format_gap, current_opcode->name,
                    current_opcode->opcode[0]);

         /* Parse operands using the current opcode flags */
         global_current_opcode_flags = current_opcode->flags;
         global_pic30_dot_expression = FALSE;
         valid_operands = pic30_parse_operands (&operands[0]);
         ...
         input_line_pointer = line_pointer_start;
         (void) strcpy(input_line_pointer, line_save_buf);
         ...
         for (i = 0; i < current_opcode->number_of_operands; i++)
         {
            ...
            int tried_match = 0;
            int populated_operand = 0;

            if (flag_debug)
              printf("    pic30_find_opcode_based_on_operands::operand->type: %lx, operator_mask: %lx\n",
                   operand->type, operator_mask);
                ...
                (operand->immediate == operands[i].X_md.immediate))
            {
             ...
             if (flag_debug)
               printf ("    pic30_find_opcode_based_on_operands::partial match"
                       " operand %d\n", i+1);

            /* If an "is_match()" function is available,
             * populate an "op_value" structure and test it. */
            if (operand->is_match) {
                 populated_operand =
                   pic30_populate_operand_value(&(operands[i]),
                                                &op_value,0);
                 if (populated_operand) {
                   tried_match = 1;
                   types_match = (*operand->is_match)(&op_value);
                 } else { 
                   cannot_populate_all_operands = 1;
                 }
               }
            }
            /* GM: A separate test may not be required here. */
            else if (operand->type == OPND_PAGE_SYMBOL) {
              tried_match = 1;
              pic30_populate_operand_value(&(operands[i]), &op_value,
					   OPND_PAGE_SYMBOL);
              types_match = (*operand->is_match)(&op_value);
            }
            else
            ...
            if (!types_match)
            {
               ...
               */
              if ((tried_match) && (discovered_opcode == NULL) &&
                  ((i+1 > *err_operand) || (*err_string == NULL))) {
                  if (flag_debug) {
                    printf("    maybe report:\n    '%s'\n   for opcode '%s'\n",
                           operand->info_string, current_opcode->name);
                  }
                  *err_operand = i+1;
              ...
              if (flag_debug)
              {
                 printf("    pic30_find_opcode_based_on_operands::match failed\n");
              }
         ...
         if (types_match)
         {
            ...
            match = TRUE;
            if (discovered_opcode) {
              /* pick the first opcode that matches, unless
               *    1) a smaller one is found, and all operands are populated 
               * or 
               *    2) a bigger one is found, and not all operands are
               *       populated 
               */
              if (cannot_populate_all_operands) {
                if (discovered_opcode->bytes_per_IW >=
                    current_opcode->bytes_per_IW) {
                  continue;
                }
              } else {
                if (discovered_opcode->bytes_per_IW <=
            ...
            *err_operand = 0;
            *err_string = NULL;
            discovered_opcode = current_opcode;
         }
      }
   }
   ...
   if (flag_debug && discovered_opcode) {
      printf ("%s::selected '%s' opcode %lx", format_gap,
              discovered_opcode->name, discovered_opcode->opcode[0]);
      if (discovered_opcode->number_of_words == 2)
        printf (" %lx\n", discovered_opcode->opcode[1]);
      else
        printf("\n");
   }

   if (discovered_opcode) {
      /* Parse operands again, using the selected opcode flags */
      global_current_opcode_flags = discovered_opcode->flags;
      global_pic30_dot_expression = FALSE;
      (void) pic30_parse_operands (&operands[0]);
   }
...
#define CREATE_CALL_GOTO_ACCESS_INSTR {                                      \
         ...
         char *                                                              \
         out_frag = frag_more  (PIC30_SIZE_OF_PROGRAM_WORD * 2);             \
                                                                             \
         /* output the first instruction word */                             \
         md_number_to_chars (out_frag, insn, PIC30_SIZE_OF_PROGRAM_WORD);    \
                                                                             \
         /* create a fixup */                                                \
         fix_new (frag_now, out_frag - frag_now->fr_literal,                 \
                  2, operands[0].X_add_symbol,                               \
                  frag_now->fr_offset, FALSE,                                \
                  BFD_RELOC_PIC30_CALL_ACCESS);                              \
                                                                             \
         /* output the second instruction word */                            \
         md_number_to_chars (out_frag + PIC30_SIZE_OF_PROGRAM_WORD,          \
   ...
   expressionS operands[];
{
   ...
   unsigned int phantom_byte = pic30_has_phantom_byte(global_PROCESSOR);

   if (flag_debug)
      printf ("--> pic30_create_call_insn::begin\n");

   pic30_pad_insn(opcode);

   /* keep globals current */
   global_current_location += ((opcode->bytes_per_IW + phantom_byte) *
                               opcode->number_of_words);
   global_p_directive_active = FALSE;;
   ...
   switch (operands[0].X_op)
   {
        ...
        else
      {
         ...
         md_number_to_chars (output_frag, insn, 
                             opcode->bytes_per_IW + phantom_byte);
       }
...
{
   unsigned short i;
   unsigned long full_insn[MAX_WORDS_PER_INSN];
   unsigned int phantom_byte = pic30_has_phantom_byte(global_PROCESSOR);
   char *output_frag[2];
   ...
   for (i = 0; i < MAX_WORDS_PER_INSN; i++) {
     full_insn[i] = opcode->opcode[i];
   }
   ...
   {
     unsigned int num_words = opcode->number_of_words;
     unsigned int word_len = (opcode->bytes_per_IW + phantom_byte);

     /* If the instruction has multiple words,
        allocate both in the same frag. */
     output_frag[0] = frag_more (num_words * word_len);
     if (num_words == 2) {
       output_frag[1] = output_frag[0] + word_len;
     }

     if (num_words > 2) {
       as_bad (_("Unexpected instruction length"));
     }
   }
      ...
      switch (operands[i].X_op)
      {
         case O_constant:
            if (!operands[i].X_md.immediate)
            {
               ...
               fix = fix_new_exp (frag_now, output_frag[i] - frag_now->fr_literal,
         ...
         /* fall through */
         case O_register:
            (void) pic30_add_operand (full_insn, operands[i].X_add_number, opnd);

            md_number_to_chars (output_frag[i], full_insn[i], 
                                opcode->bytes_per_IW + phantom_byte);
         ...
         {
            fixS * fix =
               fix_new_exp (frag_now, output_frag[i] - frag_now->fr_literal,
                            pic30_relocation_info[opnd->relocation].fix_size,
                            &(operands[i]),
                            pic30_relocation_info[opnd->relocation].pc_relative,
  ...
  if (flag_debug) {
      ...
      for (i = 0; i < opcode->number_of_words; i++)
        printf ("    pic30_create_do_insn::opcode[%d] = %#lx\n", i, full_insn[i]);
 ...
 *   on dsPIC classic devices (but not for 33A devices.)
 *
   ...
   expressionS operands[];
{
   ...
   switch (insn_word1)
   {
      ...
      case CALL_INSN:
         dwarf2_emit_insn (0); /* since length is unknown */
         pic30_create_call_insn (opcode, operands);
         break;
...
#if defined(OBJ_ELF)
  // This flag is set with assembler -g option, but not with compiler's debug info!
...
/******************************************************************************
 *
 *   This function is called to encode a 16, 32, or 64-bit instruction.
 *
 ******************************************************************************/

{
   static int inception_number = 0;
   unsigned int insn = opcode->opcode[0];
   ...
   unsigned char have_bra6_insn = ((insn == CPWBEQ_W_INSN) ||
                                   ...
                                   (insn == CPWBNE_W_INSN) ||
                                   (insn == CPWBNE_B_INSN));
   unsigned char have_isa32bra_insn = ((opcode->flags & F_PCREL_INSN) != 0);
   unsigned long full_insn[MAX_WORDS_PER_INSN];
   unsigned int phantom_byte = pic30_has_phantom_byte(global_PROCESSOR);

   short i;
   char * output_frag[MAX_WORDS_PER_INSN];
   ...
   if (flag_debug) {
      printf ("--> pic30_create_insn (%d operands)::begin insn %d\n",
              operand_count, inception_number++);
      printf ("    pic30_create_insn::\'%s\' bytes_per_IW = %d, number_of_words = %d\n", 
              opcode->name, opcode->bytes_per_IW, opcode->number_of_words);
  }

   if (opcode->flags & F_DEPRECATED) {
      as_warn(_("Instruction is deprecated."));
   }

   if (opcode->flags & F_NOTIMPLEMENTED) {
      as_warn(_("Instruction may not be support in bitfile yet."));
   }
   ...
   unsigned int which_reloc_bra[wrk_last] = {
     0,
     BFD_RELOC_PIC30_BRANCH_ABSOLUTE,
     BFD_RELOC_PIC30_PCREL_ACCESS
   };

   unsigned int which_reloc_bra6[wrk_last] = {
     0,
     BFD_RELOC_PIC30_BRANCH_ABSOLUTE6,
     BFD_RELOC_PIC30_WORD_ACCESS
   };
   
   unsigned int which_reloc_bra32[wrk_last] = {
     0,
     BFD_RELOC_PIC30_BRANCH_ABSOLUTE24,
     0
   };

   unsigned int *which_reloc;

   if (have_bra_insn) {
     which_reloc = which_reloc_bra;
   } else if (have_isa32bra_insn) {
     which_reloc = which_reloc_bra32;
   } else {
     which_reloc = which_reloc_bra6;
   }
         
  if (have_isa32bra_insn && global_pic30_dot_expression) {
    as_warn (_("This 'dot expression' may not point to a valid or correct instruction! "
               "It is strongly recommended to use a label or local symbol instead."));
  }
   ...
   for (i = 0; i < operand_count; i++)
   {
      ...
      insn_word = (operand->encoding[0].shift / 32);
      switch (operands[i].X_op)
      {

         case O_constant:
            if (((have_bra_insn) || (have_bra6_insn) || (have_isa32bra_insn)) &&
                (!operands[i].X_md.immediate))
            {
               ...
               fix = fix_new_exp (frag_now, output_frag[insn_word] - frag_now->fr_literal,
                                  reloc.fix_size, &(expr), reloc.pc_relative,
                                  which_reloc[wrk_absolute]);
                                              ...
                                              &operand_value,operand->type))
            {
              ...
              */
               if (operand->insert)
               {
                  ...
                  insn = (*operand->insert)(full_insn, opcode->flags,
                                            operand, &operand_value,
                                            &error_msg);

                  if (error_msg != (char *) NULL)
                  {
                     as_fatal ("%s", error_msg);
                     free (error_msg);
                  } /* if (error_msg != (char *) NULL) */
               } /* if (operand->insert) */
               else
               {
                  ...
                  insn = pic30_add_operand (full_insn, value, operand);
               } /* else */
              ...
              const char *err_str = " modifier not allowed in instruction";
               ...
               if (operands[i].X_op != O_W_REG)
               {
                 ...
                 */
                  fixS * fix;
                  long fixup_where =
                       output_frag[insn_word] - frag_now->fr_literal;  /* offset within frag */

                  if (flag_debug)
                     printf ("    pic30_create_insn::fixup->type = %s\n"
                             "    pic30_create_insn::fixup->where = %ld\n"
                  ...
                  if (fixup_where < 0) {
                    as_fatal (_("%s%s\n"), "invalid frag offset detected for ",
                            bfd_get_reloc_code_name (reloc.value));
                  }
...
#                 define FLASH_RELOC_SIZE 4
                  ...
                  if (operands[i].X_md.modifier == PIC30_TBLOFFSET_FOUND)
                  {
                    ...
                    {
                      if (flag_debug)
                        fprintf (stdout, "    pic30_create_insn::PACKED_HI\n");

                      if (reloc.value != BFD_RELOC_PIC30_WORD ) {
                        as_bad (_("%s%s"), "packed_hi", err_str);
                        break;
                      }
                      else
                    ...
                    } /* else if (PIC30_PACKED_HI_FOUND) */
                  else if (operands[i].X_md.modifier == PIC30_PACKED_LO_FOUND)
                    {
                      if (flag_debug)
                        fprintf (stdout, "    pic30_create_insn::PACKED_LO\n");

                      if (reloc.value != BFD_RELOC_PIC30_WORD ) {
                        as_bad (_("%s%s"), "packed_LO", err_str);
                        break;
                      }
                      else
                        fix = fix_new_exp (frag_now,
                    ...
                    } /* else if (PIC30_UNIFIED_LO_FOUND) */
                    ...
                    } /* else if (PIC30_UNIFIED_HI_FOUND) */
                  else
                  {
                     fix = fix_new_exp (frag_now,
                                        fixup_where,
...
#if defined(OBJ_ELF)
  // FIXME: Should this arg include "* opcode->number_of_words" to accommodate isav32?
  dwarf2_emit_insn(opcode->bytes_per_IW + phantom_byte);
  // This flag is set with assembler -g option, but not with compiler's debug info!
  if (global_DEBUGINFO != NO_GEN_DEBUGINFO)
  {
    frag_wane (frag_now);  /* close off the current frag */
  ...
  if (flag_debug) {
      ...
      for (i = 0; i < opcode->number_of_words; i++) 
        printf ("    pic30_create_insn::opcode[%d] = %#lx\n", i, full_insn[i]);
...
/******************************************************************************
 *
 *   Return whether the current instruction is a hazard when following REPEAT.
 *
 ******************************************************************************/
bfd_boolean
insn_is_REPEAT_hazard(struct pic30_opcode * opcode,
                      expressionS operands[], short operand_count)
{
  int i,j;
  bfd_boolean rc = FALSE;
  int match_reg = -1;

  /* SFRs from table 2.1 in DOS-04094_RevB1.pdf */
  const char *isav32_sfr[] =
    { "PC", "SPLIM", "RCOUNT", "DISIIPL", "CORCON",
      "MODCON", "XMODSRT", "XMODEND", "YMODSRT",
      "YMODEND", "XBREV", "PCTRAP", "FEX", "FEX2",
      "PCHOLD", "VFA",
      0 };
  ...
  {
    switch (operands[j].X_op)
    {
      case O_symbol:
        i = 0;
        while (isav32_sfr[i]) {
          /* Does this operand reference an SFR symbol? */
          if (!strcmp(operands[j].X_add_symbol->bsym->name, isav32_sfr[i])) {
            rc = TRUE;
            break;
          }
          i++;
      ...
      case O_REGISTER_POST_INCREMENT:
      case O_REGISTER_POST_DECREMENT:
        if (operands[j].X_add_number == match_reg)
          rc = TRUE;
        else
          match_reg = operands[j].X_add_number;
        break;
      ...
      {
         expressionS * left =
            symbol_get_value_expression (operands[j].X_add_symbol);
         /* integer addend is in right->X_add_number, but we don't need it */

         if (left->X_add_number == match_reg)
           rc = TRUE;
         else
           match_reg = left->X_add_number;
         break;
      }
   ...
   char * input_line;
{
   if (flag_debug) {
     ...
     (void) as_where(&line_num);

     printf ("\n--> md_assemble::begin  >>> %3d %06lx   %s\n",
             line_num, global_current_location, input_line);
   }
   ...
   if (input_line != NULL)
   {
      ...
      /* If the last char is a comma, flag an error. */
      if (input_line[strlen(input_line)-1] == ',')
      {
        as_bad (_("Invalid syntax; is there an operand missing? ('%s')."), input_line);
        silent_ignore_rest_of_line();
        return;
      }
      ...
      else
      {
         ...
         if (token != NULL)
         {
            ...
            else
            {
               ...
               /* Note: The opcode returned by hash_find is the first
                * of several opcode records to match the token, but
                * it may not have the correct opcode flags needed to
                * parse the operands accurately.
                *
                * To address this problem, pic30_parse_operands() must be called
                * for each opcode whose string matches the token, not just
                * the first one. This function call has been moved into the
                * opcode loop in pic30_find_opcode_based_on_operands().
                */
               {
                  unsigned short max_operands;
                  unsigned short min_operands;
                  ...
                  if (opcode != NULL)
                  {
                     ...
                     if (pic30_operands_encode_identically (opcode, operands))
                     {
                        if ((previous_insn_is_repeat) &&
                           ...
                           as_bad (_("'%s' cannot follow a REPEAT."),
                                   opcode->name);
                        else if ((previous_insn_is_repeat) &&
                            (insn_is_REPEAT_hazard(opcode, operands, operand_count)))
                           as_bad (_("This instruction cannot follow a REPEAT."));
                        else if (opcode->number_of_words == 0)
                           as_fatal (_("A dummy instruction cannot be "
                                       "used!"));
                        else if ((opcode->number_of_words == 2) &&
                                 (opcode->bytes_per_IW != 4))
                          {
                           pic30_create_2word_insn (opcode, operands);
                           pic30_check_isa (opcode, operands);
                          }
                     ...
                     if (pic30_is_isav32_machine(global_PROCESSOR))
                     {
                       previous_insn_is_repeat =
                         ((opcode->opcode[0] == ISAV32_REPEATLS_INSN)  |
                          (opcode->opcode[0] == ISAV32_REPEAT_INSN)    |
                          (opcode->opcode[0] == ISAV32_REPEATW_INSN));
                     }
                     else
                     {
                       previous_insn_is_repeat =
...

{
   ...
   if ((subseg_text_p (now_seg)) || (now_seg == text_section))
   {
     if (pic30_debug) {
#if 0
      printf("    global_must_align_location_counter = %s\n",
             global_must_align_location_counter ? "TRUE" : "FALSE");
#endif
      printf("    global_current_location = %ld\n",
             global_current_location);
      /* Not sure of the benefit of 'global_must_align_location_counter' for
       *   the purported value of 'saving time'.  We know labes must be
       *   aligned and its simple to check if the global_current_location is
       *   aligned.
       */
     }
      if (
#if 0
          (global_must_align_location_counter) &&
#endif
   ...
   segT seg __attribute__ ((__unused__));
{
   ...
   if (flag_debug)
   {
      printf ("--> md_apply_fix (..., 0x%04lx, ...)::begin\n",
              (long) *valueP);
      printf ("    md_apply_fix::line number = %d\n", fixP->fx_line);
      printf ("    md_apply_fix::frag address       (fx_frag) = 0x%lx\n",
              (long unsigned int) fixP->fx_frag);
      printf ("    md_apply_fix::frag offset       (fx_where) = 0x%04lx\n", fixP->fx_where);
      printf ("    md_apply_fix::object offset (frag_address) = 0x%04lx\n",
              (long) fixP->fx_frag->fr_address);
      printf ("    md_apply_fix::data address    (fr_literal) = 0x%lx\n",
              (long unsigned int) fixP->fx_frag->fr_literal);
      printf ("    md_apply_fix::fixup address (fix_position) = 0x%lx\n",
              (long unsigned int) fix_position);
      printf ("    md_apply_fix::size = %d\n", fixP->fx_size);
      printf ("    md_apply_fix::fixP->fx_pcrel = %s\n",
              (fixP->fx_pcrel ? "TRUE" : "FALSE"));
      printf ("    md_apply_fix::fixP->fx_addsy = %s\n",
              fixP->fx_addsy ? S_GET_NAME(fixP->fx_addsy) : "<none>");
      printf ("    md_apply_fix::fixP->fx_subsy = %s\n",
              fixP->fx_subsy ? S_GET_NAME(fixP->fx_subsy) : "<none>");
      printf ("    md_apply_fix::fixP->fx_offset = %ld\n", fixP->fx_offset);
   } /* if (flag_debug) */

  /* Don't process these relocation types here. Instead,
   * tc_gen_reloc() will store fixP->fx_offset in the
   * relocation record addend field, so that very large
   * (32-bit) signed offsets can be supported in devices
   * with lots of RAM. XC16-2181 */
  if (fixP->fx_addsy != NULL)
  {
    switch (fixP->fx_r_type)
    {
      ...
      case BFD_RELOC_PIC30_WORD_EDSOFFSET:
        ...
        if (flag_debug) {
          printf ("    md_apply_fix:: Will be processed by the linker\n");
          printf ("<-- md_apply_fix::exit\n");
        }
        return;
   ...
   /* This means the relocation is fully resolved */
   fixP->fx_done = TRUE;
   ...
   {
     /*
      * In this block we perform error-checking based
      * on the relocation type and the nature of the
      * symbol, such as what kind of section defined it.
      */
      ...
      else
      {
         ...
         {   
	      ...
	      if (fixP->fx_r_type == BFD_RELOC_PIC30_PGM_ADDR_MSB)
            ...
            switch (fixP->fx_r_type)
            {
               case BFD_RELOC_PIC30_PGM_ADDR24:
               case BFD_RELOC_PIC30_PGM_ADDR_LSB:
               case BFD_RELOC_PIC30_PGM_ADDR_MSB:
   ...
   if (flag_debug)
      printf ("    md_apply_fix::tc_fix_data.operand = %ld\n",
   ...
   if (fixP->tc_fix_data.operand >= 0)
   {
      ...
      if (howto->bitsize > 32) {
        insn[1] = bfd_getl32(fix_position+4);
        insn[0] = bfd_getl32(fix_position);
      } else {
        insn[0] = bfd_getl32(fix_position);
      }
      ...
      /*
       * This function inserts a value directly into the opcode.
       * Some 16-bit fixups are not processed here, as noted above.
       */
      (void) pic30_add_operand(insn, value, opnd);

      if (howto->bitsize > 32) {
        bfd_putl32(insn[1], fix_position+4);
        bfd_putl32(insn[0], fix_position);
      } else {
        bfd_putl32(insn[0], fix_position);
      }
   } /* if (fixP->tc_fix_data.operand >= 0) */
   else
   {
      ...
      if (fixP->tc_fix_data.fill.in_text_section)
      {
         switch (fixP->fx_r_type)
         {
...
#endif
            {
               ...
               for (i = 0; i < size; i++)
                  PIC30_INSERT_CHAR_INTO_FIX (fixP, buf[i], fix_position,
                                               location)
               free (buf);
               write_value = FALSE;  /* don't write it below, again */
      ...
      {
         if (flag_debug)
            printf ("    md_apply_fix::Final value = 0x%08lx\n", (long) value);

         if (!fixP->tc_fix_data.fill.in_text_section)
           {
             /*
             ** In data memory, we might interleave
             ...
             */
             ...
             switch (fixP->fx_size)
               ...
               case 1 :
               case 2 :
                 md_number_to_chars (fix_position, num1, size);
                 break;
               case 4 :
                 md_number_to_chars (fix_position, num1, size);
                 if (size > 4) md_number_to_chars (fix_position+4, num2, 4);
               }
           }
         else
...
#if 0
   if (1) {
      printf ("\n--> pic30_relax_call::fr_address = %lx\n",
              fragP->fr_address);
      printf ("--> pic30_relax_call::fr_line = %d\n",
              fragP->fr_line);
      printf ("--> pic30_relax_call::last_fr_address = %lx\n",
              fragP->last_fr_address);
      printf ("--> pic30_relax_call::fr_fix = %lx\n",
              fragP->fr_fix);
      printf ("--> pic30_relax_call::fr_var = %lx\n",
              fragP->fr_var);
      if (fragP->fr_next)
        printf ("--> pic30_relax_call::fr_next->fr_address = %lx\n",
                fragP->fr_next->fr_address);
      printf ("--> pic30_relax_call::tc_frag_data.in_text_section = %s\n",
              fragP->tc_frag_data.in_text_section ? "TRUE" : "FALSE");
      printf ("--> pic30_relax_call::tc_frag_data.p_directive_active = %s\n",
   ...
   segT sec;
{
    ...
    */
   /* make sure a symbol is not local before checking if it is weak
    * fix for xc16-107 */
   if ((global_RELAX != DO_RELAX) ||
       (fragP->fr_symbol->bsym && (fragP->fr_symbol->bsym->flags & BSF_WEAK)) ||
   ...
   PTR this_type __attribute__ ((__unused__));
{
   ...
   if (flag_debug)
   {
      printf ("    pic30_prepare_relax_scan::aim = %ld (was %ld)\n", *aim, save_aim);
...

{
   ...
   *reloc->sym_ptr_ptr = symbol_get_bfdsym (fixp->fx_addsy);
   ...
   switch (fixp->fx_r_type)
   {
     ...
     case BFD_RELOC_PIC30_EDSOFFSET:
     case BFD_RELOC_PIC30_WORD_EDSOFFSET:
      /* Linker will resolve symbol+offset for these types */
      reloc->addend = fixp->fx_offset;
     break;

     default:
     /* All other offsets have been handled/encoded by md_apply_fix() */
      reloc->addend = 0;
   }

   if (flag_debug) {
      printf ("    tc_gen_reloc:: reloc->addend = %lx\n", reloc->addend);
...
/******************************************************************************
 *
 *   This function is called to encode bfins, bfext instructions
 *   on dsPIC 'CH' devices. For dsPIC 33A devices, pic30_create_insn()
 ...
 *****************************************************************************/
{
   ...
   unsigned long insn[2];
   char    *insn_addr[2];
   unsigned int idx = 0;  /* begin with focus on 1st insn word */
   unsigned int num_words = opcode->number_of_words;
   unsigned int word_len = (opcode->bytes_per_IW + phantom_byte);

   unsigned int is_BFINS  = (opcode->opcode[0] == BFINS_INSN);
   unsigned int is_BFINSF = (opcode->opcode[0] == BFINSF_INSN);
   unsigned int is_BFINSL = (opcode->opcode[0] == BFINSL_INSN);
   unsigned int is_BFEXT  = (opcode->opcode[0] == BFEXT_INSN);
   unsigned int is_BFEXTF = (opcode->opcode[0] == BFEXTF_INSN);
   ...
   /* keep globals current */
   global_current_location += ((opcode->bytes_per_IW + phantom_byte) * num_words);
   global_p_directive_active = FALSE;

  /* allocate both instruction words in the same frag */
  insn_addr[0] = frag_more (num_words * word_len);
  insn_addr[1] = insn_addr[0] + word_len;

  insn[0] = opcode->opcode[0];
  insn[1] = opcode->opcode[1];
  ...
  for (i = 0; i < opcode->number_of_operands; i++)
   {  
      ...
      if (((is_BFINS  || is_BFINSF) && (i == 3)) ||
          ((is_BFINSL || is_BFEXT || is_BFEXTF) && (i == 2))) {
         idx = 1; /* switch focus to the second word */
      }
      ...
      switch (operands[i].X_op)
      {
         ...
         {  
           ...
           if ((is_BFEXT || is_BFEXTF) && (i == 3)) {
             idx = 0;  /* switch focus back to the first word */
           }

           (void) pic30_add_operand (&insn[idx], operands[i].X_add_number, opnd);

           /* copy the updated instruction into the output frag */
           md_number_to_chars (insn_addr[idx], insn[idx],
                                 PIC30_SIZE_OF_PROGRAM_WORD);
           break;  
        }
        ...
        default:
         {
            ...
            if (pic30_populate_operand_value (&(operands[i]),
              ...
              if (opnd->insert) {
                ...
                insn[idx] = (*opnd->insert)(&insn[idx], opcode->flags,
                                            opnd, &operand_value,
                                            &error_msg);
             ...
             if (operands[i].X_op != O_W_REG)  {
               ...
               fixS * fix;
               fix = fix_new_exp (frag_now,
                                  insn_addr[1] - frag_now->fr_literal,
                                  reloc.fix_size, &(operands[i]),
                                  reloc.pc_relative, reloc.value);
             }
          }
  ...
  if (flag_debug) {
    ...
    for (j = 0; j < opcode->number_of_words; j++)
      printf ("    pic30_create_bf_insn::opcode[%d] = %06lx\n", j, insn[j]);
    printf ("<-- pic30_create_bf_insn::exit\n");
  }
}
/******************************************************************************/

void pic30_define_label (sym) 
  symbolS *sym;
{
  global_last_sym = sym;
  global_last_sym_loc = global_current_location;
