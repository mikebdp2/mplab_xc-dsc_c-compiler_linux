	     ...
	     const char *string, unsigned int address)
{
  ...
  if (user_listing_lhs_width == 0)
    listing_lhs_width =
      (PIC30_FRAG_IN_PROGRAM_MEMORY (list->frag) ?
       pic30_bytes_per_insn(0) : LISTING_LHS_WIDTH);

  if (user_listing_lhs_width_second == 0)
    listing_lhs_width_second =
      (PIC30_FRAG_IN_PROGRAM_MEMORY (list->frag) ?
       pic30_bytes_per_insn(0) : LISTING_LHS_WIDTH_SECOND);
#endif
  ...
  if (had_errors ())
    ...
    /* this format is used for data memory in dsPIC classic devices */
  ...
  */
  if ((PIC30_FRAG_IN_PROGRAM_MEMORY (list->frag)) && (listing_lhs_width > 2))
    switch (list->frag->fr_address % PIC30_SIZE_OF_PROGRAM_WORD)
      {
      case 3:
        if (pic30_has_phantom_byte(0)) break;
        /* not phantom byte */
        nchars -= 3;
        fprintf (list_file, "   ");
      case 2:
...
#ifdef TC_PIC30
  emit_line (list, "%s%s\n",
           pic30_has_phantom_byte(0) ? "" : "   ", string ? string : "");
#else
  emit_line (list, "\t%s\n", string ? string : "");
#endif
