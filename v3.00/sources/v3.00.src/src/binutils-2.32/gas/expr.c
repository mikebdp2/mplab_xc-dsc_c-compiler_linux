...
# endif
	...
	{
#ifdef PIC30
    global_pic30_dot_expression = TRUE;
#endif
	  current_location (expressionP);
	  break;
	}
		...
		&& ! is_part_of_name (input_line_pointer[8]))
               || (strncasecmp (input_line_pointer, "startof_hi.", 11) == 0
                   && ! is_part_of_name (input_line_pointer[11]))
	       || (strncasecmp (input_line_pointer, "sizeof.", 7) == 0
		   && ! is_part_of_name (input_line_pointer[7]))
               || (strncasecmp (input_line_pointer, "sizeof_hi.", 10) == 0
                   && ! is_part_of_name (input_line_pointer[10]))
               || (strncasecmp (input_line_pointer, "endof.", 6) == 0
                   && ! is_part_of_name (input_line_pointer[6]))
               || (strncasecmp (input_line_pointer, "endof_hi.", 9 ) == 0
                   && ! is_part_of_name (input_line_pointer[9])))
#else
      else if ((strncasecmp (input_line_pointer, "startof.", 8) == 0
		&& ! is_part_of_name (input_line_pointer[8]))
	       || (strncasecmp (input_line_pointer, "sizeof.", 7) == 0
	...
	{
#ifdef TC_PIC30
	  int start = 0, start_hi = 0;
          int end= 0, end_hi = 0;
          int size = 0, size_hi = 0;
...
#ifdef TC_PIC30
	  if (*input_line_pointer != '(')
	    as_bad (_("syntax error in .startof., .sizeof., .endof.,"
                      ".startof_hi., or .endof_hi."));
	  else
	    {
	      char *buf;
	      ...
	      buf = (char *) xmalloc (strlen (name) + 13);
              if (end)
                sprintf (buf, ".endof.%s", name);
              else if (end_hi)
                sprintf (buf, ".endof_hi.%s", name);
	      else if (start)
		sprintf (buf, ".startof.%s", name);
              else if (start_hi)
                sprintf (buf, ".startof_hi.%s", name);
	      else if (size_hi)
	        sprintf (buf, ".sizeof_hi.%s", name);
              else
  ...
  while (op_left != O_illegal && op_rank[(int) op_left] > rank)
...
#ifdef TC_PIC30
      /*
      ** We prefer to treat bignums in expressions the same
      ** as if they appeared alone. See, for example,
      ** .long <bignum>
      */
      if (resultP->X_op == O_big)
	{
	  if (resultP->X_add_number > 0)
            {
              as_warn (_("left operand is a bignum; truncated to 4 bytes"));
              resultP->X_add_number = generic_bignum_to_int32();
            }
	  else
            {
      ...
      if (right.X_op == O_big)
	{
	  if (right.X_add_number > 0)
            {
              as_warn (_("right operand is a bignum; truncated to 4 bytes"));
              right.X_add_number = generic_bignum_to_int32();
            }
	  else
            {
...
#if PIC30
/*
 * This function disappeared in v2.32, but tc-pic30.c needs it
 */
char
get_symbol_end ()
{
