...
#define MARK_DWARF_STRING_SECTIONS_FOR_MERGING 0
...
generic_dwarf2_emit_offset (symbolS *symbol, unsigned int size)
{
  ...
  exp.X_md.modifier = PIC30_NO_MODIFIER_FOUND;
  exp.X_md.immediate = FALSE;
#endif
  emit_expr (&exp, size);
}
#endif
...
#if 0
  printf("  offset in current frag = %lx\n", ofs);
#endif
  if (linkrelax)
    {
      char name[120];
...
dwarf2_emit_insn (int size)
{
  ...
  dwarf2_gen_line_info (frag_now_fix () - (size / OCTETS_PER_BYTE), &loc);
...
dwarf2_directive_loc (int dummy ATTRIBUTE_UNUSED)
{
  ...
  while (ISALPHA (*input_line_pointer))
      ...
      else if (strcmp (p, "discriminator") == 0)
	{
	  ...
	  if (value >= 0)
#if defined(TC_PIC30)
          /* FIXME: we're not generating correct DWARF for this */
	    current.discriminator = 0;
#else
...
size_inc_line_addr (int line_delta, addressT addr_delta)
{
...
#if 0
  printf("\nsize_inc_line_addr(): line_delta = %x, addr_delta = %lx\n",
         line_delta, addr_delta);
#endif
  ...
  if (line_delta == INT_MAX)
	...
	len = 1;
      else if (addr_delta)  /* FIXME: XC32 does not test addr_delta here - GM */
...
#if 0
  printf("\nemit_inc_line_addr(): line_delta = %x, addr_delta = %lx\n",
         line_delta, addr_delta);
#endif
...
#ifndef TC_PIC30
  /* FIXME: This can't be right, because addr_delta is an unsigned
   * type. Yet signed values are intentionally passed as addr_delta
   * at multiple points in this file. See for example:
   *   max_chars = size_inc_line_addr (line_delta, -DWARF2_LINE_MIN_INSN_LENGTH);
   * in relax_inc_line_addr(). WTF?  GM 3/23/22  */
...
#if 0
  printf("\nemit_fixed_inc_line_addr(): addr_delta = %lx, len = %x, calc = %x\n",
         addr_delta, len, OCTETS_PER_BYTE * size_fixed_inc_line_addr (line_delta, addr_delta));
#endif
...
#if defined(TC_PIC30)
  if (!pic30_is_isav32_machine(0)) {
    gas_assert (len == (OCTETS_PER_BYTE * size_fixed_inc_line_addr (line_delta, addr_delta)));
  } else
#endif
  ...
  if (addr_delta > ADDR_DELTA_LIMIT)
    {
      ...
      if (!pic30_is_isav32_machine(0)) {
        *p++ = DW_LNS_extended_op;
        *p++ = (PIC30_PHANTOM_BYTE);
        p += pic30_output_leb128 (p, sizeof_address + 1, 0);
...
#if defined(TC_PIC30)
      if (!pic30_is_isav32_machine(0)) {
        p += sizeof_address * OCTETS_PER_BYTE;
      } else {
        p += sizeof_address;
      }
#else
...
relax_inc_line_addr (int line_delta, symbolS *to_sym, symbolS *from_sym)
{
...
#if 0
  printf("\nrelax_inc_line_addr(): line_delta = %x, to_sym = %s, from_sym = %s\n",
         line_delta, to_sym->bsym->name, from_sym->bsym->name);
#endif
  ...
  if (!pic30_is_isav32_machine(0)) {
#if 0
    printf("relax_inc_line_addr(): adjusting max_chars from %x to %x\n",
         max_chars, (max_chars * OCTETS_PER_BYTE));
#endif
    max_chars *= OCTETS_PER_BYTE; /* adjust for phantom bytes */
   }
#endif
...
#if defined(TC_PIC30)
  /* FIXME: The offset between frag addresses should not be negative.
   * Here we are working around a problem whose root cause
   * remains undiscovered.  - GM 02/17/22  */
  if (addr_delta < 0)
    addr_delta = 0; /* suppress negative offsets */
#endif
...
#if defined(TC_PIC30)
if (!pic30_is_isav32_machine(0)) {
  size *= OCTETS_PER_BYTE; /* adjust for phantom bytes */
}
#endif
...
#if 0
  printf("\ndwarf2dbg_convert_frag(): symbol %s resolves to %lx\n",
         frag->fr_symbol->bsym->name, addr_diff);
#endif
#if defined(TC_PIC30)
  /* FIXME: Same sad tale as noted above - GM 02/17/22  */
  if (addr_diff < 0)
    addr_diff = 0; /* suppress negative offsets */
#endif
...
#if defined(TC_PIC30)
  /* Calculate size and skip now, for simplicity */
  offsetT skip;
...
#elif defined(TC_PIC30)
      PIC30_INSERT_DATA_INTO_DATA_MEMORY(dirname,len);
  ...
  if (emit_other_sections)
    {
      ...
      PIC30_SET_INFO_ATTR(str_seg);
#if MARK_DWARF_STRING_SECTIONS_FOR_MERGING
      str_seg->flags |= (SEC_MERGE | SEC_STRINGS);
#endif
#else
...
#if defined(TC_PIC30)
    PIC30_SET_INFO_ATTR(ranges_seg); /* FIXME: XC32 does not do this - GM */
#else
