...
lang_output_section_statement_type **pic30_last_os;
#else
...
#if PIC30
static unsigned int opb_shift = 1; /* based on OCTETS_PER_BYTE */
#else
...
extern bfd_boolean pic30_allocation_complete;
extern char * pic30_section_attr_string(asection *);
extern struct pic30_undefsym_table *pic30_undefsym_init (void);
extern void pic30_init_section_list (struct pic30_section **);
extern void pic30_append_section_to_list (struct pic30_section *, lang_input_statement_type *, asection *);
extern int pic30_is_generic_machine (unsigned int);
extern void pic30_load_codeguard_settings (const bfd_arch_info_type *, int);
extern void pic30_create_data_init_templates (void);
extern void pic30_create_rom_usage_template (void);
extern void pic30_create_ram_usage_template (void);
extern void pic30_create_specific_fill_sections (void);
#endif
...
#if PIC30
void pic30_print_list(lang_statement_list_type *, lang_statement_union_type *);
int pic30_list_count (lang_statement_list_type *);
#endif
...
#if PIC30
#define PIC30_PROGRAM_MEMORY_ADDR PIC30_SECTION_IN_PROGRAM_MEMORY
...
void lang_select_startup_file(const char* file, int num) {
  ...
  switch (num)
    ...
    case 0:  /* record file associated with __resetPRI */
      ldlang_add_undef("__resetPRI", FALSE); /* FALSE means not from cmd line */
    ...
    case 1:  /* record file associated with __resetALT */
      ldlang_add_undef("__resetALT", FALSE);
    ...
    case 2:  /* record start mode module */
      ...
      if (pic30_preserve_application_info) {
        ldlang_add_undef("__crt_start_mode", FALSE);
      } else
        ldlang_add_undef("__crt_start_mode_normal", FALSE);
...
init_os (lang_output_section_statement_type *s, flagword flags)
{
...
#if PIC30
  /* Do not create bfd section for this output statement */
  if (pic30_has_floating_aivt) {
    if ((strncmp(s->name,".aivt",5) == 0) && !aivt_enabled)
      return;
  }
  if ((pic30_debug) && (strstr(s->name, "%z") == 0))  /* avoid auto-generated, zero-length sections */
    /* part 1 of message */
    printf("    initializing output section %s, flags = %x, ", s->name, flags);
#endif
  ...
  if (s->section_alignment != NULL)
    s->bfd_section->alignment_power = exp_get_power (s->section_alignment,
						     "section alignment");
#ifdef PIC30
  if ((pic30_debug) && (strstr(s->name, "%z") == 0)) {
    /* part 2 of message */
    char *attr = pic30_section_attr_string(s->bfd_section);
    unsigned int attr_map = pic30_attribute_map(s->bfd_section);
    printf("map = %x, %s\n", attr_map, attr ? attr : "");
    if (attr) free(attr);
  }
#endif
...
{
#if PIC30
  /* section flags may be modified by internal header information. So
    we need to make sure that SEC_CODE is not set for auxflash section */
  if (section->auxflash == 1)
...
#if PIC30
  {
    /* Ensure new output sections have the correct attributes */
    int *temp;
    PIC30_SAVE_FLAGS(section,temp);
    init_os (output, flags);
    PIC30_RESTORE_FLAGS(output->bfd_section,temp);
    free(temp);
  }
#else
    init_os (output, flags);
...
#if PIC30
  if (section->linker_generated)
    output->bfd_section->linker_generated= 1;
  ...
  /* Validate the attributes for this input section */
  {
    ...
    if ((link_info.emitrelocations == 0) && (section->flags & SEC_DEBUGGING))
      { /* skip it */ }
      ...
      printf("\nError: attribute map conflict\n");
      printf("  input section:  flags = %x, map = %x\n",
             section->flags, input_map);
      printf("  output section: flags = %x, map = %x\n",
             output->bfd_section-> flags, output_map);
  ...
  */
  ...
  section->output_section->preserved = section->preserved;
  section->output_section->update = section->update;
  section->output_section->priority = section->priority;
#endif
	  ...
	  while (len < SECTION_NAME_MAP_LENGTH)
	    {
          ...
          if (PIC30_PROGRAM_MEMORY_ADDR (section))
            fprintf(config.map_file, "  0x%06lx %#12lx", section->vma, section->size / opb);
          else
            fprintf(config.map_file, "    0x%04x %#12lx", section->vma, section->size / opb);
#else
...
#ifdef PIC30
    if (!PIC30_PROGRAM_MEMORY_ADDR (osec))
      fprintf(config.map_file, "    0x%04lx  ", value);
    else
      fprintf(config.map_file, "  0x%06lx  ", value);
#else
      minfo ("0x%V", value);
#endif
	  if (is_dot)
...
static void pic30_print_sorted_symbols (struct pic30_symbol *lst, int count)
{
  ...
  for (i=0; i<count; i++) {
    ...
    if (lst[i].format == PIC30_DATA)
      fprintf(config.map_file, "%4s0x%04lx%5s", s, lst[i].value, s);
    else
      fprintf(config.map_file, "%2s0x%06lx%5s", s, lst[i].value, s);
    minfo ("             %pT\n", lst[i].name);
  }
}
...
print_input_section (asection *i, bfd_boolean is_discarded)
{
  ...
  {
    if (i->output_section &&
        PIC30_PROGRAM_MEMORY_ADDR (i->output_section))
      minfo ("  0x%V %W %pB\n",
            addr, size / opb, i->owner);
    else
    {
      fprintf(config.map_file, "    0x%04lx  ", addr);
      minfo (" %W %pB\n", size / opb, i->owner);
    }
  }
#else
...
print_data_statement (lang_data_statement_type *data)
{
  ...
  print_dot = addr + TO_ADDR (size); /* FIXME: 33A adds "/ opb" here */
}
...
print_padding_statement (lang_padding_statement_type *s)
{
...
#if PIC30
  {
    if (PIC30_PROGRAM_MEMORY_ADDR (s->output_section))
      ...
      minfo ("    0x%Y %W", addr, TO_ADDR (s->size) / opb);
  }
#else
  minfo ("0x%V %W ", addr, TO_ADDR (s->size));
#endif
  ...
  print_dot = addr + TO_ADDR (s->size);  /* FIXME: 33A adds "/ opb" here */
}
   ...
   bfd_vma dot)
{
  ...
  if (i->sec_info_type == SEC_INFO_TYPE_JUST_SYMS)
	...
	{
	  //insert_pad (this_ptr, fill, alignment_needed * opb, o, dot); /* FIXME: 33A has this */
      ...
      dot += TO_ADDR (i->size); /* FIXME: 33A adds "/ opb" here */
      if (!(o->flags & SEC_FIXED_SIZE))
	o->size = TO_SIZE (dot - o->vma); /* FIXME: 33A adds "* opb" here */
    }
...
#if PIC30
/* Test attributes for a pair of sections
 * to see if the address overlap test is valid.
 */
static bfd_boolean pic30_overlap_test_valid(asection* s1, asection *s2)
{
  int s1_in_prog_space = PIC30_SECTION_IN_PROGRAM_MEMORY(s1) |
                          PIC30_IS_PACKEDFLASH_ATTR(s1) |
                          PIC30_SECTION_IN_PSV_MEMORY(s1);

  int s2_in_prog_space = PIC30_SECTION_IN_PROGRAM_MEMORY(s2) |
                          PIC30_IS_PACKEDFLASH_ATTR(s2) |
                          PIC30_SECTION_IN_PSV_MEMORY(s2);

  int one_is_external = PIC30_IS_MEMORY_ATTR(s1) |
                         PIC30_IS_MEMORY_ATTR(s2);

  /* Ignore overlap if one is prog space and the other is not */
  if ((s1_in_prog_space && !s2_in_prog_space) ||
      (s2_in_prog_space && !s1_in_prog_space) ||
      one_is_external)
    return FALSE;
  ...
  */
  if (((strncmp(s1->name,".ivt.",5) == 0) ||
      (strncmp(s1->name,".aivt.",6) == 0)) &&
      (s1->linker_generated == 1)) {
       s1->flags = 0;   /* forget this section, something overlaps it */
    return FALSE;
  }

  if (((strncmp(s2->name,".ivt.",5) == 0) ||
      (strncmp(s2->name,".aivt.",6) == 0)) &&
      (s2->linker_generated == 1)) {
       s2->flags = 0;   /* forget this section, something overlaps it */
    return FALSE;
  }

  /* Ignore overlap if both sections are explicitly shared */
  if (PIC30_IS_SHARED_ATTR(s1) && PIC30_IS_SHARED_ATTR(s2) &&
    PIC30_IS_ABSOLUTE_ATTR(s1) && PIC30_IS_ABSOLUTE_ATTR(s2))
    return FALSE;

  /* Ignore overlap if one of the sections is preserved... */
  if (PIC30_IS_PRESERVED_ATTR(s1) || PIC30_IS_PRESERVED_ATTR(s2))
    return FALSE;

  /* If we get here, an overlap test is valid */
  return TRUE;

} /* pic30_overlap_test_valid */
#endif
...
{
  asection *s, *p;
  unsigned opb = bfd_octets_per_byte (link_info.output_bfd);
  ...
  for (p = NULL, i = 0; i < count; i++)
    ...
    {
#if PIC30
      if (pic30_overlap_test_valid(s, p) == FALSE)
        continue;

	      einfo (_("%X%P: Link Error: section %s LMA [%V,%V]"
#else
  ...
  if (!overlays)
      ...
      for (p = NULL, i = 0; i < count; i++)
	{
	  s = sections[i].sec;
#if PIC30
    /* Avoid the VMA overlap test for PSV-type sections */
    if (PIC30_IS_PSV_ATTR(s) || PIC30_IS_AUXPSV_ATTR(s))
      continue;
#endif
	  s_start = s->vma;
	  s_end = s_start + TO_ADDR (s->size) - 1;
		  ...
		  || p_end < p_start))
    {
      ...
      if (pic30_overlap_test_valid(s, p) == FALSE)
	    ...
	    einfo (_("%X%P: Link Error: section %s VMA [%V,%V]"
#else
   ...
   bfd_boolean check_regions)
{
...
#if 0
  printf("\nlang_size_sections_1: os = %lx, dot = %lx\n",
  (bfd_vma) output_section_statement, dot);
#endif

  //if (pic30_allocation_complete)
    //return;
  ...
  for (s = *prev; s != NULL; s = s->header.next)
	...
	case lang_output_section_statement_enum:
	  {
...
#if PIC30
        {
		      ...
		      /* Address of this output section was set by user */
		      os_fixed_address = TRUE;
        }
#else
		      dot += expld.result.section->vma;
#endif
		...
		if (os->addr_tree == NULL)
		  {
			...
			&& expld.phase != lang_mark_phase_enum)
		      {
#if PIC30
      /* Note: This was a warning in binutils v2.14 */
#endif
        ...
        newdot = aivt_base;
        os->bfd_section->user_set_vma = TRUE;
        in_aivt_section = 1;
			} else
#endif
    ...
    if (os_fixed_address)
      os->bfd_section->user_set_vma = TRUE;
    /* Binutils-v2.14 did not require this line - GM 10/13/21 */
    os->addr_tree = exp_intop (newdot);
      ...
      {
		    exp_fold_tree (os->load_base, (asection *) abs_output_section, &newdot);
		    /*xc16-337*/
		    if (!expld.result.valid_p) {
          einfo (_("%P: warning: non constant address "
                  ...
                  os->name, os->name, os->region->name_list.name);
          }
#if 0
		    else {
          /* debug */
          printf("line 6000: setting lma of \'%s\' to %lx\n",
                 os->bfd_section->name, expld.result.value +
            expld.section->lma);
            os->bfd_section->lma = expld.result.value +
            expld.section->lma;
          }
#endif
		    }
      ...
      */
      if (pic30_avoid_absolute_sections) {
        ...
        bfd_vma os_start = bfd_section_lma (link_info.output_bfd,
                                            os->bfd_section);
        bfd_vma os_len   = bfd_section_size (link_info.output_bfd,
                                            os->bfd_section) / opb;
   ...
   | PIC30_IS_STACK_ATTR(s) | PIC30_IS_HEAP_ATTR(s))
        ...
        if (pic30_debug) {
          if (os->region && os->bfd_section &&
              !PIC30_IS_INFO_ATTR(os->bfd_section)) {
            printf("\nSequential section %s in region '%s'"
                   "\n  [section: lma = %lx, length = %lx, user_set_vma = %s]"
                   "\n  [region:  origin = %lx, dot = %lx, length = %lx]\n",
                    os->bfd_section->name, os->region->name_list.name,
                    os->bfd_section->lma, os->bfd_section->size,
                    os->bfd_section->user_set_vma ? "TRUE" : "FALSE",
                    os->region->origin, newdot,
                    os->region->origin + os->region->length);
          }
        }
        if (os->bfd_section && os->bfd_section->size > 0) {
          for (s = unassigned_sections; s != NULL; s = next) {
            ...
            if (s->sec &&
              ...
              s_start = s->sec->lma;
              s_end   = s_start + (s->sec->size / opb) - 1;
              if ((SEC_IN_PROG_SPACE(os->bfd_section) &&
                ...
                if (os->bfd_section->user_set_vma) {
                  if (((strncmp(s->sec->name,".ivt",4) == 0) ||
                    ...
                    s->sec->flags = 0;
                    if (pic30_debug)
                      printf("  --> vector section %s excluded\n", s->sec->name);
                  } else
...
#if 0
		/* debug */
		printf("line 6195, setting lma of \'%s\' to %lx\n",
		        os->bfd_section->name, lma);
#endif
		os->bfd_section->lma = lma;
	      }
...
#if PIC30
      /* Note: Placing this code was difficult; very much
       of the above logic is new
       FIXME: This code might be redundant - GM 6/14/22 */
	    dot = os->bfd_section->lma + os->bfd_section->size / opb;
#else
	    dot = os->bfd_section->vma + os->bfd_section->size / opb;
#endif
    ...
    printf("seq alloc %s, lma = %lx, len = %lx, region = %s, next addr = %lx\n\n",
            ...
            os->bfd_section->lma, os->bfd_section->size / opb,
            (os->region ? os->region->name_list.name : "none"), dot);
#endif
	...
	case lang_reloc_statement_enum:
	  {
	    ...
	    dot += TO_ADDR (size); /* FIXME: 33A adds '/ opb' here */
	    if (!(output_section_statement->bfd_section->flags
		  & SEC_FIXED_SIZE))
	      output_section_statement->bfd_section->size
		= TO_SIZE (dot - output_section_statement->bfd_section->vma);
...
one_lang_size_sections_pass (bfd_boolean *relax, bfd_boolean check_regions)
{
  lang_statement_iteration++;
#if PIC30
  lang_statement_union_type **start_os;

  if (pic30_allocation_complete)
    /* start sizing where the sequential allocator finished */
    start_os = pic30_last_os;
  else
    /* start sizing from the beginning */
    start_os = &statement_list.head;

  lang_size_sections_1 (start_os, abs_output_section,
  ...
  for (; s != NULL; s = s->header.next)
	...
	case lang_data_statement_enum:
	    ...
	    einfo (_("%F%P: invalid data statement\n"));
	  {
	    ...
	    if (size < TO_SIZE ((unsigned) 1)) /* FIXME: 33A is testing opb here */
	      size = TO_SIZE ((unsigned) 1);
	    dot += TO_ADDR (size);
	  }
	  break;
	...
	case lang_reloc_statement_enum:
	  ...
	  dot += TO_ADDR (bfd_get_reloc_size (s->reloc_statement.howto)); /* FIXME: 33A adds '/ opb/ here */
	  break;
	...
	case lang_input_section_enum:
	  {
	    ...
	    if ((in->flags & SEC_EXCLUDE) == 0)
	      dot += TO_ADDR (in->size); /* FIXME: 33A adds '/ opb' here */
	  }
	  break;
	...
	case lang_padding_statement_enum:
	  dot += TO_ADDR (s->padding_statement.size); /* FIXME: 33A adds '/opb ' here */
	  break;
...
/* PIC30 FIXME: The following comment is not accurate.
 * This function creates preliminary definitions of
 * start/stop symbols for all output sections.
 */
  ...
  for (s = link_info.output_bfd->sections; s != NULL; s = s->next)
    {
      const char *secname = s->name;
#ifdef PIC30
      /* Allocate space for all 6 variants */
      char *symbol = (char *) xmalloc (15 + strlen (secname));
      char *c;
      ...
      memcpy (symbol + 1, ".size", 5); /* .sizeof.<secname> */
      lang_define_start_stop (symbol + 1, s);
#ifdef PIC30
      /* Define additional variants */
      sprintf (symbol, ".startof_hi.%s", secname);
      lang_define_start_stop (symbol, s);

      memcpy (symbol + 1, ".size", 5); /* .sizeof_hi.<secname> */
      lang_define_start_stop (symbol + 1, s);

      memcpy (symbol + 2, ".end", 4);  /* .endof_hi.<secname> */
      lang_define_start_stop (symbol + 2, s);

      memcpy (symbol + 5, ".endof", 6); /* .endof.<secname> */
      lang_define_start_stop (symbol + 5, s);
#endif
      free (symbol);
    }
}
...
#ifdef PIC30
/* PIC30 adds .startof_hi., .endof., .endof_hi., .sizeof_hi. variants */
#endif
...
#ifdef PIC30
  {
    if ((h->root.string[1] == 's') && (h->root.string[2] == 't'))
    {
      /* process .startof. and .startof_hi. */
      bfd_vma value = h->u.def.section->vma;

      if (h->root.string[8] == '.')
        h->u.def.value = value & 0xFFFF; /* .startof. */
      else
        h->u.def.value = (value >> 16) & 0xFFFF; /* .startof_hi. */
    }
    else if ((h->root.string[1] == 's') && (h->root.string[2] == 'i'))
    {
      /* process .sizeof. and .sizeof_hi. */
      bfd_vma value = (h->u.def.section->size / opb);

      if (h->root.string[7] == '.')
        h->u.def.value = value & 0xFFFF; /* .sizeof. */
      else
        h->u.def.value = (value >> 16) & 0xFFFF; /* .sizeof_hi. */
    }
    else if (h->root.string[1] == 'e')
    {
      /* process .endof. and .endof_hi. */
      bfd_vma value = h->u.def.section->vma +
        (h->u.def.section->size / opb);

      if (h->root.string[6] == '.')
        h->u.def.value = value & 0xFFFF; /* .endof. */
      else
        h->u.def.value = (value >> 16) & 0xFFFF; /* .endof_hi. */
    }
...
#if PIC30
  /* The following logic belongs here, or in
   * bfd_define_common_symbol(), but the source code
   * for that function has not been found. - GM 9/10/21
   */
#if 0
...
lang_process (void)
{
...
#if PIC30
  /* Delay this step until after best-fit allocation */
#else
...
#if PIC30
  pic30_last_os = (lang_output_section_statement_type **) statement_list.tail;
#endif
  ...
  /* Binutils-v2.32 wants to size all output statements
   * every time lang_size_sections() is called. But at this
   * point only newly added statements should be sized.
   *
   * 'pic30_last_os' is used to keep track of where the
   * sequential allocator finished, before any new statements
   * were added by ldemul_after_allocation().
   */
  pic30_avoid_absolute_sections = FALSE;
  if (pic30_last_os != (lang_output_section_statement_type **) statement_list.tail)
      lang_size_sections (NULL, TRUE);
#endif
