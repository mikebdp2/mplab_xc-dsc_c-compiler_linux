...
void execute(char *cmd, char **argv) {
  int rc;
  int sys_result ATTRIBUTE_UNUSED;
...
#if PIC30
  link_info.init_function = "__reset";
#else
...
static void
bfd_pic30_report_sections(bfd *abfd, asection *sect,
		...
		bfd_vma address)
{
  ...
  if (entry)
	...
	case bfd_link_hash_undefweak:
#if PIC30
	  einfo (_(" Link Error: relocation truncated to fit: "
#else
...
#if PIC30
    einfo (_(" Link Error: relocation truncated to fit: %s against `%pT'"),
#else
