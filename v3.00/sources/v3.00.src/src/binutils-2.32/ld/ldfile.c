...
#if PIC30
	/* bin30-225, XC16-37 */
        /* Check for arch info availability and stop linking
           if it is not available even before calling
           bfd_arch_get_compatible() */
          if (check->arch_info == NULL)
            {
              einfo (_("%P: architecture information for `%s'"
                       " is not available\n"), check->filename);
...
#ifdef PIC30
#if 0
        /* In binutils-v2.32, entry (lang_input_statement_type)
         * no longer has a field called 'is_archive'. */
	      if(entry->is_archive == 1)
	        return TRUE;
#endif
        /* Test to see if entry is an archive */
        size_t lenstr = strlen(attempt);
        if (strncmp(attempt + lenstr - 2, ".a", 2) == 0)
          return TRUE;
#endif
	      if (command_line.warn_search_mismatch)
		einfo (_("%P: skipping incompatible %s "
...
#endif
#ifdef PIC30
	  found = ldfile_open_file_search (arch->name, entry, "lib", "-elf.a");
	  if (found)
	    break;
#endif
