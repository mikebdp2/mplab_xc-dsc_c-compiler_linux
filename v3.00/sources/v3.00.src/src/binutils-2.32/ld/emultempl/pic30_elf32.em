...
extern void pic30_add_section_attributes (asection *,
                                          char *, char *, char *);
extern void pic30_get_aivt_settings(const bfd_arch_info_type *, int);
extern void pic30_get_ivt(const bfd_arch_info_type *, int);
...
static void build_free_block_list
  (struct memory_region_struct *, unsigned int, unsigned int);
...
static void bfd_pic30_redefine_global_symbol
  (const char *const , asection *, bfd_vma);
...
static lang_output_section_statement_type *
 gld${EMULATION_NAME}_place_orphan( asection *, const char *, int);
...
static struct pic30_memory *shared_data_memory_blocks;
...
void pic30_init_section_list
  (struct pic30_section **);

void pic30_append_section_to_list
  (struct pic30_section *, lang_input_statement_type *, asection *);

static bfd_boolean pic30_name_in_section_list
  (struct pic30_section *, const char *);

static void pic30_remove_from_section_list
  (struct pic30_section *, struct pic30_section *);
...
static struct pic30_memory * select_free_block
  (struct pic30_section *, unsigned int, unsigned int,
          struct memory_region_struct *, int more);
...
static void create_remainder_blocks
  (struct pic30_memory *, struct pic30_memory *, unsigned int,
...
program_base_address(void) {
...
#else 
  static bfd_vma base_addr;
  static int base_address_found = 0;
  struct memory_region_struct *region;

  if (base_address_found) return base_addr;
  ...
  for (; region; region = region->next) {
    if ((region->flags & (SEC_CODE|SEC_READONLY))==(SEC_CODE|SEC_READONLY)) {
      ...
      if (region->origin < base_addr) base_addr = region->origin;
    }
  }
  return base_addr;
#endif
...
program_end_address(void) {
...
#else
  static bfd_vma end_addr;
  static int end_address_found = 0;
  struct memory_region_struct *region;

  if (end_address_found) return end_addr;
  ...
  for (; region; region = region->next) {
    if ((region->flags & (SEC_CODE|SEC_READONLY))==(SEC_CODE|SEC_READONLY)) {
      /* an executable region */
      end_address_found = 1;
      if (region->origin + region->length - 2 > end_addr)
        end_addr = region->origin + region->length - 2;
    }
  }
  return end_addr;
#endif
...
data_end_address(void) {
  ...
  for (; region; region = region->next) {
    if ((region->flags & SEC_ALLOC) &&
      ...
      /* a data writable region */
      end_address_found = 1;
      if (region->origin + region->length - 1 > end_addr) 
        end_addr = region->origin + region->length - 1;
    }
  }
  return end_addr;
#endif
...
handle_hash_init ()
{
  ...
  table = ((struct handle_hash_table *)
	   bfd_alloc (link_info.output_bfd, sizeof (struct handle_hash_table)));
  if (table == NULL)
    return NULL;

  if (! bfd_hash_table_init_n (&table->table, handle_hash_newfunc,
                               sizeof(struct handle_hash_entry), HANDLE_INIT))
    {
      free (table);
      return NULL;
    }
...
/*
** Search a section list by name
**  and return a section record or NULL
*/
struct pic30_section *
pic30_section_in_section_list (lst, name)
     struct pic30_section *lst;
     const char *name;
{
  struct pic30_section *s, *next;
      ...
      const char *name;
{
  ...
  printf("  (zero-length sections are omitted)\n");
  prev = lst;
  for (s = prev; s != NULL; s = next)
    {
      next = s->next;
     ...
     asymbol **syms;
{
  ...
  if (reloc_size == 0)
  ...
  else
    {
      ...
      relpp = (arelent **) malloc (reloc_size);
      if (!relpp)
        {
     ...
     flagword flags;
{
  ...
  name = (char *) bfd_alloc (link_info.output_bfd, (strlen (sym) + 1));
  if (!name)
    {
...
*/
static asection *
bfd_pic30_create_section (abfd, name, flags, alignment)
     bfd *abfd;
     const char *name;
     int flags;
     int alignment;
{
  ...
  bfd_set_section_alignment (abfd, sec, alignment);
...
*/
static void
bfd_pic30_create_reloc (abfd, addr, which_howto, symidx)
     bfd *abfd;
     int addr;
     int which_howto;
     int symidx;
...
bfd_pic30_save_relocs (asection *sec)
{
  ...
  sec->orelocation = (arelent **) bfd_alloc (link_info.output_bfd,
     ...
     PTR p;
{
  ...
  bfd_pic30_create_symbol (hinfo->abfd,
                           handle->sym,
                           /* hinfo->sec, BSF_GLOBAL, 0); */
                           bfd_und_section_ptr, handle->flags, 0);

  /* create two relocs for the jump target */
  bfd_pic30_create_reloc (hinfo->abfd,
                          handle->index * 8,
...
asymbol **bfd_pic30_load_symbol_table(bfd *abfd,long *num) {
  ...
  tbl = bfd_alloc(link_info.output_bfd, sizeof(struct symtbl_list));
  if (!tbl) {
  ...
  tbl->symbols = (asymbol **) bfd_alloc(link_info.output_bfd, size); /* do not free */
  if (!tbl->symbols) {
...
bfd_pic30_dump_reloc_table(bfd *abfd, asection *sec)
{
  ...
  /* FIXME - CAW */
  rels = bfd_pic30_load_relocs(abfd, sec, 0); /* sets relcount too */
     ...
     bfd *parent;
{
  ...
  struct traverse_hash_info *tinfo = &hinfo;
  ...
  */
  symptr = 0;
  symtab = (asymbol **) bfd_alloc (link_info.output_bfd,
  ...
  */
  ...
  alignment = 1; /* 2^1 */
  sec = bfd_pic30_create_section (abfd, ".handle", flags, alignment);
  ...
  */
  handle_data = (unsigned char *) bfd_alloc (link_info.output_bfd, size);
  if (!handle_data)
  {
     ...
     char *section_name;
{
  ...
  */
  ...
  alignment = 1; /* 2^1 */
  sec = bfd_pic30_create_section (abfd, section_name, flags, alignment);
     ...
     bfd *parent ATTRIBUTE_UNUSED;
{
  ...
  */
  flags = SEC_CODE | SEC_HAS_CONTENTS | SEC_IN_MEMORY | SEC_KEEP;
  alignment = 1; /* 2^1 */
  sec = bfd_pic30_create_section (abfd, ".rom_usage", flags, alignment);
     ...
     bfd *parent ATTRIBUTE_UNUSED;
{
  ...
  */
  flags = SEC_CODE | SEC_HAS_CONTENTS | SEC_IN_MEMORY | SEC_KEEP;
  alignment = 1; /* 2^1 */
  sec = bfd_pic30_create_section (abfd, ".ram_usage", flags, alignment);
  sec->linker_generated = 1;
  size = 0; /* will update later */
  bfd_set_section_size (abfd, sec, size);
     ...
     bfd *parent;
{
  ...
  */
  flags = SEC_CODE | SEC_HAS_CONTENTS | SEC_IN_MEMORY;
  alignment = 1; /* 2^1 */
  sec = bfd_pic30_create_section (abfd, ".isr", flags, alignment);
  size = (bfd_size_type) 8;
  bfd_set_section_size (abfd, sec, size);

  /* allocate memory */
  isr_data = (unsigned char *) bfd_alloc (link_info.output_bfd, size);
  if (!isr_data)
    {
...
bfd_pic30_create_user_init_bfd  (bfd *parent)
{
  ...
  /* allocate memory */
  user_init_data = (unsigned char *) bfd_alloc (link_info.output_bfd, size);
...
bfd_pic30_create_config_word_bfd  (char *name, unsigned int val)
{
  ...
  /* create a bare-bones bfd */
  oname = (char *) bfd_alloc (link_info.output_bfd, strlen(name) + 5);
  sprintf (oname, "/CW/_%s", name);
  abfd = bfd_create (oname, link_info.output_bfd);
  bfd_find_target ("coff-pic30", abfd);
  bfd_make_writable (abfd);
  ...
  /* create a bare-bones section */
  sec_name = (char *) bfd_alloc (link_info.output_bfd, strlen(name) + 6 + 1);
  ...
  /* allocate memory */
  sec_data = (char *) bfd_alloc (link_info.output_bfd, size);
  if (!sec_data) abort();
...
bfd_pic30_create_heap_bfd (bfd *parent) {
  ...
  if (pic30_is_isav32_machine(global_PROCESSOR))
      ...
      scale_factor = 2; /* classic devices have phantoms in data memory */
  ...
  flags = SEC_ALLOC;
  alignment = 1;
  sec = bfd_pic30_create_section (abfd, ".heap", flags, alignment);
  size = pic30_heap_size * scale_factor; /* adjust for phantom bytes */
  bfd_set_section_size (abfd, sec, size);
...
bfd_pic30_create_slave_id_bfd  (char *name, unsigned int val)
{
  ...
  if (pic30_slave_id_location) {
    ...
    einfo (_("%F%B: could not determine slave id location;\n\tconsider using --mslave-id-location option\n"));
  }
  size = (bfd_size_type) 4;
  bfd_set_section_size (abfd, sec, size);
...
#define ACCESS_ENTRY_ISR_SLOT 16
  ...
  tab = (unsigned char *) bfd_alloc (link_info.output_bfd, size);
  if (!tab)
    {
     ...
     unsigned int *max_priority;
{
  int *data_size = (int *) p;
  int scale_factor = 2;
  int data_record_header_size = 12;
      ...
      */
      ...
      if (pic30_debug)
        printf("  %s (skipped), size = %x bytes\n",
               sect->name, (unsigned int) sect->size / scale_factor);
      return;
  }
  ...
  */
  if (PIC30_SECTION_IS_BSS_TYPE(sect) && (sect->size > 0))
  {
  ...
  */
  if (PIC30_IS_DATA_ATTR(sect) && (sect->size > 0))
  {
      ...
      if (pic30_bytes_per_insn(0) == 4) {
        /* always 4 bytes per insn word */
        delta = data_record_header_size + (((count + 3) / 4) * 4);
      } else {
        count = count / 2;    /* ignore phantom in data image */
        delta = data_record_header_size +
      ...
      */
      ...
      if (pic30_debug)
        printf("  %s (data), size = %x bytes, template += %x bytes\n",
               sect->name, count, delta);
  }

  /*
  **
  */
  if (PIC30_IS_CODE_ATTR(sect) &&(sect->size > 0) && (sect->priority > 0)){
    /* this will get an entry */
    *data_size += data_record_header_size;
    if (sect->priority > *max_priority) *max_priority = sect->priority;

    if (pic30_debug)
      printf("  %s (call), template += %x bytes\n",
             sect->name, data_record_header_size);
  }
...

{
  ...
  if (PIC30_IS_INFO_ATTR(sect)) {
    info = "(info)";
  }
  else if (sect->user_set_vma) {
    info = "(fixed)";
  }
  ...
  printf("  %16s %8x %6x %6x %6x %s\n", sect->name, sect->flags,
         ...
         (unsigned int) sect->size / bfd_octets_per_byte (abfd),
         info);
  return;
}
     ...
     bfd *target;
{
    ...
    bfd_link_hash_traverse (link_info.hash,
			    bfd_pic30_undefine_one_symbol_bfd, &hinfo);
...
get_aivt_base ()
{
        ...
        if (PIC30_IS_ABSOLUTE_ATTR(sec))
        {
              ...
              (aivtloc_ptr < sec->vma + len/opb))
          {
            ...
            else
            {
              ...
              if (pic30_debug) {
                printf("\nAIVT base address: 0x%4.4lx\n", aivt_base);
                printf("AIVT len:          0x%4.4lx\n", aivt_len);
    ...
    /* FIXME: This code forces the value of pic30_has_user_boot,
       which is supposed to represent a command-line option.

       It adjusts the boot segment after recognizing some
       alignment of the aivt table, and hi-jacks the
       command option for that purpose. GM 5/53/23 */
...
bfd_pic30_after_parse()
{
  ...
  for (; region; region = region->next) {
    ...
    if (pic30_debug) {
      printf("  '%s' ", region->name_list.name);
...
static bfd_boolean
find_section(bfd *abfd, const char *name) {
  asection *sec;
 ...
 * flags cleared. Other operations on section flags
 * are performed, such as the application of attributes
 * specified with --add-flags-code, --add-flags-const
 * linker options.
 *
 ...
 */
static bfd_boolean
bfd_pic30_process_bfd_after_open (abfd, linfo)
     bfd *abfd;
     struct bfd_link_info *linfo;
{
  ...
  */
  {
    ...
    for (i=0; i<num; i++) {
      ...
      /* look for a reference to PSVPAG or DSRPAG not in .init section */
        if ((strcmp(sym_name,"_PSVPAG") == 0) ||
            (strcmp(sym_name,".const") == 0) ||
            (strcmp(sym_name,"_DSWPAG") == 0) ||
            (strcmp(sym_name,"_DSRPAG") == 0)) {
        /* slightly better than looking at the file name; however, we should
         * consider using a meaningful identifier to say that this is the
         * CRT */
        if (find_section(abfd,".init") == FALSE) {
  ...
  */
  for (sec = abfd->sections; sec != 0; sec = sec->next)
  {
    ...
    int is_isav32 = pic30_is_isav32_machine(global_PROCESSOR);
    int scale_factor = is_isav32 ? 1 : 2;
#if 0
    if (pic30_debug) {
      char *attr = pic30_section_attr_string(sec);
      printf("  input section %s, len = 0x%lx, %s\n",
             sec->name, sec->size, attr ? attr : "");
      if (attr) free(attr);
    }
#endif
    ...
    if (PIC30_IS_PSV_ATTR(sec) && (sec->size > 0))
      has_psv_section = TRUE;

    /* if section has HEAP attribute, record that fact */
    if (PIC30_IS_HEAP_ATTR(sec) && (sec->size > 0)) {
      ...
      if (h == NULL) {
        if (pic30_debug)
          printf("  ..creating global symbol for user-defined memory %s\n",
                 region_name);
        _bfd_generic_link_add_one_symbol (linfo, abfd,
    ...
    /* this code was moved from bfd/elf.c */
    if (pic30_add_data_flags || pic30_add_code_flags) {
      pic30_add_section_attributes(sec,
    ...
    /* report if flags were changed */
     {
      ...
      if ((pic30_debug) && ( new_attr != old_attr)) {
        char *attr = pic30_section_attr_string(sec);
        printf("    %s attributes changed from %#x to %#x %s\n",
               sec->name, old_attr, new_attr, attr ? attr : "");
        if (attr) free(attr);
      }
    }
    ...
    rels = bfd_pic30_load_relocs(abfd, sec, symbols); /* sets relcount */
    if (!rels)
      continue;
	...
	asymbol *psym;
            ...
            (q->howto->type == R_PIC30_HANDLE))
        {
	      ...
	      break;
            ...
            if (pic30_debug)
	    {
              printf("  Handle symbol: %s %x %lx %x\n",
                     psym->name,
                     psym->flags,
                     (unsigned long int) psym->section,
            ...
            if ((psym->flags & BSF_LOCAL) == BSF_LOCAL) 
            {
                ...
                char * global_name;
                global_name = (char *)
		   bfd_alloc(link_info.output_bfd, (strlen(abfd->filename) +
                                          strlen(sym_name) + 2));
                if (!global_name)
                {
                    fprintf( stderr, 
                ...
                if (!handle)
                {
                    ...
                    _bfd_generic_link_add_one_symbol (linfo, abfd, global_name,
...
bfd_pic30_adjust_psv_sections(void)
{
  bfd_vma next_page;
  unsigned int opb = bfd_octets_per_byte (link_info.output_bfd);
  struct lma_adjust_info lma_info;
  ...
  */
  for (s = pic30_section_list; s != NULL; s = s->next)
  {
        ...
        (s->sec->size > 0))
    {
        ...
        **
        ** Note: sec->size includes phantom bytes,
        **       divide by opb to get actual length
        */
        actual_size = s->sec->size / opb;
  ...
  */
  ...
  #define DEFAULT_PROG_LIMIT 0x80000
          ...
          if (s->sec)
            {
              printf("  section %s %lx %lx %x %s\n",
                     s->sec->name, s->sec->lma,
                     s->sec->size/opb, s->sec->alignment_power,
                     s->sec->user_set_vma ? "(fixed)" : "");
            }
        printf("\nCODE sections:\n");
    ...
    */
    ...
    pic30_add_to_memory_list (free_blocks, lma_info.base, (lma_info.limit - lma_info.base));
    ...
    */
    for (s = psv_sections; s != NULL; s = s->next)
    {
      ...
      {
          b = pic30_static_assign_memory (free_blocks, s->sec->size/opb,
                                          s->sec->lma);
          if (!b)
  	{
  	      ...
  	      (command_line.check_section_addresses == FALSE))
  	  {
          ...
          else
          {
              ...
              if ((b->offset + s->sec->size/opb) < b->size)
                {
                  bfd_vma new_addr = b->addr + b->offset + s->sec->size/opb;
                  pic30_add_to_memory_list (free_blocks, new_addr,
                                         b->size - b->offset - s->sec->size/opb);
                }
              /*
    ...
    */
    for (s = psv_sections; s != NULL; s = s->next)
    {
      ...
      {
          b = pic30_best_fit_memory (free_blocks, s->sec->size/opb,
  ...
  if (reallocate_auxflash) {
    auxflash = lang_memory_region_lookup ("auxflash", FALSE);
  #define DEFAULT_AUXFLASH_LIMIT 0x7FFFFA
    if (auxflash->length == ~(bfd_size_type) 0)
    {
        /* if no auxflash region specified */
        lma_info.limit = DEFAULT_AUXFLASH_LIMIT;
        einfo(_("%P: Warning: auxflash memory region not specified."
                " Using default upper limit of 0x%v\n"), lma_info.limit);
    }
    else
      lma_info.limit = auxflash->origin + auxflash->length;
  
    /*
    ** build a list of AUXPSV sections above the threshold
    */
    pic30_init_section_list (&auxpsv_sections);
    bfd_map_over_sections (link_info.output_bfd, &pic30_build_auxpsv_section_list, &lma_info);
  
    /*
    ** build a list of AUXFLASH sections above the threshold
    */
    pic30_init_section_list (&auxflash_sections);
    bfd_map_over_sections (link_info.output_bfd, &pic30_build_auxflash_section_list, &lma_info);
    ...
    ** this frees both A & B lists
    */
    pic30_free_section_list (&auxpsv_sections);
    pic30_free_memory_list  (&free_blocks);
 }
} /* static void bfd_pic30_adjust_psv_sections()*/
     ...
     PTR p ATTRIBUTE_UNUSED;
{
  ...
  if ((PIC30_IS_CODE_ATTR(sect) || PIC30_IS_PSV_ATTR(sect)) &&
      sect->size > 0)
  {
    bfd_vma end = sect->lma + (sect->size / opb) - PROGRAM_WORD_SIZE;
...
bfd_pic30_finish(void)
{
  ...
  */
  pic30_strip_sections(link_info.output_bfd);

  if (pic30_application_id) {
    alias_name = xmalloc(1+ strlen(application_id) + 80);
  }
  ...
  */
  for (psname = pic30_codesecs; psname[0]; ++psname)
        /* not sure why we exclude info sections */
  {
    sec = bfd_get_section_by_name(link_info.output_bfd, psname[0]);
    if (sec)
    {
      sec->flags |= SEC_CODE;
      sec->flags &= ~SEC_DATA;
      bfd_set_section_alignment(link_info.output_bfd, sec, 1);
    }
  }
  ...
  sec = bfd_get_section_by_name(link_info.output_bfd, ".const");
  if (sec)
  {
      has_const_section = TRUE;
      const_psvpage = (sec->lma >> 15) | FLASH_mask;
      const_length  = sec->size /opb ;
  }
  ...
  sec = bfd_get_section_by_name(link_info.output_bfd, ".boot_const");
  if (sec) {
    boot_const_psvpage = (sec->lma >> 15) | FLASH_mask;
    pic30_has_managed_psv = TRUE;
  }

  sec = bfd_get_section_by_name(link_info.output_bfd, ".secure_const");
  if (sec) {
  ...
  sec = bfd_get_section_by_name(link_info.output_bfd, _COMMENT);
  if (sec)
  ...
  */
  ...
  if (stack_section_defined) {
    ...
    if (sec == 0)
      ...
      if (pic30_has_phantom_byte(global_PROCESSOR)) divisor = 2;
      if (pic30_debug)
      ...
      stack_limit = stack_base +
                   (((sec->size / divisor) - pic30_stackguard_size + 1) & ~1);
    }
  }
  ...
  if (!stack_section_defined && !user_defined_stack) {
    if (!pic30_coresident_app) {
      if (pic30_debug)
        printf("\nCreating stack from remaining data memory...\n");
      allocate_default_stack();
    }
    else einfo("%F%X Error: When linking with --coresident"
               " a user defined stack must be created\n");
  }
  ...
  */
  if (!heap_section_defined) {
    ...
    if (heap_section == 0)
      ...
      heap_limit = heap_base + heap_section->size / divisor;
    }
  }
...
#define link_add_one_symbol(link_info, output_bfd, name, \
  ...
  link_add_one_symbol (&link_info, link_info.output_bfd, "__heap",
                                    BSF_GLOBAL, bfd_abs_section_ptr,
                                    heap_base, "__heap", 1, 0, 0);
  if (pic30_debug)
    printf("Creating __eheap = %x\n", heap_limit);
  link_add_one_symbol (&link_info, link_info.output_bfd, "__eheap",
                                    BSF_GLOBAL, bfd_abs_section_ptr,
  ...
  */
  if (!user_defined_stack)
    {
      ...
      link_add_one_symbol (&link_info, link_info.output_bfd, "__SP_init",
                                        BSF_GLOBAL, bfd_abs_section_ptr,
                                        stack_base, "__SP_init", 1, 0, 0);
      if (pic30_debug)
        printf("Creating __SPLIM_init = %x\n", stack_limit);
      link_add_one_symbol (&link_info, link_info.output_bfd, "__SPLIM_init",
                                        BSF_GLOBAL, bfd_abs_section_ptr,
  ...
  link_add_one_symbol (&link_info, link_info.output_bfd, "__MIN_STACK_SIZE",
                                    BSF_GLOBAL, bfd_abs_section_ptr,
                                    pic30_stack_size, "__MIN_STACK_SIZE", 1, 0, 0);

  if (pic30_debug)
    printf("Creating __STACKGUARD_SIZE = %x\n", pic30_stackguard_size);
  link_add_one_symbol (&link_info, link_info.output_bfd, "__STACKGUARD_SIZE",
                                    BSF_GLOBAL, bfd_abs_section_ptr,
  ...
  */
  ...
  if (pic30_data_init)
    { 
      ...
      /* find the shared  template's output section */
      ...
      /* find the template's output section */
      ...
      if (sec)
        {
          ...
          unsigned int tblpage,tbloffset;
      ...
      if (sec)
        {  bfd_vma dinit_addr = sec->lma +
           ...
           if (pic30_debug)
             printf("Creating __restart_shared_dinit_addr = 0x%lx\n", dinit_addr);
           link_add_one_symbol (&link_info, link_info.output_bfd,
  ...
  */
  h = bfd_link_hash_lookup (link_info.hash, "__PROGRAM_END", FALSE, FALSE, TRUE);
  if (h) {
  upper_limit_program = program_end_address();

  bfd_map_over_sections(link_info.output_bfd, &record_last_program_address, NULL);
  if (pic30_debug)
  ...
  bfd_map_over_sections(link_info.output_bfd, &bfd_pic30_clean_section_names, 0);

  asection *s;

  for (s = link_info.output_bfd->sections; s != NULL; s= s->next) {
    if ((strcmp(s->name, ".shared.dinit") == 0) &&
        ((s->flags & SEC_EXCLUDE) == 0))
      pic30_append_section_to_list(shared_dinit_sections, 0, s);
  } 
#if 0
  /* change output section_names to have application id */
  if (pic30_application_id) {
    for (s = link_info.output_bfd->sections; s != NULL; s= s->next) {
      if (s->linked == 0) {
      char *alias_name;
      alias_name = xmalloc( strlen(s->name) + strlen(application_id) + 2);
      if (s->name[0] == '.')
        (void) sprintf(alias_name, "%s%s", application_id, s->name);
      else
        (void) sprintf(alias_name, "%s.%s", application_id, s->name);
      s->name = alias_name;
      }
...
#endif
  /* Create linked section symbol after changing output section names */
  for (s = link_info.output_bfd->sections; s != NULL; s= s->next) {
#if 0
     if (!PIC30_IS_INFO_ATTR(s)) 
        /* not sure why we exclude info sections */
#else
     if (1) 
#endif
     {
       if (s->linked == 0) {
         //s->linked = 1;  /* Don't set it in the bfd now it will be set */
                           /* in subsequent links using the symbol */
         char *name;
         ...
         (void) sprintf(name, "%s%s", linked_prefix, s->name);
          ...
          link_add_one_symbol (&link_info, link_info.output_bfd,
                                            ...
                                            bfd_abs_section_ptr, 1,
                                            name, 1, 0, 0);
         if ((PIC30_IS_DATA_ATTR(s) || PIC30_IS_BSS_ATTR(s)) &&
              !PIC30_IS_SHARED_ATTR(s))
           continue;
         else
            PIC30_SET_ABSOLUTE_ATTR(s);
       }
     }
  }
} /* static void bfd_pic30_finish () */
...
static int
is_valid_config_word_section(bfd_vma addr)
{
  fuse_setting_type *f;
  codeguard_setting_type *s;
  for (f = fuse_settings; f != NULL; f = f->next) {
     if (f->name && (f->address == addr)) 
...
gld${EMULATION_NAME}_set_output_arch()
{
  ...
  /* C30-specific */
  pic30_output_bfd = link_info.output_bfd;  /* save a copy for bfd to see */
...
static bfd_boolean
pic30_smartio_fn_list(struct bfd_link_hash_entry *h, PTR p ATTRIBUTE_UNUSED) {
  ...
  if (strncmp(pic30_smartio_symbol, h->root.string,
    ...
    for (c = (char *) &h->root.string[strlen(pic30_smartio_symbol)]; *c; c++) {
      if (*c == '_') continue;
      if (*c == '0') bit = 0; 
      else {
        bit = *c - 'A' + 1;
...
smartio_symbols(struct bfd_link_info *info) {
  ...
  while ( function_pairs[i].full_set )
    {  struct smartio_sym_list *l;
       ...
       if (smartio_fn_list) {
         ...
         char *buffer_map_to = xmalloc(strlen(function_pairs[i].prefix) +
                                       strlen(suffix) + 1);
         if (buffer_map_to)
         ...
         if (undefs) {
           ...
           if (bfd_hash_lookup( (struct bfd_hash_table *) info->hash, buffer_map_to, 0, 0) == 0) {
             ...
             new = (struct bfd_link_hash_entry *) bfd_hash_lookup( (struct bfd_hash_table *) info->hash, buffer_map_to, 1, 1);
             new->u.undef.next = newundefs;
  ...
  if (newundefs) {
    if (info->hash->undefs_tail)
      info->hash->undefs_tail->u.undef.next = newundefs;
     ...
     bfd *parent ATTRIBUTE_UNUSED;
{
  ...
  */
  ...
  flags = SEC_CODE | SEC_HAS_CONTENTS | SEC_IN_MEMORY;
  alignment = 1; /* 2^1 */
  sec = bfd_pic30_create_section (abfd, sname, flags, alignment);
  size = 0; /* will update later */
  bfd_set_section_size (abfd, sec, size);
                      ...
                      (struct pic30_fill_option *opt)
{
  ...
  if (pic30_is_isav32_machine(global_PROCESSOR))
    scale_factor = 1; /* ie, no phantom bytes */
  ...
  for (b = program_memory_free_blocks; b != NULL; b = next)
    {
     ...
     if ((b->addr == 0) && (b->size == 0))
      continue;
     fill_bfd = bfd_pic30_create_fill_bfd(link_info.output_bfd);
     bfd_pic30_add_bfd_to_link (fill_bfd, fill_bfd->filename);
     size = b->size * scale_factor;
     ...
     /* FIXME: 33A had this line : */
     /* pic30_append_section_to_list(opt->fill_section_list, s->file, sec); */
     pic30_append_section_to_list(opt->fill_section_list, NULL, sec);
...
void pic30_pad_flash() {
  ...
  else einfo(_("%P%F: Error: Flash padding size is not available.\n" ));
    ...
    for (b = program_memory_free_blocks; b != NULL; b = next)
      {
        ...
        if (len) {     
          size = len * 2;
          pad_bfd = bfd_pic30_create_pad_bfd(link_info.output_bfd);
  ...
  */
  if ((pic30_data_init) && 
    ...
    { 
      ...
      ait->max_priority = 0;
      ait->abfd = bfd_pic30_create_data_init_bfd (link_info.output_bfd,
                                                  ait->output_name, 
                                                  ait->section_name);
      ...
      if (pic30_debug)
        {
          printf("  null terminator, template += 4 bytes\n");
          printf("\nTotal initialized data %s: %x bytes\n\n",
                 pic30_pack_data ? "(packed)" : "(not packed)",
                 total_data);
        }

      /* allocate memory for the template */
      ait->init_data = (unsigned char *) bfd_alloc (link_info.output_bfd, total_data + 4);
      if (!ait->init_data)
        {
          fprintf( stderr, "Link Error: not enough memory for data template\n");
          abort();
      ...
      memset(ait->init_data+total_data, 0x21, 4);
...
void pic30_create_rom_usage_template(void) {
  ...
  rom_usage_bfd = bfd_pic30_create_rom_usage_bfd (link_info.output_bfd);
  ...
  LANG_FOR_EACH_INPUT_STATEMENT (f)
    {
      for (sec = f->the_bfd->sections;
           sec != (asection *) NULL;
           sec = sec->next) {
        if ((sec->size > 0) && ((sec->flags & SEC_EXCLUDE) == 0) &&
...
void pic30_create_ram_usage_template(void) {
  ...
  ram_usage_bfd = bfd_pic30_create_ram_usage_bfd (link_info.output_bfd);
...
gld${EMULATION_NAME}_after_open()
{
  ...
  */
  ...
  /*
  ** create sections for the rest of the ivts if we need to
  */
  if  (pic30_ivt) {
    for (r = ivt_records_list; r != NULL; r = next) {
      next = r->next;
      ...
      if ((r->flags & ivt_seen) == 0) {
        ...
        ** handler entry
        **
        ** Dual-panel devices (ex: 33CK512MP606) define two vectors
        ** for each slot, so if this section exists, just move on. */
        if (bfd_get_section_by_name(link_info.output_bfd, sec_name)) {
           continue;
        }
        r->flags |= ivt_default;

        ivt_sec = bfd_make_section(link_info.output_bfd, sec_name);
        if (!ivt_sec) {
          if (pic30_debug)
            printf("  _after_open(): ivt section %s is already defined\n",
                   sec_name);
          continue;
        }
        if (pic30_debug)
          printf("  _after_open(): creating ivt section %s\n", sec_name);
        ...
        if (r->is_alternate_vector) {
          ...
          bfd_set_section_vma(NULL, ivt_sec, aivt_base + r->offset);
        } else {
          if (ivt_base == 0) {
            einfo(_("%P%F: Link Error: can't locate vector "
                           "table base address.\n" ));
          }
          bfd_set_section_vma(NULL, ivt_sec, ivt_base + r->offset);
        }
      }
      ...
      if (r->is_alternate_vector) {
        if (r->ivt_sec) {
          if (aivt_base) {
            ...
            bfd_set_section_vma(NULL, r->ivt_sec, aivt_base + r->offset);
          } else {
            r->ivt_sec->flags = 0;
          }
        }
  ...
  */
  if (pic30_has_icd2_option)
    _bfd_generic_link_add_one_symbol (&link_info, link_info.output_bfd, icd2ram,
                                      BSF_GLOBAL, bfd_abs_section_ptr,
                                      1, icd2ram, 1, 0, 0);
  ...
  */
  {
    unsigned int val;
    bfd *abfd;
    char *p,*names = pic30_unique_selected_configword_names();
  ...
  if (handle_relocs)
    {
      ...
      handle_bfd = bfd_pic30_create_jump_table_bfd (link_info.output_bfd);
  ...
  if (has_boot_access || has_secure_access)
    {
      ...
      if (has_boot_access) {
        abfd = bfd_pic30_create_branch_table_bfd(".boot",
                                                 ...
                                                 boot_access, link_info.output_bfd,
      ...
      if (has_secure_access) {
        abfd = bfd_pic30_create_branch_table_bfd(".secure",
                                                 ...
                                                 secure_access, link_info.output_bfd,
    ...
    LANG_FOR_EACH_INPUT_STATEMENT (f)
   {
     for (sec = f->the_bfd->sections;
        ...
        if ((sec->size > 0) &&
          ...
          unsigned char *buf = malloc(sec->size);
          unsigned char *packed_data, *temp, *dat;
          ...
          /* FIXME: Does this need adjustment for 33A? */
          bfd_size_type original_size = sec->size;
          int size = sec->size / 2;
          sec->size = ((size+2) / 3) * 4;
          packed_data = (unsigned char *) malloc (sec->size);
          if (!packed_data)
           {
          ...
          /* FIXME: Does this need adjustment for 33A? */
          { int count = 0;
            for (dat = buf; dat < (buf + original_size); ) {
              *temp++ = *dat++; dat++; /* copy data bytes,
                                          * avoid phantom */
              count++;
              if (count == 3) {
    ...
    if ((h = bfd_pic30_is_defined_global_symbol("__reset"))) 
      {
        if (h->u.def.section != bfd_abs_section_ptr) {
          ...
          if (pic30_startup0_file &&
              strcmp(pic30_startup0_file, h->u.def.section->owner->filename))
  ...
  */
  {
    ...
    if ((h = bfd_pic30_is_defined_global_symbol("__DefaultInterrupt")))
    ...
    else if (pic30_isr)
      {
        ...
        isr_bfd = bfd_pic30_create_default_isr_bfd (link_info.output_bfd);
  ...
  */
  if (has_user_init_functions) {
    ...
    user_init_bfd = bfd_pic30_create_user_init_bfd (link_info.output_bfd);
  ...
  */
  if (!heap_section_defined && (pic30_heap_size > 0)) {
    ...
    heap_bfd = bfd_pic30_create_heap_bfd (link_info.output_bfd);
...
pic30_set_output_section_flags(lang_output_section_statement_type *os)
{
  ...
  */
  if ((os->bfd_section->flags & (SEC_LOAD)) &&
    ...
    {
      if (strstr(os->region->name_list.name, "data") ||
          (os->lma_region && strstr(os->lma_region->name_list.name, "data")))
        {
...
gld${EMULATION_NAME}_before_allocation()
{
  ...
  if (fbslim_address && 
    ...
    region = lang_memory_region_lookup ("program", FALSE);
                                          /* elements ? */
  ...
  if (pic30_debug)
    {
      ...
      region = pic30_all_regions();
      for (; region; region = region->next) {
        bfd_pic30_print_region_info(region->name_list.name);
      }
    }
...
gld${EMULATION_NAME}_after_allocation()
{
...

  {
    ...
    */
    ...
    for (u = lang_output_section_statement.head;
         ...
         u = (lang_statement_union_type *) lookup->next) {
  ...
  if (pic30_debug)
    {      
      ...
      bfd_pic30_print_section_header();
      bfd_map_over_sections(link_info.output_bfd, &bfd_pic30_report_sections, 0);
  ...
  */
  if (pic30_allocate) {
    allocate_memory();
    pic30_allocation_complete = TRUE;
     ...
     char * name;
{
  ...
  if (region->length == ~(bfd_size_type) 0) {
    ...
    /* CAW - setting region to 0 requires that all callers of region_lookup
       check the return value... none of them do.

       returning to the old behaviour for safety */
...
*/
static lang_output_section_statement_type *
gld${EMULATION_NAME}_place_orphan( sec, secname, constraint)
  asection *sec;
  const char *secname;
  int constraint ATTRIBUTE_UNUSED;
{
...
#if 0
  if (pic30_debug)
    printf("__place_orphan(): section %s from %s\n",
           secname ? secname : "NULL", sec->owner ? sec->owner->filename : "NULL" );
#endif
  ...
  if (pic30_allocate) {  /* if best-fit allocator enabled */
    ...
    /* if this section represents a memory region,
       ...
       processed. */
    if (PIC30_IS_INFO_ATTR(sec) &&
        strncmp(secname, memory_region_prefix,
                strlen(memory_region_prefix)) == 0) {
                
      if (!memory_region_list)
        pic30_init_section_list(&memory_region_list);

      if (!pic30_name_in_section_list(memory_region_list, secname))
        pic30_append_section_to_list(memory_region_list, NULL, sec);
      has_user_defined_memory = TRUE;
      return (lang_output_section_statement_type *) 1;
    }
    ...
    /*
    ** If section is absolute, have we already recorded it?
    **  Note: MCC may specify the same interrupt vector
    **  more than once, as in XC16-2214.
    */
    if PIC30_IS_ABSOLUTE_ATTR(sec) {
      struct pic30_section *s;
      if (s = pic30_section_in_section_list(unassigned_sections, secname)) {
        if (PIC30_IS_ABSOLUTE_ATTR(s->sec) && (sec->vma == s->sec->vma)) {
          if (pic30_debug)
            printf("Unmapped section %s at address 0x%lx has already been recorded\n",
                    secname, sec->vma);
          return (lang_output_section_statement_type *) 1;
        }
      }
    ...
    if (pic30_debug && sec->size) {
      char *str1 = pic30_section_attr_string(sec);
      printf("%sRecording unmapped section '%s', len = 0x%lx, flags = %x, %s\n",
             orphan_cnt++ ? "" : "\n", secname, sec->size, sec->flags, str1);
      if (str1) free(str1);
    }
    ...
    pic30_append_section_to_list(unassigned_sections, NULL, sec);
  ...
  if (sec->psv)
    {
      ...
      os = lang_output_section_statement_lookup (secname, 0, TRUE);

      os->region = region_lookup ("data");
      os->lma_region = region_lookup ("program");

      lang_add_section (&os->children, sec, NULL, os);
      was_placed = 1;
    }
  else if (sec->eedata)
    {
  ...
  return was_placed ? os : NULL;

} /*static void gld${EMULATION_NAME}_place_orphan () */

static bfd_boolean
elf_link_check_archive_element (name, abfd, linfo, new_mask_bits, new_set_bits)
     char *name;
     bfd *abfd;
     struct bfd_link_info *linfo;
     ...
     unsigned int *new_set_bits;
{
  ...
  bfd_boolean has_sig = bfd_has_signature(abfd,
                                          ...
                                          &signature_set);
  if (pic30_debug)
    printf("\nelf_link_check_archive_element: looking for %s in %s\n",
         name, abfd->filename);

  if (((global_PROCESSOR) &&
      (bfd_default_compatible(abfd->arch_info, global_PROCESSOR) == 0)) ||
      (bfd_default_compatible(abfd->arch_info, link_info.output_bfd->arch_info) == 0)) {
    if (pic30_debug) {
       printf("\n%s failed bfd_default_compatible\n", abfd->filename);
    }
    return FALSE;
  }
  ...
  if (strcmp(name, CRT0_KEY) == 0) {
    if (pic30_data_init == 0) {
      _bfd_generic_link_add_one_symbol (linfo, pic30_output_bfd, CRT0_KEY,
                                        BSF_GLOBAL, bfd_abs_section_ptr,
                                        0, CRT0_KEY, 1, 0, 0);
      if (pic30_debug)
  ...
  if (strcmp(name, CRT1_KEY) == 0) {
    if (pic30_data_init) {
      _bfd_generic_link_add_one_symbol (linfo, pic30_output_bfd, CRT1_KEY,
                                        BSF_GLOBAL, bfd_abs_section_ptr,
                                        0, CRT1_KEY, 1, 0, 0);
      if (pic30_debug)
...
#if 0
        _bfd_generic_link_add_one_symbol (linfo, pic30_output_bfd, CRTMODE_KEY,
                                          BSF_GLOBAL, bfd_abs_section_ptr,
                                          0, CRTMODE_KEY, 1, 0, 0);
#endif
...
#if 0
          _bfd_generic_link_add_one_symbol (linfo, pic30_output_bfd, 
...
/* In case when user references __reset in their code without defining it,
 ...
 *  in more than one source file from which objects are built in the archive 
 *  with the same options. So when the linker finds the first object that
 ...
 *  FS */
  ...
  */
  ...
  if (pic30_force_keep_symbol)
      (void) pic30_force_keep_symbol(name, (char *) abfd->filename);
  return result;
} 
...
{			     
  *isfile = 0;
  ...
  if ((link_info.type == type_relocatable) && (config.build_constructors == TRUE))
    return
EOF
...
struct ld_emulation_xfer_struct ld_${EMULATION_NAME}_emulation = 
{
  ...
  gld${EMULATION_NAME}_after_parse,
  gld${EMULATION_NAME}_after_open,
  after_check_relocs_default,	/* after_check_relocs */
  gld${EMULATION_NAME}_after_allocation,
  gld${EMULATION_NAME}_set_output_arch,
