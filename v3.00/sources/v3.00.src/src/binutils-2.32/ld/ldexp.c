  ...
  if (expld.result.valid_p)
      ...
      switch (tree->type.node_code)
	{
	    ...
	    case NEXT:
          ...
          if ((pic30_current_os) && pic30_current_os->region &&
               strcmp (pic30_current_os->region->name_list.name, "*default*") != 0)
          {
              expld.result.value = align_n (pic30_current_os->region->current,
                                              expld.result.value);
          }
          else
...
fold_name (etree_type *tree)
{
  ...
  switch (tree->type.node_code)
    ...
    case DEFINED:
      ...
      expld.result.section = abs_output_section->bfd_section;
#endif
      break;
	      ...
	      if (output_section == NULL)
		{
		  if (expld.phase <= lang_mark_phase_enum)
		    ...
		    einfo (_("%X%P:%pS: unresolvable symbol `%s'"
			   ...
			   tree, tree->name.name);
#else
        /* Don't report an error, because the best-fit
         * allocator will create the output section later.
         */
#endif
		}
      ...
      if (tree->assign.dst[0] == '.' && tree->assign.dst[1] == 0)
	{
	  ...
	  if (expld.phase != lang_first_phase_enum)
		       ...
		       || expld.section == bfd_abs_section_ptr)
		{
		   ...
		   *       it to an ABS section; don't add the vma again.
		   */
		  if ((expld.result.section == abs_output_section->bfd_section) &&
		    (expld.result.value > expld.section->vma)) /* is this right? - GM 9/8/21 */
