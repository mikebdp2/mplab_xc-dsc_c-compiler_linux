...
#define PIC30_OPERAND_SHIFTBITS 3

struct pic30_operand
{
  /* an operand may not be consecutive bits - allow operand bits to be split
       int N bits/shift combinatons */
  unsigned max_operand_shiftbits;
  ...
  } encoding[PIC30_OPERAND_SHIFTBITS];
  ...
  char *(*extract) (unsigned long *insn,
                    struct disassemble_info *info, /* ignore warning */
                    unsigned long flags,
                    const struct pic30_operand * operand,
                    unsigned char *err);

  /* mode shift */
  unsigned int register_mode_offset;
};
...
struct pic30_opcode
{
  ...
  /*
   * The number of words in this instruction, and the number of bytes in the
   *   instruction word.
   *   older than AA insn will have n words and 4 bytes per IW (inc phantom)
   *   AA or oder will  have n words and 2 bytes per IW
   *
   * Encoding it here allows us to vary the way we grab memory in frag_more()
   */
  unsigned short bytes_per_IW; 
  ...
  /* void * (opaque) used by objdump only */
  void *pic30_usage_stats;
};
/******************************************************************************/
 ...
 *
*/
void record_private_data(struct disassemble_info *, long, int); /* ignore warning */
