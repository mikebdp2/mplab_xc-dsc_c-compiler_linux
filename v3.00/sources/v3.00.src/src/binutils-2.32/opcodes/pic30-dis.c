...
unsigned char global_dec_literals = FALSE;

void pic30_parse_disassembler_options ( char * option);

void pic30_disassemble_do_insn
( const struct pic30_opcode * opcode,
   unsigned long *insn, struct disassemble_info * info);

void pic30_disassemble_call_goto_insn 
( const struct pic30_opcode * opcode, unsigned long insn_word1,
   unsigned long insn_word2, struct disassemble_info *info);

void pic30_disassemble_bf_insn 
( const struct pic30_opcode * opcode, unsigned long insn_word1,
   unsigned long insn_word2, struct disassemble_info *info);

char pic30_disassemble_2word_insn 
( const struct pic30_opcode * opcode, unsigned long insn_word1,
   bfd_vma memaddr, struct disassemble_info *info);

unsigned char pic30_disassemble_1word_insn 
( const struct pic30_opcode * opcode, unsigned long *insn,
   struct disassemble_info *info);

int pic30_print_insn
( bfd_vma memaddr, struct disassemble_info *info);
...
{
   fprintf(stream,
       "\nMicrochip specific disassembler options (supported with -M):\n"
         "  symbolic -- Include symbolic information during disassembly.\n"
         "  dec      -- display literals in decimal format.\n"
         "\n");
}
...
void
pic30_disassemble_do_insn (opcode, insn, info)
   const struct pic30_opcode * opcode;
   unsigned long *insn;
   struct disassemble_info * info;
{
   /* CONVERT OPERANDS TO USE OFFSET... get rid of special functions?
      CAW */
   int i;
   ...
   for (i = 0; i < opcode->number_of_operands; i++)
   {
      ...
      {
         operand_value = pic30_extract_operand(&insn[i], opnd);
   ...
   struct disassemble_info *info;
{
   ...
   int status;
   bfd_byte bytes_read[PIC30_MAX_SIZE_OF_PROGRAM_WORD + 1];
   int i;
   ...
   next_word_offset = pic30_au_per_insn(
                        bfd_lookup_arch(info->arch,info->mach)
                      );

   private_data = info->private_data;

   status = (*info->read_memory_func) (memaddr + next_word_offset,
                                       bytes_read,
                                       PIC30_MAX_SIZE_OF_PROGRAM_WORD,
                                       info);

   if (status != 0)
      return -1;

   for (i = 0; i < PIC30_MAX_SIZE_OF_PROGRAM_WORD; i++)
      insn_word2 = insn_word2 | ((unsigned long)bytes_read[i] << (8 * i));

   full_insn[0] = insn_word1;
   full_insn[1] = insn_word2;
   ...
   switch (insn_word1 & opcode->mask)
   {
      case DO_INSN:
      case DOW_INSN:
         pic30_disassemble_do_insn (opcode, &full_insn, info);
         break;
      ...
      case BFINS_INSN:
         ...
         break;
      default:
         rc = pic30_disassemble_1word_insn(opcode, &full_insn, info);
         break;
   } /* switch (insn_word1) */
   ...
   struct disassemble_info *info;
{
   ...
   if (rc)
   {
      ...
      for (j = 0; j < opcode->number_of_operands; j++)
      {
         ...
         if (opnd->extract)
         {
            ...
            extracted_string =
               (*opnd->extract) (insn, info,
                                 opcode->flags, opnd, &error_found);
            /* allow for delayed printing of operands */
            if (error_found) {
              fprintf(stderr,"Error!\n");
            } else if (extracted_string) {
              skip_operand=0;
            strcpy (operand_string, extracted_string);
            free (extracted_string);
            }
         ...
         {
            operand_value = pic30_extract_operand(insn, opnd);
            skip_operand=0;
            switch (opnd->type)
            {
               case OPND_W_REG | OPND_REGISTER_DIRECT:
                  /* encoded as a register number */
                  /* fall through */
               case OPND_REGISTER_DIRECT:
...
/* This function returns number of bytes disassembled at address 'memaddr' */
   ...
   struct disassemble_info *info;
{
   int rc = 0;
   bfd_byte bytes_read[PIC30_MAX_SIZE_OF_PROGRAM_WORD + 1];
   unsigned long insn[MAX_WORDS_PER_INSN];
   int opb = info->octets_per_byte;
   int status;
   int i,j;

   const bfd_arch_info_type *proc = bfd_lookup_arch(info->arch,info->mach);

   int is_ecore= pic30_is_ecore_machine(proc);
   int is_isav4= pic30_is_isav4_machine(proc);
   int is_isav32= pic30_is_isav32_machine(proc);

   struct pic30_private_data *private_data;

   int pic30_sizeof_program_word = PIC30_MAX_SIZE_OF_PROGRAM_WORD;

   if (is_isav32) pic30_sizeof_program_word = 2;
   private_data = info->private_data;
   if (private_data) {
   ...
   for (i = 0; i < MAX_WORDS_PER_INSN; i++) insn[i] = 0;
   ...
   if (PIC30_DISPLAY_AS_DATA_MEMORY(info->section))
   {
      ...
      for (i = 0, j = 0; i < PIC30_SIZE_OF_DATA_WORD * opb; i++)
        if (PIC30_CAN_PRINT_IN_DATA_MEMORY(i,opb))
          {
            insn[0] = insn[0] | ((unsigned long)bytes_read[i] << (8 * j));
            j++;
          }

      (*info->fprintf_func) (info->stream, ".word %#04lx", insn[0]);
   ...
   else if (PIC30_DISPLAY_AS_PROGRAM_MEMORY(info->section))
   {
      ...
      unsigned long insn_half = PIC30_SIZE_OF_INSN_MASK;
      unsigned char found_count = 0;
      bfd_vma readaddr = memaddr;
      unsigned int num_bytes;

      /* CAW -
       *   read 'aligned' memaddr; because if we just read a 2byte insn
       *   we will be off.  adjust which part we print later.
       *
       */
      if (pic30_sizeof_program_word < PIC30_MAX_SIZE_OF_PROGRAM_WORD) {
        readaddr &= -PIC30_MAX_SIZE_OF_PROGRAM_WORD;
      }

      num_bytes = PIC30_MAX_SIZE_OF_PROGRAM_WORD;

      /*
       * If stop_vma is specified, don't read past it.
       *
       * This is a problem for 16-bit instructions
       * located just before a label.
       */
      if (info->stop_vma &&
          ((num_bytes/opb + readaddr) > info->stop_vma))
      {
        num_bytes = info->stop_vma - readaddr;
      }

      status=(*info->read_memory_func)(
               readaddr, bytes_read, num_bytes, info);
      ...
      /* gather instruction word from little endian bytes */
      for (i = 0; i < num_bytes; i++)
         insn[0] = insn[0] | ((unsigned long)bytes_read[i] << (8 * i));

      /* scan the opcode table */
      for (i = 0; i < pic30_num_opcodes; i++)
      {
         int all_valid = 1;
         int valid_insn = 0;

         /* only so marked insns are available for this device */
         if (is_isav32) all_valid = 0;

         if ((pic30_opcodes[i].flags & F_ISA32V0) ||
             (pic30_opcodes[i].flags & F_ISA32V016)) {
           all_valid = 0;
           valid_insn |= is_isav32;
         }
         ...
         if (pic30_opcodes[i].flags & F_FCORE) {
           all_valid = 0;
           valid_insn = (!is_ecore) && (!is_isav4) && (!is_isav32);
         }
         if (pic30_opcodes[i].flags & F_GENERATED) {
           all_valid = 0;
           valid_insn = 0;
         }
         if ((all_valid == 0) && (valid_insn == 0)) continue;
         if (is_isav32 &&
             ((memaddr & (PIC30_MAX_SIZE_OF_PROGRAM_WORD-1)) != 0) &&
             (pic30_opcodes[i].bytes_per_IW > 2)) {
           /* based on isav32 and memaddr, we are seeking a 2-byte instruction;
              so we can skip longer opcodes*/
           continue;
         }
         if (pic30_opcodes[i].number_of_words == 0)
           continue; /* skip invalid opcodes */

         if (pic30_opcodes[i].bytes_per_IW < 3) {
           /* consider this 16-bit isav32 opcode */
           /* ...begin by testing the 'long insn' bit of program word */
            if ((insn[0] & PIC30_SIZE_OF_INSN_MASK) == 0) {
              /* this program word has two instructions, which half do we want? */
              if ((memaddr & (PIC30_MAX_SIZE_OF_PROGRAM_WORD-1)) != 0) {
                 /* high half */
                 insn_half = insn[0] >> (pic30_opcodes[i].bytes_per_IW*8);
              } else {
                 insn_half = insn[0] &
                             ((1 << (pic30_opcodes[i].bytes_per_IW*8))-1);
              }

              if ((pic30_opcodes[i].mask & insn_half) ==
                   pic30_opcodes[i].opcode[0]) {
                /* 16-bit opcode could be a match */
            ...
            } /* this program word has two instructions */
         } else {
           /* consider a 3 or 4-byte opcode */
           if ((pic30_opcodes[i].mask & insn[0]) == pic30_opcodes[i].opcode[0])
         {
            /* opcode could be a match */
      ...
      if (found_insn) {
         rc = pic30_opcodes[i].bytes_per_IW + pic30_has_phantom_byte(proc);
         rc *= pic30_opcodes[i].number_of_words;
      }
      else {
         rc = PIC30_MAX_SIZE_OF_PROGRAM_WORD;
         (*info->fprintf_func) (info->stream, ".pword %#08x", insn[0]);
      }
   } /* else if (PIC30_DISPLAY_AS_PROGRAM_MEMORY(info->section))*/
     ...
     {
       status = (*info->read_memory_func) (memaddr, bytes_read,
                                           pic30_sizeof_program_word, info);
       ...
       for (i = 0; i < pic30_sizeof_program_word; i++)
         insn[0] = insn[0] | ((unsigned long)bytes_read[i] << (8 * i));

       rc = pic30_sizeof_program_word;

       (*info->fprintf_func) (info->stream, ".word %#0x", insn[0] & 0xFFFF);
     } /* else if (PIC30_DISPLAY_AS_READONLY_MEMORY(info->section)) */
   ...
   struct disassemble_info * info;
{
   ...
   for (i = 0; i < opcode->number_of_operands; i++)
   {
      ...
      {
         operand_value = pic30_extract_operand(&insn_word, opnd);
