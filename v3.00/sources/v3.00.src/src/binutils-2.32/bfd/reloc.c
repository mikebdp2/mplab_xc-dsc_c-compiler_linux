...
{                                                    \
   fprintf (stdout,"  %s sym %s, addr %lx, addend %lx\n", \
	   n, (*(r->sym_ptr_ptr))->name,             \
	   r->address, r->addend);                   \
}

/*     arelent *q = reloc_entry; */
/*     DUMP_CANON_RELOC(q->howto->name, q); */
  ...
  if (pic30_debug) {
    ...
    printf("symbol->section->output_offset = %lx\n", symbol->section->output_offset);
    printf("reloc_entry->addend = %lx\n", reloc_entry->addend);
...
#if PIC30
  /* call a target-specific range check function */
  if (flag == bfd_reloc_ok)
    flag = pic30_bfd_reloc_range_check(howto, relocation,
                                       abfd, symbol, error_message);
#endif
...
#if PIC30
	      off = (*parent)->address; /* no need to convert address */
#else
...
#if PIC30
          /* convert address from octets to PC units for error reporting */
          (*parent)->address /= bfd_octets_per_byte (input_bfd);
#endif
