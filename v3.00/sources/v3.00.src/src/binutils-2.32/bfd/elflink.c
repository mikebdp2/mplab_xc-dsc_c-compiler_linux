...
bfd_boolean (*pic30_elf_link_check_archive_element)
  (char *, bfd *, struct bfd_link_info *, unsigned int *, unsigned int *);
void (*pic30_update_object_compatibility)
  (const char *, bfd *, unsigned int *, unsigned int *);
bfd_boolean (*pic30_bfd_has_signature)
  (bfd *, unsigned int *, unsigned int *, unsigned int *);
bfd_boolean bfd_elf_process_merge_sections
  (bfd *, struct bfd_link_info *);

extern int pic30_debug;
		       ...
		       bfd_boolean *matched)
{
  ...
  *sym_hash = h;
...
#if 0
  /* debug */
  printf("_bfd_elf_merge_symbol: %s, matched=%s, "
         "newweak=%s, oldweak=%s, newdef=%s, olddef=%s, oldbfd=%s, oldsec=%s\n",
         name, *matched ? "TRUE" : "FALSE",
         newweak ? "TRUE" : "FALSE", oldweak ? "TRUE" : "FALSE",
         newdef ? "TRUE" : "FALSE", olddef ? "TRUE" : "FALSE",
         oldbfd ? oldbfd->filename : "NONE", oldsec ? oldsec->name : "NONE");
#endif

#if 0
  if (*matched && olddef)
    *override = TRUE;  /* keep the old definition */
  else if (*matched && oldweak)
    *skip = TRUE;      /* replace old with new */
  return TRUE;
#endif
  ...
  if (olddef && !olddyn && !oldweak && newdef && !newdyn && !newweak
    ...
    {
#if PIC30
      /* Allow multiple symbol definitions from archives.
       * This can happen with deferred matching of
       * c30 object signatures.
       */
      if (abfd->archive_pass) {
        if (pic30_debug) {
          printf("...skipping multiple (archive) definition of %s\n",
               name);
        }
        *skip = TRUE;
        return TRUE;
...
_bfd_elf_link_sec_merge_syms (struct elf_link_hash_entry *h, void *data)
{
  ...
  if ((h->root.type == bfd_link_hash_defined
      ...
      && sec->sec_info_type == SEC_INFO_TYPE_MERGE)
    {
      ...
      if (pic30_debug)
      {
        printf("_bfd_elf_link_sec_merge_syms(): %s\n",
               h->root.root.string);
      }
#endif
...
#if PIC30
/* Traverse through the elf linker hash table
 * and process SEC_MERGE sections.
 *
 * In a vanilla elf link, this work is performed by
 * bfd_elf_final_link(), but that function is not
 * called by the pic30 linker.
 */
bfd_boolean
bfd_elf_process_merge_sections(bfd *abfd, struct bfd_link_info *info)
{
  asection *o;
  struct bfd_link_order *p;
  bfd_boolean merged = FALSE;
  struct elf_link_hash_table *htab = elf_hash_table (info);
  ...
  if (pic30_debug)
  {
    printf("\nbfd_elf_process_merge_sections(): %s ...",
           abfd->filename);
  }
...
error_free_dyn:
  ...
  for (create_application_id = 0; 
       (int) create_application_id <= pic30_application_id;
      ...
      if (is_elf_hash_table (htab))
	{
	  ...
	  if ((pic30_ivt) && (create_application_id == 0)) {
              ...
              if (r->name && (strcmp(r->name, name+1) == 0)) {
                ...
                ivt_sec->rawsize = ivt_sec->size = 4;
                r->ivt_sec = ivt_sec;
    ...
    /* If this symbol has default visibility and the user has
      ...
      if (create_application_id) {
            if ((sec->linked == 0) && (sec != bfd_abs_section_ptr)) {
              ...
              if (! _bfd_elf_merge_symbol(abfd, info, alias_name, isym, &sec, &value,
...
#ifdef PIC30
      if (create_application_id) {
        if (! (_bfd_generic_link_add_one_symbol /* ignore warning - alias_name is defined above */
               (info, abfd, alias_name, flags | BSF_SHARED, sec, value, (const char *) NULL,
                FALSE, bed->collect, (struct bfd_link_hash_entry **) sym_hash)))
        goto error_free_vers;
      } else
#endif
...
#if PIC30
        if (pic30_select_objects && (definition == FALSE)) {
          pic30_update_object_compatibility(name, abfd, 
                                      &signature_mask, 
                                      &signature_set);
        } else if (pic30_debug) {
          printf("  defined: %s\n", name);
            ...
            if (warned_unknown_arch == 0) {
              ...
              (*_bfd_error_handler)
                  (_("%s: warning: unknown objects in library;"
                     " incorrect DFP specified?"), bfd_get_filename (abfd));
            }
	    continue;
          }
#else
...
#ifdef PIC30
      if (pic30_debug) {
        printf("Checking deferred objects\n");
        debug_deferred_archive_members();
      }
      while (loop == 0) {
...
#ifdef PIC30
        if (pic30_debug) {
          printf("...adding archive element %s\n", m->symdef->name);
        }
#endif
				 ...
				 elf_gc_mark_hook_fn mark_hook ATTRIBUTE_UNUSED)
{
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link.next)
	...
	{
#if PIC30
        char *attr = pic30_section_attr_string(isec);
        if (pic30_debug)
          printf("      looking at section %s, flags = %#x %s\n",
                 isec->name, isec->flags, attr ? attr : "NULL");
        if (attr) free(attr);
#endif
	  if ((isec->flags & SEC_LINKER_CREATED) != 0)
        ...
        /* Note: PIC30 compiler does not create debug section
         fragments, so the section name will never match. */
	  if (!debug_frag_seen
	  ...
	  if ((isec->flags & SEC_CODE) != 0
	      && isec->gc_mark == 0)
	    {
		 ...
		 debug section associated with .text.foo.  */
            /* Note: PIC30 compiler does not create debug section
             fragments and does not associate sections this way. */
  ...
  for (sub = info->input_bfds; sub != NULL; sub = sub->link.next)
    {
      ...
      if (pic30_debug)
        printf("    elf_gc_sweep(): input_bfd = %s\n", sub->filename);
      if (bfd_get_flavour (sub) != bfd_target_elf_flavour
	...
	{
#if PIC30
        char *attr = pic30_section_attr_string(o);
        if (pic30_debug)
          printf("      looking at section %s, flags = %#x %s\n",
                 o->name, o->flags, attr ? attr : "NULL");
        if (attr) free(attr);
#endif
...
#if PIC30
        if (pic30_debug && o->size != 0)
          printf("        %s marked for removal\n", o->name);
#endif
...
#if PIC30
      if (pic30_debug)
        printf("    bfd_elf_gc_sections(): looking at input_bfd %s\n", sub->filename);
#endif
      if (bfd_get_flavour (sub) != bfd_target_elf_flavour
