...
{ 
  ...
  fprintf (file, _("  --no-handles           Don't create jump table for"
                   " function pointers\n")); 
  fprintf (file, _("  --fill=VALUE           Fill unused program memory\n"));
  fprintf (file, _("  --fill-upper VALUE     Set fill value for upper byte of data"
                   " in program memory (default = 0)\n"));
...
static void pic30_create_fill_option (struct pic30_fill_option *option)
{
  ...
  option->fill_width = pic30_bytes_per_insn(0);
  option->range_size = 0;
}
...
#if 1      
         if ((option->fill_width != pic30_bytes_per_insn(0)) && 
             (option->fill_width != 1)){
           einfo(_("%P: Warning: You can't use a width_value of %d."
                   ...
                   pic30_bytes_per_insn(0), option->fill_width);
           option->fill_width = pic30_bytes_per_insn(0);
         }
#endif
...
{ 
  ...
  const char *not_supported_on_target = "option is not supported on";
...
/*   const char *general_option_warn = "multiple general options specified\n"; */
  ...
  switch (optc) 
    ...
    case FILL_UPPER_OPTION:
      if (pic30_is_isav32_machine(global_PROCESSOR)) {
        einfo(_("%P: Warning: --fill-upper option ignored on %s\n"),
    ...
    */
    ...
    case ADD_CONST_FLAGS:
      if (pic30_is_isav32_machine(global_PROCESSOR)) {
        einfo(_("%P: Warning: --add-flags-const option ignored on %s\n"),
              global_PROCESSOR->printable_name);
      }
      else {
