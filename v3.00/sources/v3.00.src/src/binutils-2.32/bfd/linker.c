...
bfd_boolean pic30_is_valid_archive_element(struct bfd_link_info *, asymbol *,
...
bfd_boolean pic30_is_valid_archive_element
  (struct bfd_link_info *, asymbol *, bfd *, unsigned int *,
             unsigned int *);
#endif
  ...
  for (; pp < ppend; pp++)
...
#ifdef PIC30
      if (deferred_member) {
        if (deferred_member->duplicate_skip) continue;
        BFD_ASSERT(pp >= ppend);
	...
	continue;
      ...
      if (pic30_force_keep_symbol &&
          pic30_force_keep_symbol(p->name, abfd->filename)) { /* ignore warning */
#endif
      if (! bfd_is_com_section (p->section)
	  || (h->type == bfd_link_hash_undefined
	      && h->u.undef.abfd == NULL))
...
#if 0
        /* This pic30-specific code is no longer used,
         * because the enclosing function was called
         * only from the COFF linker. -- GM 7/26/23 */
...
#if 0
  if (pic30_debug) {
    printf("  _bfd_generic_link_add_one_symbol: %s, flags: %x\n", name, flags);
  }
#endif
...
#if 0
  if (pic30_debug) {
    printf("  _bfd_generic_link_add_one_symbol: %s, row = %x, h->type: %x\n", name, row, h->type);
  }
#endif
  if (info->notice_all
...
#if 0
  if (pic30_debug) {
    printf("  _bfd_generic_link_add_one_symbol: %s, link_action: %x\n", name, action);
  }
#endif
      switch (action)
	{
	...
	case MDEF:
	  /* Handle a multiple definition.  */
          ...
          if (PIC30_IS_SHARED_ATTR(section) &&
              ...
              section->size == h->u.def.section->size &&
...
#if PIC30
              if (pic30_debug) {
                fprintf( stderr, "Warning: _bfd_generic_final_link() reports generic reloc error at "
                        "offset %lx of section '%s'\n", p->offset, o->name);
                return TRUE;
              }
              else {
                return FALSE; /* reports "final link failed: bad value" */
              }
#else
              return FALSE;
#endif
...
#if PIC30
              if (pic30_debug) {
                fprintf( stderr, "Warning: _bfd_generic_final_link() reports indirect link order error at "
                        "offset %lx of section '%s'\n", p->offset, o->name);
              return TRUE;
              }
            else {
...
#if PIC30
              if (pic30_debug) {
                fprintf( stderr, "Warning: _bfd_generic_final_link() reports default link order error at "
                        "offset %lx of section '%s'\n", p->offset, o->name);
              return TRUE;
              }
            else {
  ...
  for (; sym_ptr < sym_end; sym_ptr++)
    {
      ...
      sym = *sym_ptr;
#ifdef PIC30
      /* a paragraph of code was excluded here,
       * starting at line 2776 in the v2.14 version
       */
#endif
	  ...
	  if (h != NULL)
	    {
		 ...
		 check that.  */
              ...
              /* The above comment may no longer be relevant. - GM 8/31/21 */
#if !defined(PIC30)
		...
		case bfd_link_hash_indirect:
		  ...
		  /* fall through */
		case bfd_link_hash_defined:
#if defined(PIC30) /* Prepare for a subsequent, co-resident link */
...
#if 0
  if (pic30_debug)
    printf("  %s, section '%s', flags 0x%x, type %d\n", sym->name,
           sym->section->name ? sym->section->name : "NONE", sym->flags, h ? h->root.type : -1);
      if (h && !h->sym)
        h->sym = sym;
#endif
	...
	output = FALSE;
#ifdef PIC30
      else if (sym->flags == 0 && h &&
        h->root.type == bfd_link_hash_defweak)
        /* we created this condition above, to support co-resident applications */
        output = TRUE;
#endif
      else
	abort ();
			       ...
			       struct bfd_link_order *link_order)
{
  ...
  if (link_order->type == bfd_section_reloc_link_order)
    ...
    {
#if PIC30
      /* Note: Throughout this file we are using
       * "struct generic_link_hash_entry" instead
       * of "struct elf_link_hash_entry" as was the
       * case in previous binuils. See for example
       * line 3203 in the previous version.
       *                             - GM 9/14/21
       */
#endif
      struct generic_link_hash_entry *h;
...
#if PIC30
 /* None of the below functions appear in the
  * binutils 2.14 version of this file.
  */
#endif
