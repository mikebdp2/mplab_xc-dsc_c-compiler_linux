...
#if PIC30_DEBUG
  printf("  name = %s, sh_flags = %lx, sh_addr = %lx, sh_info = %x\n",
...
/*FS This check is necessary to keep .isr section against garbage collection*/
  ...
  if (pic30_debug) {
   char *attr = pic30_section_attr_string(newsect);
   printf("\nCreating ELF section '%s' from '%s', initial map = %x, %s\n",
           name, abfd->filename ? abfd->filename : "<none>",
           pic30_attribute_map(newsect), attr ? attr : "");
    printf("  sh_flags = %lx, sh_addr = %lx, sh_info = %x\n",
           hdr->sh_flags, hdr->sh_addr, hdr->sh_info);
    if (attr) free(attr);
  }
...
/* We do not parse the PT_NOTE segments as we are interested even in the
  ...
  if ((flags & SEC_ALLOC) != 0)
      ...
      for (i = 0; i < elf_elfheader (abfd)->e_phnum; i++, phdr++)
	{
      ...
      /* what follows does not exactly match previous binutils code */
#endif
      ...
      /* All of the compress logic below is quite new */
#endif
...
{
  char *attr = pic30_section_attr_string(newsect);
  printf("_bfd_elf_make_section_from_shdr: %s, map = %x, %s\n",
         name, pic30_attribute_map(newsect), attr ? attr : "");
  printf("_bfd_elf_make_section_from_shdr: %s, vma = %lx, lma = %lx, len = %lx\n",
         name, newsect->vma, newsect->lma, newsect->size/2);
  if (attr) free(attr);
}
#endif
...
elf_fake_sections (bfd *abfd, asection *asect, void *fsarg)
{
...
#if PIC30_DEBUG
  printf("\nelf_fake_sections: %s, vma = %lx, lma = %lx, size = %lx\n",
         asect->name, asect->vma, asect->lma, asect->size);
  ...
  if ((asect->map_head.link_order == 0) &&
  ...
  if ((asect->flags & SEC_GROUP) != 0)
    sh_type = SHT_GROUP;
#ifdef PIC30
  else if ((asect->flags & SEC_DEBUGGING) != 0)
    sh_type = SHT_NOTE;
  /* FIXME: May need another test here to protect noload sections.
   * See line 2844 in the old binutils
   */
#endif
  else
...
#if PIC30_DEBUG
{
  char *attr = pic30_section_attr_string(asect);
  printf("\nelf_fake_sections: %s, initial sh_flags = %lx\n",
         asect->name, this_hdr->sh_flags);
  printf("elf_fake_sections: %s, map = %x, %s\n",
         asect->name, pic30_attribute_map(asect), attr ? attr : "");
  if (attr) free(attr);
}
#endif
  ...
  printf("elf_fake_sections: %s, sh_flags = %lx\n",
         asect->name, this_hdr->sh_flags);
  printf("elf_fake_sections: %s, sh_addr = %lx, sh_size = %lx, sh_info = %x\n",
  ...
  for (idx = 0; idx < symcount;)
...
#ifdef PIC30
      symstrtab[idx+1].sym.st_name = -1;  // in case we skip this symbol
#endif
      ...
      else
	{
...
#ifdef PIC30
      {
		    ...
		    /* is we can't identify the section, don't emit the symbol */
		     if (shndx == SHN_BAD)
		       continue;
      }
#else
...
#else
      symstrtab[idx+1].sym = sym;
      symstrtab[idx+1].dest_index = outbound_syms_index;
      symstrtab[idx+1].destshndx_index = outbound_shndx_index;
#endif
