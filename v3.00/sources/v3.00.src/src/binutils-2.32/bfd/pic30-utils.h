...
#define PIC30_SAVE_FLAGS(sec,to)                         \
  { to = malloc(32*sizeof(int));  /* modify as needed */ \
    to[0] = (sec)->flags;                                \
    to[1] = (sec)->near;                                 \
    to[2] = (sec)->persistent;                           \
    to[3] = (sec)->xmemory;                              \
...
struct pic30_section
{
  ...
  PTR *file;
  asection *sec;
  unsigned int alloc_error_reported;
};
...
extern unsigned int pic30_octets_per_byte_power(void);
extern unsigned int pic30_has_phantom_byte(const bfd_arch_info_type *);
extern unsigned int pic30_bytes_per_insn(const bfd_arch_info_type *);
extern unsigned int pic30_au_per_insn(const bfd_arch_info_type *);
...
void pic30_create_data_init_templates(void);
...
#define OCTETS_PER_BYTE_POWER pic30_octets_per_byte_power()
...
int pic30_is_valid_attributes(unsigned int, unsigned char);
int pic30_display_as_program_memory_p(asection *sec);
int pic30_display_as_data_memory_p(asection *sec);
int pic30_display_as_readonly_memory_p(asection *sec);
...
extern bfd_boolean pic30_is_reserved_symbol(char * name);
