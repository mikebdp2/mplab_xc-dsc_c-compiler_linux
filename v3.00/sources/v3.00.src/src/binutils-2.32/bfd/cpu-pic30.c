...
char *tool_name = NULL;  /* other files need this */
static char *pic30_resource_file = 0;

#define ARCH2(machine, printable_name, default_machine, next, flags, \
              bits_per_byte, bits_per_word, bits_per_address) \
{ bits_per_word,          \
  bits_per_address,       \
  bits_per_byte,          \
  bfd_arch_pic30,         \
...
#define ARCH(machine, printable_name, default_machine, next, flags) \
   ARCH2(machine, printable_name, default_machine, next, flags, 16, 16, 24)
...
// the last 4-bits of the flags are not used in records
 ...
 *
 * 99 is the last ID available in this sequuence
 */
static bfd_arch_info_type generic_8 = 
   ARCH (97,                "GENERIC-16DSP-BB", TRUE, 0, HAS_BB | HAS_EDS | HAS_ISAV4 | HAS_DUALCORE);

static bfd_arch_info_type generic_7a = 
   ARCH2(96,                "GENERIC-32DSP-AK", TRUE, &generic_8, HAS_DSP | HAS_ISA32V0, 8, 16, 32);

static bfd_arch_info_type generic_7 = 
   ARCH2(96,                "GENERIC-16DSP-AA", TRUE, &generic_7a, HAS_DSP | HAS_ISA32V0, 8, 16, 32);

static const bfd_arch_info_type generic_6 = 
   ARCH ( 95,                "GENERIC-16DSP-CH", TRUE, &generic_7, HAS_DSP | HAS_EDS | HAS_ISAV4 | HAS_DUALCORE);

static const bfd_arch_info_type generic_5 = 
   ARCH ( 94,                "GENERIC-16DSP-EP", TRUE, &generic_6, HAS_DSP | HAS_EDS | HAS_ECORE);

static const bfd_arch_info_type generic_4 = 
   ARCH ( 93,                "GENERIC-16BIT-EP", TRUE, &generic_5, HAS_EDS | HAS_ECORE);

static const bfd_arch_info_type generic_3 = 
...
static struct pic30_resource_info arch_flags_head[] = {
   ...
   { HAS_DSP | HAS_EDS | HAS_ISAV4, &generic_6, &arch_flags_head[7] },
   { HAS_DSP | HAS_ISA32V0, &generic_7, &arch_flags_head[8] },
   { HAS_DSP | HAS_ISA32V0, &generic_7a, &arch_flags_head[9] },
   { HAS_BB | HAS_EDS | HAS_DSP | HAS_ISAV4, &generic_8, 0 },
};


/* Last index value of generic device in arch_flags_head */
#define LAST_GENERIC_DEVICE 8
...
static int pic30_build_arch_table(const char *name, unsigned int flags,
                                  enum bfd_architecture machine) {
  static struct pic30_resource_info *arch_flags_tail = 
              &arch_flags_head[sizeof(arch_flags_head)/
                               sizeof(struct pic30_resource_info)-1];

  bfd_arch_info_type *arch;
  struct pic30_resource_info *resource;

  arch = xmalloc(sizeof(bfd_arch_info_type));
  resource = xmalloc(sizeof(struct pic30_resource_info));
  resource->next = 0;

  if (flags & HAS_ISA32V0) {
    bfd_arch_info_type generic_2 = ARCH2(machine,name, TRUE, 0, flags, 8, 16, 32);
    memcpy(arch, &generic_2, sizeof(bfd_arch_info_type));
  } else {
    bfd_arch_info_type generic_1 = ARCH(machine, name, TRUE, 0, flags);
    memcpy(arch, &generic_1, sizeof(bfd_arch_info_type));
  }
  resource->flags = flags;
  resource->arch_info = arch;
  
  /* link */
  arch_flags_tail->next = resource;
  arch_flags_tail->arch_info->next = arch;
  
  arch_flags_tail = resource;
}

int
pic30_is_generic_machine(unsigned int machine) {
  if ((machine == 0) || ((machine >= 90) && (machine <= 97))) return 1;
  return 0;
}
...
static void get_resource_path(const char *resource) {
  ...
  char *c;
  tool_name = (char *) resource; /* tool_name is global */
...
#pragma GCC diagnostic pop
  ...
  if ((mode == ARCH_TABLE) && rib->field_count >= 3) {
    ...
    for (record = 0; move_to_record(record); record++) {
      ...
      if ((d2.v.i & RECORD_TYPE_MASK) == IS_DEVICE_ID) {
        read_value(rik_int, &d3);
        pic30_build_arch_table(d.v.s, d2.v.i & 0x0FFFFFFFUL, d3.v.i);
      } else {
        /* end of IS_DEVICE_ID records */
        free(d.v.s);
        break;
      ...
      for (;move_to_record(record); record++) {
        ...
        if (((d2.v.i & RECORD_TYPE_MASK) == IS_CODEGUARD_ID)  && 
          ...
          ivt_next->offset = ((d2.v.i >> VECTOR_IDX_SHIFT) & 
                      ((1 << (VECTOR_IDX_WIDTH+1)) - 1)) * pic30_au_per_insn(0);
...
unsigned int
pic30_has_phantom_byte(const bfd_arch_info_type *proc) {
  if (proc == 0) {
     proc = global_PROCESSOR;
  }
  if (proc && proc->bits_per_address == 32) return 0;
  if (proc && proc->bits_per_byte == 8) return 0;
  return 1;
}
 
unsigned int
pic30_bytes_per_insn(const bfd_arch_info_type *proc) {
  if (proc == 0) {
     proc = global_PROCESSOR;
  }
  if (proc && proc->bits_per_address == 32) return 4;
  if (proc && proc->bits_per_byte == 8) return 2;
  return 3;
}

unsigned int
pic30_au_per_insn(const bfd_arch_info_type *proc) {
  if (proc == 0) {
     proc = global_PROCESSOR;
  }
  if (proc && proc->bits_per_address == 32) return 4;
...
int
pic30_is_isaBB_machine(const bfd_arch_info_type *proc)
{
  int rc = 0;
  struct pic30_resource_info *f;
...
int
pic30_is_isav32_machine(const bfd_arch_info_type *proc)
{
...
void pic30_defer_archive(carsym *symdef,
  ...
  for (m = pic30_deferred; m; m = m->next) {
    // if (m->duplicate_skip) continue;
    if ((strcmp(m->abfd->filename,abfd->filename) == 0) &&
        (m->duplicate_skip == 1)) {
      if (pic30_debug) {
        printf("...File already skipped in list\n");
      }
      duplicate_skip = 1;
    } else if (m->symdef ? 
      ...
      } else if (strcmp(m->abfd->filename,abfd->filename) == 0) {
        if (pic30_debug) {
          printf("...Symbol/file already in list\n");
        }
        return;
      } else {
        if (pic30_debug) {
  ...
  m->aye = aye;
  m->duplicate_skip = duplicate_skip;
  pic30_deferred = m;
}
...
struct pic30_deferred_archive_members *pic30_pop_tail_archive() {
  struct pic30_deferred_archive_members *m,*last,*next,*return_m;
...
void debug_deferred_archive_members() {
  ...
  for (m = pic30_deferred; m; m = next) {
    ...
    if (header) {
      printf(
" idx)                 name                   file  mask,bits      flag\n"
"----------------------------------------------------------------------\n");
      header--;
    }
    printf("%4.4d) %20.20s [%20.20s]  [%2.2d,%2.2d]\t%s\n",
           i++, m->symdef ? m->symdef->name : m->sym->name,
           m->abfd->filename,
           m->new_mask_bits, m->new_set_bits,
           m->duplicate_skip ? "[skip]" : "");
  }
}
