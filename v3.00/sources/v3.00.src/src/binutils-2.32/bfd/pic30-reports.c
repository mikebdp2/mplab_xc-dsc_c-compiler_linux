...
bfd_pic30_report_program_sections (asection *sect, FILE *fp) {
  ...
  bfd_size_type total = sect->size;
  bfd_size_type PCunits;
  bfd_size_type actual;
  ...
  if (REPORT_AS_PROGRAM(sect)) {
    ...
    actual_prog_memory_used += actual;
    free(name);
  }
...
bfd_pic30_report_auxflash_sections (asection *sect, FILE *fp) {
  ...
  if (REPORT_AS_AUXFLASH(sect)) {
      ...
      fprintf( fp, "%-24s%#10lx%#20lx%#16lx  (%ld)\n", name,
             start, PCunits, actual, actual);
      actual_auxflash_memory_used += actual;
...
bfd_pic30_report_data_sections (asection *sect, FILE *fp) {
  ...
  bfd_size_type total = sect->size / opb; /* ignore phantom bytes */
...
bfd_pic30_report_memory_usage (FILE *fp) {
  ...
  int small_data_size = is_isav32 ? 65535 : 8192;
  ...
  fprintf(fp,"\n\nxc-dsc-ld %s", pic30_resource_version); 
  if ((pic30_mem_info.ram[0] + pic30_mem_info.sfr[0]) > small_data_size) {
    fprintf(fp,
  ...
  for (region = pic30_all_regions(); region; region=region->next) {
  
    actual_prog_memory_used = 0;
    header_printed=0;
    region_name = strdup(region->name_list.name);
    ...
    if (header_printed) {
      bfd_size_type used = actual_prog_memory_used;
      ...
      fprintf(fp, "\n                   Total \"%s\" memory used (bytes):"
              "     %#10lx  (%ld) ",
              region->name_list.name,actual_prog_memory_used, actual_prog_memory_used);
      report_percent_used(used, region->length, fp);
      fprintf( fp, "\n");
    }
  }
  ...
  for (region = pic30_all_regions(); region; region=region->next) {
    ...
    data_memory_used = 0;
    fprintf( fp, "\n\n\"%s\" Memory  [Origin = 0x%lx, Length = 0x%lx]\n\n",
             region->name_list.name, region->origin, region->length);
    ...
    fprintf(fp, "\n                      Total \"%s\" memory used (bytes):"
            "     %#10lx  (%ld) ",
            region->name_list.name,data_memory_used, data_memory_used);
   ...
   */
  if (( (int) data_memory_used < (small_data_size - pic30_mem_info.sfr[0]))
      && (large_data_scalar == 1)
...
**
** Write an XML file for IDE to use.
** - memory region lengths displayed in decimal
** - enabled with hidden linker option --memorysummary
**
**/
static void
bfd_pic30_memory_summary (char *arg) {
  ...
  int is_isav32 = pic30_is_isav32_machine(global_PROCESSOR);
  int data_scale = is_isav32 ? 1 : 2;
  float program_scale = is_isav32 ? (1.0) : (.75);
       ...
       for (s = pic30_section_list; s != NULL; s=s->next)
          if ((s->sec) && in_bounds(s->sec, region))
            if (REPORT_AS_PROGRAM(s->sec))
              used += (int) (s->sec->size * program_scale);
